(() => {
  ...
  const STORAGE_KEY = `${APP_ID}:config:v30`;
  ...
  const allEffects = effectCatalog.flatMap((group) => group.items.map((effect) => ({ ...effect, group: group.group }))).concat(textEffectExtras.map((effect) => ({ ...effect, group: "Text" })));
  ...
  function featuresForCurrentTarget() {
    ...
    const preferredOrder = kind === "button"
      ? ["text-options", "heading-styles", "paragraph-styles", "link-styles"]
      : kind === "text"
        ? ["text-options", "heading-styles", "paragraph-styles", "link-styles"]
        : kind === "image"
          ? ["text-options", "heading-styles", "paragraph-styles", "link-styles"]
...
`;
  ...
  function setupCursorFollower(theme) {
    ...
    Object.entries(config.effects || {}).forEach(([selector, effects]) => {
      Object.entries(effects || {}).forEach(([id, settings]) => {
        if (settings && settings.enabled && /^txt-cursor-/.test(id)) entries.push({ selector, id, settings });
      });
    });
    ...
    cursorMoveHandler = (event) => {
      ...
      const large = entries.find((entry) => entry.selector === matched.selector && entry.id === "txt-cursor-large");
      const emoji = entries.find((entry) => entry.selector === matched.selector && entry.id === "txt-cursor-emoji");
      ...
      <div class="ssx-panel" role="dialog" aria-label="Studio Poema">
        <div class="ssx-head" data-ssx-drag>
          ...
          <div>
            ...
            <div class="ssx-subtitle">v30 · text studio</div>
          </div>
  ...
  function shouldRenderTextControls(section, effect) {
    ...
    return /^txt-format-/.test(effect.id);
  }
  ...
  function cleanTextLabel(label) {
    const raw = String(label || "").replace(/^(Text options|Formatting|Size|Line gap|Align|Heading|Paragraph|Links|Scroll|Loop|Hover|Block|Cursor)\s*[·:]\s*/i, "");
    const replacements = {
  ...
  function bindPanel(root) {
    ...
    root.querySelectorAll("[data-ssx-text-section-none]").forEach((button) => {
      button.addEventListener("click", () => {
        const section = textSections.find((item) => item.id === button.dataset.ssxTextSectionNone);
        if (!section) return;
    ...
    root.querySelectorAll("[data-ssx-feature-effect]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        const section = textSections.find((item) => item.options.includes(effectId));
        if (section) {
          pendingTextAnchor = section.id;
          pendingEffectAnchor = `${featureId}:${effectId}`;
          pendingEffectOffset = captureRelativeOffset(button);
        }
        if (!current.enabled && section && !["text-options", "block", "cursor"].includes(section.id)) {
          section.options.forEach((id) => {
    ...
    root.querySelectorAll("[data-ssx-switch-effect]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (section && section.id === "cursor" && (effectId === "txt-cursor-emoji" || effectId === "txt-cursor-large")) {
          const otherId = effectId === "txt-cursor-emoji" ? "txt-cursor-large" : "txt-cursor-emoji";
  ...
  function cleanRuntimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/studio-poema-v30.js?v=${defaultConfig.version}`;
    try {
