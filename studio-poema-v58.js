(() => {
  ...
  const STORAGE_KEY = `${APP_ID}:config:v58`;
  ...
  const buttonStyleIds = [
    "btn-style-gradient-sweep",
    "btn-style-slippery-shine",
    "btn-style-neon-outline",
    "btn-style-press-3d",
    "btn-style-magnetic-arrow",
    "btn-style-glass-glow",
    "btn-style-split-fill",
    "btn-style-liquid-bubble",
  ...
  const buttonStyleEffects = [
    {
      id: "btn-style-gradient-sweep",
      label: "Gradient sweep",
      description: "Rounded gradient button with a smooth animated light sweep.",
      ...
      css: (s, i, settings, theme) => {
        ...
        const radius = Math.round(10 + param(settings, "radius", 68) * .48);
        return `${buttonBaseCss(s, text, text)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;isolation:isolate!important;background:linear-gradient(135deg,${fill},${theme.gradientA})!important;color:${text}!important;border:1px solid ${hexToRgba(settings.outlineColor || hover,.34)}!important;border-radius:${radius}px!important;box-shadow:0 ${Math.round(8+i*20)}px ${Math.round(18+i*46)}px ${hexToRgba(fill,.22)}!important;transition:transform .22s ease,box-shadow .22s ease!important;} ${buttonTargets(s)}:before{content:"";position:absolute;inset:-40% -70%;background:linear-gradient(90deg,transparent,${hexToRgba(hover,.72)},transparent);transform:translateX(-60%) rotate(18deg);transition:transform ${Math.max(.38,1.05-i*.48).toFixed(2)}s ease;z-index:0;pointer-events:none;} ${buttonTargets(s)}:hover{transform:translateY(-${Math.round(1+i*5)}px)!important;} ${buttonTargets(s)}:hover:before{transform:translateX(60%) rotate(18deg);}`;
      }
    },
    {
      id: "btn-style-slippery-shine",
      label: "Slippery shine",
      description: "Glossy button with a fast diagonal shine on hover.",
      intensityLabel: "Shine",
      preview: "button-glass",
      css: (s, i, settings, theme) => {
        ...
        return `${buttonBaseCss(s, text, text)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(settings.outlineColor || theme.primary,.18)}!important;border-radius:${Math.round(10+param(settings,"radius",55)*.5)}px!important;box-shadow:inset 0 1px 0 ${hexToRgba("#ffffff",.5)},0 12px 28px ${hexToRgba(theme.primary,.10)}!important;transition:transform .2s ease!important;} ${buttonTargets(s)}:before{content:"";position:absolute;top:-90%;left:-60%;width:42%;height:280%;background:linear-gradient(90deg,transparent,${hexToRgba(shine,.82)},transparent);transform:rotate(24deg);transition:left ${Math.max(.28,.78-i*.34).toFixed(2)}s ease;pointer-events:none;z-index:0;} ${buttonTargets(s)}:hover:before{left:125%;} ${buttonTargets(s)}:hover{transform:translateY(-${Math.round(1+i*3)}px)!important;}`;
      }
    },
    {
      id: "btn-style-neon-outline",
      label: "Neon outline",
      description: "Outlined CTA with glow and dark editorial feel.",
      intensityLabel: "Glow",
      preview: "button-radius",
      css: (s, i, settings, theme) => {
        const outline = settings.outlineColor || theme.accent;
        const text = settings.textColor || outline;
        const fill = settings.color || "transparent";
        return `${buttonBaseCss(s, text, settings.hoverColor || theme.ink)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;background:${fill}!important;color:${text}!important;border:${Math.max(1,Math.round(1+i*3))}px solid ${outline}!important;border-radius:${Math.round(8+param(settings,"radius",48)*.44)}px!important;box-shadow:0 0 ${Math.round(8+i*30)}px ${hexToRgba(outline,.28)},inset 0 0 ${Math.round(4+i*16)}px ${hexToRgba(outline,.10)}!important;text-shadow:0 0 ${Math.round(4+i*12)}px ${hexToRgba(outline,.35)};transition:background .22s ease,color .22s ease,transform .22s ease!important;} ${buttonTargets(s)}:hover{background:${outline}!important;color:${settings.hoverColor || theme.ink}!important;transform:translateY(-${Math.round(i*4)}px)!important;}`;
      }
    },
    {
      id: "btn-style-press-3d",
      label: "Pressed 3D",
      description: "Tactile offset button that presses down on hover.",
      intensityLabel: "Depth",
      preview: "button-3d",
      css: (s, i, settings, theme) => {
        const fill = settings.color || theme.accent;
        const outline = settings.outlineColor || theme.ink;
        const depth = Math.round(4 + i * 12);
        return `${buttonBaseCss(s, settings.textColor || theme.ink, settings.textColor || theme.ink)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;background:${fill}!important;color:${settings.textColor || theme.ink}!important;border:2px solid ${outline}!important;border-radius:${Math.round(6+param(settings,"radius",35)*.34)}px!important;box-shadow:${depth}px ${depth}px 0 ${outline}!important;transform:translate(0,0)!important;transition:transform .15s ease,box-shadow .15s ease!important;} ${buttonTargets(s)}:hover{transform:translate(${Math.round(depth*.45)}px,${Math.round(depth*.45)}px)!important;box-shadow:${Math.round(depth*.35)}px ${Math.round(depth*.35)}px 0 ${outline}!important;}`;
      }
    },
    {
      id: "btn-style-magnetic-arrow",
      label: "Magnetic arrow",
      description: "Pill button with arrow movement for premium CTAs.",
      intensityLabel: "Motion",
      preview: "button-arrow",
      css: (s, i, settings, theme) => {
        const fill = settings.color || theme.primary;
        const text = settings.textColor || theme.soft;
        const hover = settings.hoverColor || theme.accent;
        return `${buttonBaseCss(s, text, text)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(settings.outlineColor || fill,.2)}!important;border-radius:999px!important;padding-right:${Math.round(34+i*18)}px!important;transition:transform .22s cubic-bezier(.2,1.4,.3,1),background .22s ease!important;} ${buttonTargets(s)}:after{content:"↗";position:absolute;right:${Math.round(12+i*5)}px;top:50%;transform:translateY(-50%);width:22px;height:22px;border-radius:999px;background:${hover};color:${theme.ink};display:grid;place-items:center;line-height:1;transition:transform .22s ease;z-index:3;} ${buttonTargets(s)}:hover{transform:translateY(-${Math.round(1+i*5)}px) scale(${(1+i*.035).toFixed(2)})!important;} ${buttonTargets(s)}:hover:after{transform:translate(${Math.round(3+i*8)}px,calc(-50% - ${Math.round(1+i*4)}px));}`;
      }
    },
    {
      id: "btn-style-glass-glow",
      label: "Glass glow",
      description: "Soft glass button with blur and luminous edge.",
      intensityLabel: "Glass",
      preview: "button-glass",
      css: (s, i, settings, theme) => {
        const fill = settings.color || theme.soft;
        const outline = settings.outlineColor || theme.primary;
        return `${buttonBaseCss(s, settings.textColor || theme.ink, settings.textColor || theme.ink)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;background:${hexToRgba(fill,.42+i*.24)}!important;color:${settings.textColor || theme.ink}!important;border:1px solid ${hexToRgba(outline,.18+i*.25)}!important;border-radius:${Math.round(12+param(settings,"radius",62)*.48)}px!important;backdrop-filter:blur(${Math.round(8+i*18)}px) saturate(1.2)!important;-webkit-backdrop-filter:blur(${Math.round(8+i*18)}px) saturate(1.2)!important;box-shadow:0 ${Math.round(12+i*22)}px ${Math.round(24+i*50)}px ${hexToRgba(outline,.14)},inset 0 1px 0 ${hexToRgba("#ffffff",.48)}!important;transition:transform .22s ease,box-shadow .22s ease!important;} ${buttonTargets(s)}:hover{transform:translateY(-${Math.round(1+i*4)}px)!important;box-shadow:0 ${Math.round(16+i*28)}px ${Math.round(30+i*58)}px ${hexToRgba(settings.hoverColor || theme.accent,.22)}!important;}`;
      }
    },
    {
      id: "btn-style-split-fill",
      label: "Split fill",
      description: "Button fills from both sides on hover.",
      intensityLabel: "Fill",
      preview: "button-line",
      css: (s, i, settings, theme) => {
        const fill = settings.color || theme.soft;
        const hover = settings.hoverColor || theme.primary;
        return `${buttonBaseCss(s, settings.textColor || theme.ink, settings.hoverTextColor || theme.soft)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;isolation:isolate!important;background:${fill}!important;color:${settings.textColor || theme.ink}!important;border:1px solid ${settings.outlineColor || theme.primary}!important;border-radius:${Math.round(8+param(settings,"radius",36)*.42)}px!important;transition:color .22s ease,transform .22s ease!important;} ${buttonTargets(s)}:before,${buttonTargets(s)}:after{content:"";position:absolute;top:0;width:51%;height:100%;background:${hover};z-index:0;transition:transform ${Math.max(.28,.75-i*.32).toFixed(2)}s ease;pointer-events:none;} ${buttonTargets(s)}:before{left:0;transform:translateX(-105%);} ${buttonTargets(s)}:after{right:0;transform:translateX(105%);} ${buttonTargets(s)}:hover{color:${settings.hoverTextColor || theme.soft}!important;transform:translateY(-${Math.round(i*3)}px)!important;} ${buttonTargets(s)}:hover:before,${buttonTargets(s)}:hover:after{transform:translateX(0);}`;
      }
    },
    {
      id: "btn-style-liquid-bubble",
      label: "Liquid bubble",
      description: "Rounded button with soft liquid bubble movement.",
      intensityLabel: "Bubble",
      preview: "button-bubbly",
      css: (s, i, settings, theme) => {
        const fill = settings.color || theme.gradientB;
        const bubble = settings.hoverColor || theme.accent;
        return `${buttonBaseCss(s, settings.textColor || theme.ink, settings.textColor || theme.ink)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;background:${fill}!important;color:${settings.textColor || theme.ink}!important;border:1px solid ${hexToRgba(settings.outlineColor || theme.primary,.16)}!important;border-radius:999px!important;box-shadow:inset 0 -${Math.round(4+i*8)}px 0 ${hexToRgba(theme.primary,.16)},0 14px 30px ${hexToRgba(theme.primary,.10)}!important;transition:transform .24s cubic-bezier(.2,1.6,.3,1)!important;} ${buttonTargets(s)}:before{content:"";position:absolute;width:${Math.round(28+i*70)}px;height:${Math.round(28+i*70)}px;border-radius:999px;background:${hexToRgba(bubble,.5)};left:50%;top:120%;transform:translate(-50%,0);transition:top ${Math.max(.28,.8-i*.3).toFixed(2)}s ease;pointer-events:none;z-index:0;} ${buttonTargets(s)}:hover{transform:scale(${(1+i*.06).toFixed(2)})!important;} ${buttonTargets(s)}:hover:before{top:50%;}`;
      }
    },
    {
...
`;
      ...
      <div class="ssx-panel" role="dialog" aria-label="Studio Poema">
        <div class="ssx-head" data-ssx-drag>
          ...
          <div>
            ...
            <div class="ssx-subtitle">v58 · button styles fix</div>
          </div>
  ...
  function renderButtonFeatureControls(effect, settings, disabled) {
    const theme = config.theme || defaultConfig.theme;
    const radius = param(settings, "radius", effect.id === "btn-style-circle-360" ? 50 : 55);
    ...
    return `
      <div class="ssx-option-controls">
        ...
        <div>
          ...
          ${effect.id === "btn-style-circle-360" ? `
            <label class="ssx-control-line"><span>Circle text</span><input type="text" value="${escapeHtml(settings.customText || "")}" data-ssx-custom-text-effect="${escapeHtml(effect.id)}" maxlength="18" placeholder="POEMA" ${disabled ? "disabled" : ""}></label>
            <div class="ssx-range-row">
              ...
              <div class="ssx-range-label">Circle size · ${Math.round(76 + size * .86)}px</div>
            </div>
            <div class="ssx-range-row">
  ...
  function bindPanel(root) {
        ...
        const paramLabel = input.dataset.ssxParamKey === "outlineSize"
          ...
          : input.dataset.ssxParamKey === "outlineWidth"
            ...
            : input.dataset.ssxParamKey === "size"
              ? `Circle size · ${Math.round(76 + amount * .86)}px`
    ...
    root.querySelectorAll("[data-ssx-custom-text-effect]").forEach((input) => {
      input.addEventListener("input", () => {
        ...
        const autoEnable = /^(txt|img|site)-cursor-emoji$/.test(input.dataset.ssxCustomTextEffect) || input.dataset.ssxCustomTextEffect === "btn-style-circle-360";
  ...
  function cleanRuntimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/studio-poema-v58.js?v=${defaultConfig.version}`;
    try {
