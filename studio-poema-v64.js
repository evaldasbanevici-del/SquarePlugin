(() => {
  ...
  const STORAGE_KEY = `${APP_ID}:config:v64`;
  ...
  const buttonStyleEffects = [
    {
      ...
      css: (s, i, settings, theme) => {
        ...
        return `${buttonBaseCss(s, text, text)} ${buttonStableSizeCss(s, settings)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(outline,.36)}!important;border-radius:${radius}px!important;font-weight:900!important;box-shadow:0 10px 24px ${hexToRgba(fill,.16)},inset 0 1px 0 ${hexToRgba("#ffffff",.38)}!important;transition:background .18s ease,box-shadow .18s ease!important;transform-origin:center!important;will-change:transform!important;} ${buttonTargets(s)}:before{content:"";position:absolute;inset:-30%;background:radial-gradient(circle at 50% 120%,${hexToRgba("#ffffff",.32)},transparent 46%);opacity:.55;transform:translateY(24%);transition:transform .22s ease,opacity .22s ease;z-index:0;pointer-events:none;} ${buttonTargets(s)}:hover{background:${hover}!important;animation:ssxBtnBouncyCta ${speed}s cubic-bezier(.22,1.55,.36,1) both!important;box-shadow:0 ${Math.round(12+i*12)}px ${Math.round(26+i*22)}px ${hexToRgba(hover,.2)},inset 0 1px 0 ${hexToRgba("#ffffff",.46)}!important;} ${buttonTargets(s)}:hover:before{transform:translateY(0);opacity:.9;} ${buttonTargets(s)}:active{transform:scale(.96)!important;} @keyframes ssxBtnBouncyCta{0%{transform:translateY(0) scale(1)}35%{transform:translateY(-${lift}px) scale(${stretch})}58%{transform:translateY(1px) scale(${squash})}78%{transform:translateY(-1px) scale(1.018)}100%{transform:translateY(0) scale(1)}}`;
      }
    },
    {
  ...
  function restoreButtonAdapter(button) {
    ...
    ["ssx-btn-letter-flip", "ssx-btn-swap-arrow", "ssx-btn-arrow-push", "ssx-button-adapter"].forEach((className) => button.classList.remove(className));
...
`;
      ...
      <div class="ssx-panel" role="dialog" aria-label="Studio Poema">
        <div class="ssx-head" data-ssx-drag>
          ...
          <div>
            ...
            <div class="ssx-subtitle">v64 · bouncy button</div>
          </div>
  ...
  function cleanRuntimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/studio-poema-v64.js?v=${defaultConfig.version}`;
    try {
