(() => {
  ...
  const STORAGE_KEY = `${APP_ID}:config:v38`;
  ...
  const textEffectExtras = [
    ...
    },
    {
      ...
      description: "Text sharpens from blur with a soft Framer-style reveal.",
      intensityLabel: "Speed",
      preview: "motion-reveal",
      className: "ssx-reveal",
      params: [
      ...
      css: (s, i, settings) => {
        const duration = Math.max(.5, 1.55 - i * .72).toFixed(2);
        const distance = Math.round(10 + param(settings, "distance", 42) * 1.35);
        const blur = Math.round(3 + param(settings, "blur", 62) * .22);
        return `${revealTextTargets(s)}{transition:opacity ${duration}s ease,transform ${duration}s cubic-bezier(.18,.82,.22,1),filter ${duration}s ease;filter:blur(0);will-change:opacity,transform,filter;} ${hiddenRevealTextTargets(s)}{opacity:0;transform:translateY(${distance}px);filter:blur(${blur}px);}`;
      }
    },
    {
      id: "txt-loop-pulse",
  ...
  let activeMode = config.mode || "Studio";
  let activeTool = null;
  ...
  function normalizeConfig(input) {
    ...
    next.mode = ["Studio", "Design", "Presets", "Advanced"].includes(next.mode) ? next.mode : "Studio";
    next.version = 37;
    return next;
  }
  ...
  function applyEffects() {
    ...
    document.querySelectorAll(".ssx-reveal,.ssx-stagger,.ssx-parallax").forEach((element) => {
      element.classList.remove("ssx-reveal", "ssx-stagger", "ssx-parallax");
    });
...
`;
  ...
  function ensureBaseStyles() {
    ...
    style.textContent = `
      ...
      .ssx-mode-tabs{display:grid;grid-template-columns:repeat(4,1fr);border-bottom:1px solid rgba(255,255,255,.10);background:#242424}
      ...
      <div class="ssx-panel" role="dialog" aria-label="Studio Poema">
        <div class="ssx-head" data-ssx-drag>
          ...
          <div>
            ...
            <div class="ssx-subtitle">v38 · clean studio</div>
          </div>
  ...
  function cleanRuntimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/studio-poema-v38.js?v=${defaultConfig.version}`;
    try {
