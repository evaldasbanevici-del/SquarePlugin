/* ============================================================
   ...
   File: FormaPlugin-v3.js · Single-file injection · No dependencies
...
(() => {
  ...
  },
  {
    ...
    items: [
      ...
      { id: "nv-pill", label: "Pill header bar", desc: "The inner header bar becomes a rounded glass pill. Layout untouched.", amount: "Round", global: true,
        params: [{ k: "glass", label: "Glass", v: 55 }, { k: "bg", label: "Pill color", type: "color", v: "#FCF6EB" }],
        css: (s, t, p, th) => {
  ...
  const state = {
    ...
    picking: false,
    hoverEl: null,
    ui: { open: new Set(["__colors"]), openFx: null, search: "", scrollTop: 0, showPub: false, showDiag: false }
  };
...
#${IDS.root} .fm-target{display:flex;gap:7px;padding:10px 14px;background:#fff;border-bottom:1px solid #eef1ec}
...
`;
  ...
  /* ---------- target dropdown: Page / Sections / Custom ---------- */
  function targetOptionsHtml() {
    const c = state.config;
    const secs = [];
    document.querySelectorAll("[data-section-id]").forEach((el, i) => {
      if (el.closest(`#${IDS.root}`)) return;
      const id = el.getAttribute("data-section-id");
      const head = el.querySelector("h1,h2,h3");
      const txt = head ? head.textContent.trim().slice(0, 24) : "";
      secs.push({ sel: `[data-section-id="${cssEsc(id)}"]`, label: `Section ${i + 1}${txt ? " — " + txt : ""}` });
    });
    const isSection = secs.some(s => s.sel === c.target);
    const isCustom = c.target !== "body" && !isSection;
    return `
<optgroup label="Page">
  <option value="body" ${c.target === "body" ? "selected" : ""}>Entire page</option>
</optgroup>
<optgroup label="Sections">
  ${secs.map(s => `<option value="${esc(s.sel)}" ${s.sel === c.target ? "selected" : ""}>${esc(s.label)}</option>`).join("")}
</optgroup>
<optgroup label="Custom">
  ${isCustom ? `<option value="${esc(c.target)}" selected>Selected: ${esc(c.target.slice(0, 30))}</option>` : ""}
  <option value="__pick">Click an element on the page…</option>
</optgroup>`;
  }
...
<div class="fm-panel" role="dialog" aria-label="Forma">
  ...
  <div class="fm-target">
    <select data-act="targetsel" aria-label="Edit target">${targetOptionsHtml()}</select>
  </div>
...
<div class="fm-pub">
  ...
  function bindPanel(root) {
    ...
    root.addEventListener("change", ev => {
      if (ev.target.dataset.act === "targetsel") {
        if (ev.target.value === "__pick") { startPicking(); return; }
        state.config.target = ev.target.value;
        state.config.scope = "selected";
        saveConfig(); applyEffects(); renderPanel();
        flash(state.config.target);
      }
  ...
  /* ============================================================
     ...
     ============================================================ */
  function runtimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/FormaPlugin-v3.js`;
    try {
