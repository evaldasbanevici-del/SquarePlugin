(() => {
  ...
  const STORAGE_KEY = `${APP_ID}:config:v79`;
  ...
  const cursorEmojiOptions = ["☺", "😊", "✨", "↗", "👀", "🔥", "💚", "✦", "★", "•", "✺", "POEMA"];
  const textCursorVisualIds = [
    "txt-cursor-dot",
    "txt-cursor-ring",
    "txt-cursor-trail",
    "txt-cursor-glow",
    "txt-cursor-spotlight",
    "txt-cursor-crosshair",
    "txt-cursor-outline",
    "txt-cursor-emoji",
    "txt-cursor-large"
  ];
  const imageCursorVisualIds = [
    "img-cursor-dot",
    "img-cursor-ring",
    "img-cursor-trail",
    "img-cursor-glow",
    "img-cursor-spotlight",
    "img-cursor-crosshair",
    "img-cursor-emoji",
    "img-cursor-large"
  ];
  const siteCursorVisualIds = [
    "site-cursor-native-dot",
    "site-cursor-dot",
    "site-cursor-ring",
    "site-cursor-trail",
    "site-cursor-glow",
    "site-cursor-spotlight",
    "site-cursor-crosshair",
    "site-cursor-outline",
    "site-cursor-emoji",
    "site-cursor-large"
  ];
  const buttonStyleIds = [
    "btn-style-animated-arrow-pill",
    "btn-style-circle-expand",
    "btn-style-line-fill-arrow",
    "btn-style-rotating-circle",
    "btn-style-letter-flip-v2",
    "btn-style-shine-fill-v2",
    "btn-style-arrow-nudge-v2",
    "btn-style-bouncy-v2",
    "btn-style-soft-glass",
    "btn-style-outline-slide"
  ];
  const retiredButtonStyleIds = [
    "btn-style-bouncy-cta",
    "btn-style-letter-flip",
    "btn-style-swap-arrow",
    "btn-style-arrow-push",
    "btn-style-shine-fill"
  ];

  function mediaTargets(s) {
    return `${s} img,${s} video,${s}.image-card,${s} .image-card,${s}.sqs-block-image img,${s} .sqs-block-image img,${s} .image-block-wrapper img,${s} .thumb-image,${s} picture img`;
  }

  function mediaWrappers(s) {
    return `${s}.image-card,${s} .image-card,${s}.sqs-block-image,${s} .sqs-block-image,${s} .image-block-wrapper,${s} figure,${s} picture`;
  }

  function mediaSurfaces(s) {
    return `${mediaWrappers(s)},${mediaTargets(s)}`;
  }

  function imageMotionTargets(s) {
    const target = `:is(img,picture,figure,.image-block,.sqs-block-image,.fluid-image-container,.image-card)`;
    return `.ssx-reveal${target},.ssx-parallax${target},${s}.ssx-reveal${target},${s} .ssx-reveal${target},${s}.ssx-parallax${target},${s} .ssx-parallax${target}`;
  }

  function hiddenImageMotionTargets(s) {
    const target = `:is(img,picture,figure,.image-block,.sqs-block-image,.fluid-image-container,.image-card)`;
    return `.ssx-reveal:not(.ssx-in)${target},.ssx-parallax:not(.ssx-in)${target},${s}.ssx-reveal:not(.ssx-in)${target},${s} .ssx-reveal:not(.ssx-in)${target},${s}.ssx-parallax:not(.ssx-in)${target},${s} .ssx-parallax:not(.ssx-in)${target}`;
  }

  function textTargets(s) {
    const t = `:is(h1,h2,h3,h4,p,li,blockquote,.sqsrte-large,.sqsrte-small,a:not(:has(img,svg,picture,video,canvas,.image-card,.sqs-block-image,.image-block-wrapper,.fluid-image-container)))`;
    const safe = `${t}:not(:has(img,picture,video,iframe,canvas,svg,button,.image-card,.sqs-block-image,.image-block-wrapper,.fluid-image-container))`;
    return `${s}${safe},${s} ${safe}`;
  }

  function headingTargets(s) {
    const safe = `:is(h1,h2,h3,h4):not(:has(img,picture,video,iframe,canvas,svg,button,.image-card,.sqs-block-image,.image-block-wrapper,.fluid-image-container))`;
    return `${s}${safe},${s} ${safe}`;
  }

  function paragraphTargets(s) {
    const safe = `:is(p,li,blockquote,.sqsrte-small,a:not(:has(img,svg,picture,video,canvas,.image-card,.sqs-block-image,.image-block-wrapper,.fluid-image-container))):not(:has(img,picture,video,iframe,canvas,svg,button,.image-card,.sqs-block-image,.image-block-wrapper,.fluid-image-container))`;
    return `${s}${safe},${s} ${safe}`;
  }
  ...
  function revealTextTargets(s) {
    const t = `:is(h1,h2,h3,h4,p,li,blockquote,.sqsrte-large,.sqsrte-small,a:not(:has(img,svg,picture,video,canvas,.image-card,.sqs-block-image,.image-block-wrapper,.fluid-image-container)))`;
    const safe = `${t}:not(:has(img,picture,video,iframe,canvas,svg,button,.image-card,.sqs-block-image,.image-block-wrapper,.fluid-image-container))`;
    return `.ssx-reveal${safe},.ssx-parallax${safe},${s}.ssx-reveal${safe},${s}.ssx-reveal ${safe},${s}.ssx-parallax${safe},${s}.ssx-parallax ${safe}`;
  }

  function hiddenRevealTextTargets(s) {
    const t = `:is(h1,h2,h3,h4,p,li,blockquote,.sqsrte-large,.sqsrte-small,a:not(:has(img,svg,picture,video,canvas,.image-card,.sqs-block-image,.image-block-wrapper,.fluid-image-container)))`;
    const safe = `${t}:not(:has(img,picture,video,iframe,canvas,svg,button,.image-card,.sqs-block-image,.image-block-wrapper,.fluid-image-container))`;
    return `.ssx-reveal:not(.ssx-in)${safe},.ssx-parallax:not(.ssx-in)${safe},${s}.ssx-reveal:not(.ssx-in)${safe},${s}.ssx-reveal:not(.ssx-in) ${safe},${s}.ssx-parallax:not(.ssx-in)${safe},${s}.ssx-parallax:not(.ssx-in) ${safe}`;
  }

  function wordRevealWordTargets(s) {
    const safe = `:is(h1,h2,h3,h4,p,li,blockquote,.sqsrte-large,.sqsrte-small):not(:has(img,picture,video,iframe,canvas,svg,button,.image-card,.sqs-block-image,.image-block-wrapper,.fluid-image-container))`;
    return `.ssx-word-blur-reveal${safe} .ssx-word,${s}.ssx-word-blur-reveal${safe} .ssx-word,${s}.ssx-word-blur-reveal ${safe} .ssx-word`;
  }

  function activeWordRevealWordTargets(s) {
    const safe = `:is(h1,h2,h3,h4,p,li,blockquote,.sqsrte-large,.sqsrte-small):not(:has(img,picture,video,iframe,canvas,svg,button,.image-card,.sqs-block-image,.image-block-wrapper,.fluid-image-container))`;
    return `.ssx-word-blur-reveal.ssx-in${safe} .ssx-word,${s}.ssx-word-blur-reveal.ssx-in${safe} .ssx-word,${s}.ssx-word-blur-reveal.ssx-in ${safe} .ssx-word`;
  }

  function wordLoopTargets(s) {
    const safe = `:is(h1,h2,h3,h4,p,li,blockquote,.sqsrte-large,.sqsrte-small):not(:has(img,picture,video,iframe,canvas,svg,button,.image-card,.sqs-block-image,.image-block-wrapper,.fluid-image-container))`;
    return `${s}${safe},${s} ${safe}`;
  }

  function buttonTargets(s) {
    return `${s}:is(.sqs-button-element--primary,.sqs-button-element--secondary,.sqs-button-element--tertiary,a.sqs-block-button-element,button.sqs-block-button-element),${s} .sqs-button-element--primary,${s} .sqs-button-element--secondary,${s} .sqs-button-element--tertiary,${s} a.sqs-block-button-element,${s} button.sqs-block-button-element`;
  }

  function buttonClassTargets(s, className) {
    return `${s}:is(.sqs-button-element--primary,.sqs-button-element--secondary,.sqs-button-element--tertiary,a.sqs-block-button-element,button.sqs-block-button-element).${className},${s} .sqs-button-element--primary.${className},${s} .sqs-button-element--secondary.${className},${s} .sqs-button-element--tertiary.${className},${s} a.sqs-block-button-element.${className},${s} button.sqs-block-button-element.${className}`;
  }

  function buttonAdapterBaseCss(s, className, text, hoverText) {
    const target = buttonClassTargets(s, className);
    return `${target}{display:inline-flex!important;align-items:center!important;justify-content:center!important;text-align:center!important;line-height:1.05!important;text-decoration:none!important;transform-origin:center!important;vertical-align:middle!important;box-sizing:border-box!important;isolation:isolate!important;position:relative!important;} ${target} .ssx-btn-label,${target} .ssx-btn-text,${target} .ssx-btn-char{color:${text}!important;text-decoration:none!important;} ${target}:hover .ssx-btn-label,${target}:hover .ssx-btn-text,${target}:hover .ssx-btn-char{color:${hoverText || text}!important;}`;
  }

  function buttonAdapterStableSizeCss(s, className, settings) {
    const target = buttonClassTargets(s, className);
    const width = Math.round(14 + param(settings, "width", 50) * .46);
    const height = Math.round(38 + param(settings, "height", 44) * .34);
    return `${target}{min-height:${height}px!important;padding:${Math.max(8, Math.round(height * .28))}px ${width}px!important;} ${target}:hover{transform:none!important;}`;
  }

  function buttonTextTargets(s) {
    return `${buttonTargets(s)} span,${buttonTargets(s)} strong,${buttonTargets(s)} em,${buttonTargets(s)} .sqs-button-element--inner`;
  }

  function buttonHoverTextTargets(s) {
    return `${buttonTargets(s)}:hover span,${buttonTargets(s)}:hover strong,${buttonTargets(s)}:hover em`;
  }

  function buttonBaseCss(s, text, hoverText) {
    return `${buttonTargets(s)}{position:relative!important;box-sizing:border-box!important;text-align:center!important;text-decoration:none!important;transform-origin:center!important;will-change:transform,box-shadow,background,color!important;} ${buttonTextTargets(s)}{position:relative!important;z-index:2!important;color:${text}!important;text-decoration:none!important;} ${buttonTargets(s)}:hover,${buttonHoverTextTargets(s)}{color:${hoverText || text}!important;}`;
  }

  function buttonStableSizeCss(s, settings) {
    const width = Math.round(12 + param(settings, "width", 50) * .42);
    const height = Math.round(36 + param(settings, "height", 44) * .38);
    return `${buttonTargets(s)}{box-sizing:border-box!important;min-height:${height}px!important;padding:${Math.max(8, Math.round(height * .28))}px ${width}px!important;vertical-align:middle!important;}`;
  }
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          css: (s, i, settings) => `${mediaWrappers(s)}{overflow:hidden!important;} ${mediaTargets(s)}{transition:transform ${(0.25 + param(settings, "speed", 45) * 0.012).toFixed(2)}s cubic-bezier(.2,.8,.2,1)!important;} ${mediaWrappers(s)}:hover img,${s}:hover img{}`
        },
        {
          id: "image-duotone",
          label: "Duotone tint",
      ...
      items: [
        {
          ...
          css: (s, i, settings) => `${s}{transition:transform ${(0.12 + param(settings,"speed",45)*.009).toFixed(2)}s ease,opacity ${(0.12 + param(settings,"speed",45)*.009).toFixed(2)}s ease,filter ${(0.12 + param(settings,"speed",45)*.009).toFixed(2)}s ease!important;} ${s}:hover{opacity:${(param(settings,"opacity",100)/100).toFixed(2)}!important;}`
        },
        {
  ...
  const textEffectExtras = [
    {
      ...
      intensityLabel: "Weight",
      preview: "type-highlight",
      css: (s, i, _settings, theme) => `${textTargets(s)}{text-decoration:line-through;text-decoration-color:${theme.primary};text-decoration-thickness:${Math.round(1 + i * 5)}px;text-decoration-skip-ink:auto;}`
    },
    {
      ...
      intensityLabel: "Mix",
      preview: "type-editorial",
      css: (s, i, _settings, theme) => `${textTargets(s)}{color:${i > .5 ? theme.primary : theme.ink}!important;}`
    },
    {
      id: "txt-color-accent",
    ...
    },
    {
      ...
      intensityLabel: "Gap",
      preview: "type-balance",
      css: (s, i) => `${paragraphTargets(s)}{line-height:${(1.25 + i * .75).toFixed(2)}!important;} ${headingTargets(s)}{line-height:${(0.92 + i * .34).toFixed(2)}!important;}`
    },
    {
      id: "txt-align-left",
      label: "Align · Left",
    ...
    {
      id: "txt-word-loop",
      label: "Loop · Word loop",
      description: "Type the marker word 'loop' in your text, then replace it with rotating words.",
      intensityLabel: "Speed",
      preview: "motion-reveal",
      params: [
        { key: "style", label: "Transition style", value: 50 }
      ],
      css: (s, i, settings, theme) => {
        const duration = Math.max(.18, .62 - i * .34).toFixed(2);
        const style = param(settings, "style", 50);
        return `${wordLoopTargets(s)} .ssx-word-loop{display:inline-grid!important;grid-template-areas:"word";vertical-align:baseline;min-width:var(--ssx-word-loop-width,3ch);color:${theme.primary};font-weight:inherit;white-space:nowrap;} ${wordLoopTargets(s)} .ssx-word-loop-word{grid-area:word;display:inline-block;transition:opacity ${duration}s ease,transform ${duration}s cubic-bezier(.18,.82,.22,1),filter ${duration}s ease;will-change:opacity,transform,filter;} ${wordLoopTargets(s)} .ssx-word-loop.is-slide .ssx-word-loop-word{animation:ssxWordLoopSlide ${duration}s cubic-bezier(.18,.82,.22,1) both;} ${wordLoopTargets(s)} .ssx-word-loop.is-blur .ssx-word-loop-word{animation:ssxWordLoopBlur ${duration}s ease both;} ${wordLoopTargets(s)} .ssx-word-loop.is-instant .ssx-word-loop-word{transition:none;animation:none;}@keyframes ssxWordLoopSlide{0%{opacity:0;transform:translateY(60%)}100%{opacity:1;transform:translateY(0)}}@keyframes ssxWordLoopBlur{0%{opacity:0;filter:blur(9px);transform:translateY(8px)}100%{opacity:1;filter:blur(0);transform:translateY(0)}}`;
      }
    },
    {
      id: "txt-loop-pulse",
      ...
      intensityLabel: "Pulse",
      preview: "engagement-pulse",
      css: (s, i) => `${textTargets(s)}{display:inline-block;animation:ssxTxtPulse ${Math.max(1.6,3.8-i*1.9).toFixed(1)}s ease-in-out infinite;}@keyframes ssxTxtPulse{0%,100%{transform:scale(1)}50%{transform:scale(${(1.01+i*.06).toFixed(2)})}}`
    },
    {
      id: "txt-loop-hover",
      label: "Loop · Float",
      ...
      intensityLabel: "Float",
      preview: "engagement-hover",
      css: (s, i) => `${textTargets(s)}{display:inline-block;animation:ssxTxtFloat ${Math.max(2.4,5-i*2).toFixed(1)}s ease-in-out infinite;}@keyframes ssxTxtFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-${Math.round(2+i*10)}px)}}`
    },
    {
      id: "txt-loop-fade",
      label: "Loop · Fade",
      ...
      intensityLabel: "Fade",
      preview: "motion-reveal",
      css: (s, i) => `${textTargets(s)}{animation:ssxTxtFade ${Math.max(2,5-i*2).toFixed(1)}s ease-in-out infinite;}@keyframes ssxTxtFade{0%,100%{opacity:1}50%{opacity:${(.45+i*.35).toFixed(2)}}}`
    },
    {
      id: "txt-loop-spin",
      label: "Loop · Spin",
      ...
      intensityLabel: "Spin",
      preview: "motion-spin",
      css: (s, i) => `${textTargets(s)}{display:inline-block;animation:ssxTxtSpin ${Math.max(4,12-i*7).toFixed(1)}s linear infinite;}@keyframes ssxTxtSpin{to{transform:rotate(360deg)}}`
    },
    {
      id: "txt-loop-shake",
      label: "Loop · Shake",
      ...
      preview: "engagement-wobble",
      css: (s, i) => `${textTargets(s)}{display:inline-block;animation:ssxTxtShake ${Math.max(.7,1.8-i*.8).toFixed(1)}s ease-in-out infinite;}@keyframes ssxTxtShake{0%,100%{transform:translateX(0)}25%{transform:translateX(-${Math.round(1+i*5)}px)}75%{transform:translateX(${Math.round(1+i*5)}px)}}`
    },
    {
      id: "txt-loop-wiggle",
      label: "Loop · Wiggle",
      ...
      preview: "engagement-wobble",
      css: (s, i) => `${textTargets(s)}{display:inline-block;animation:ssxTxtWiggle ${Math.max(1.2,3-i*1.3).toFixed(1)}s ease-in-out infinite;}@keyframes ssxTxtWiggle{0%,100%{transform:rotate(0)}25%{transform:rotate(-${Math.round(1+i*5)}deg)}75%{transform:rotate(${Math.round(1+i*5)}deg)}}`
    },
    {
      id: "txt-hover-scale",
      label: "Hover · Scale",
      ...
      intensityLabel: "Scale",
      preview: "hover-scale",
      css: (s, i) => `${textTargets(s)}{transition:transform .22s ease;transform-origin:center;} ${textTargets(s)}:hover{transform:scale(${(1.01+i*.055).toFixed(2)});}`
    },
    {
      id: "txt-hover-up",
      label: "Hover · Lift up",
      ...
      intensityLabel: "Lift",
      preview: "button-lift",
      css: (s, i) => `${textTargets(s)}{transition:transform .22s ease;transform-origin:center;} ${textTargets(s)}:hover{transform:translateY(-${Math.round(1+i*7)}px);}`
    },
    {
      id: "txt-hover-pop",
      label: "Hover · Pop",
      ...
      intensityLabel: "Pop",
      preview: "engagement-jelly",
      css: (s, i) => `${textTargets(s)}{transition:transform .2s cubic-bezier(.2,1.45,.3,1);transform-origin:center;} ${textTargets(s)}:hover{transform:scale(${(1.02+i*.08).toFixed(2)}) rotate(-${Math.round(i*2)}deg);}`
    },
    {
      id: "txt-hover-tilt",
      label: "Hover · Tilt",
      ...
      intensityLabel: "Tilt",
      preview: "type-3d",
      css: (s, i) => `${textTargets(s)}{transition:transform .22s ease;transform-origin:center;transform-style:preserve-3d;} ${textTargets(s)}:hover{transform:perspective(900px) rotateX(${Math.round(1+i*5)}deg) rotateY(-${Math.round(1+i*5)}deg);}`
    },
    {
      id: "txt-hover-fade",
      label: "Hover · Fade",
      ...
      intensityLabel: "Fade",
      preview: "motion-reveal",
      css: (s, i) => `${textTargets(s)}{transition:opacity .2s ease;} ${textTargets(s)}:hover{opacity:${(.62+i*.28).toFixed(2)};}`
    },
    {
      id: "txt-hover-flip",
      label: "Hover · Flip",
      ...
      intensityLabel: "Flip",
      preview: "type-3d",
      css: (s, i) => `${textTargets(s)}{transform-style:preserve-3d;transition:transform .35s ease;transform-origin:center;} ${textTargets(s)}:hover{transform:perspective(900px) rotateX(${Math.round(10+i*45)}deg);}`
    },
    {
    ...
    {
      id: "txt-cursor-dot",
      label: "Cursor · Soft dot",
      description: "Replaces the cursor with a clean brand-colored dot.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
    {
      id: "txt-cursor-ring",
      label: "Cursor · Ring",
      description: "Adds a minimal outlined cursor for editorial pages.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
    {
      id: "txt-cursor-trail",
      label: "Cursor · Dot follower",
      description: "Adds a dot with a smooth trailing ring, like modern Framer pages.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
    {
      id: "txt-cursor-glow",
      label: "Cursor · Glow",
      description: "Uses a bright brand dot with a soft halo for premium hover states.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
    {
      id: "txt-cursor-spotlight",
      label: "Cursor · Spotlight",
      description: "Creates a soft translucent lens over selected text.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
    {
      id: "txt-cursor-crosshair",
      label: "Cursor · Crosshair",
      description: "Adds a precise editorial crosshair cursor.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
    {
      id: "txt-cursor-outline",
      label: "Cursor · Outline",
      description: "Shows a clean double-outline pointer without filling the text area.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
  ...
  const imageEffectExtras = [
    {
      ...
      intensityLabel: "Depth",
      preview: "image-shadow",
      css: (s, i, _settings, theme) => {
        const radius = Math.round(i * 42);
        return `${mediaSurfaces(s)}{border-radius:${radius}px!important;box-shadow:0 ${Math.round(14+i*34)}px ${Math.round(28+i*70)}px ${hexToRgba(theme.primary,.12+i*.22)}!important;overflow:visible!important;} ${mediaTargets(s)}{border-radius:${radius}px!important;}`;
      }
    },
    {
      ...
      css: (s, i, settings, theme) => {
        const offset = Math.round(10 + param(settings, "offset", 44) * .42);
        const radius = Math.round(i * 42);
        return `${mediaSurfaces(s)}{border-radius:${radius}px!important;box-shadow:${offset}px ${offset}px 0 ${theme.primary}!important;overflow:visible!important;} ${mediaTargets(s)}{border-radius:${radius}px!important;}`;
      }
    },
    {
      id: "img-style-border",
    ...
    {
      id: "img-shape-square-orbit",
      label: "Animated image shapes · Square orbit",
      description: "Adds a rotating square frame around the image without shrinking it.",
      intensityLabel: "Speed",
      preview: "shape-divider",
      css: (s, i, _settings, theme) => {
        const gap = Math.round(8 + i * 18);
        const width = Math.round(2 + i * 4);
        const speed = Math.max(4, 15 - i * 9).toFixed(1);
        return `${mediaWrappers(s)}{position:relative!important;isolation:isolate!important;overflow:visible!important;} ${mediaWrappers(s)}:before{content:"";position:absolute;inset:-${gap}px;border:${width}px solid ${theme.accent};border-radius:${Math.round(4 + i * 14)}px;transform-origin:center!important;animation:ssxSquareOrbit360 ${speed}s linear infinite!important;pointer-events:none;z-index:2;} ${mediaWrappers(s)}:after{content:"";position:absolute;inset:${Math.round(gap * .55)}px;border:1px solid ${hexToRgba(theme.primary,.30+i*.25)};border-radius:${Math.round(2 + i * 12)}px;transform-origin:center!important;animation:ssxSquareOrbit360Inner ${Math.max(5, Number(speed) + 2).toFixed(1)}s linear infinite!important;pointer-events:none;z-index:2;} @keyframes ssxSquareOrbit360{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)}}@keyframes ssxSquareOrbit360Inner{0%{transform:rotate(360deg)}100%{transform:rotate(0deg)}}`;
      }
    },
    {
      id: "img-shape-circle-orbit",
      label: "Animated image shapes · Circle orbit",
      description: "Adds a 360 looping circular frame around the image.",
      intensityLabel: "Speed",
      preview: "cursor-dot",
      css: (s, i, _settings, theme) => {
        const gap = Math.round(8 + i * 18);
        const width = Math.round(2 + i * 4);
        const speed = Math.max(4, 15 - i * 9).toFixed(1);
        return `${mediaSurfaces(s)}{border-radius:999px!important;aspect-ratio:1/1!important;object-fit:cover!important;overflow:hidden!important;} ${mediaTargets(s)}{border-radius:999px!important;aspect-ratio:1/1!important;object-fit:cover!important;} ${mediaWrappers(s)}{position:relative!important;isolation:isolate!important;overflow:visible!important;border-radius:999px!important;} ${mediaWrappers(s)}:before{content:"";position:absolute;inset:-${gap}px;border:${width}px solid ${theme.accent};border-radius:999px;transform-origin:center!important;animation:ssxCircleOrbit360 ${speed}s linear infinite!important;pointer-events:none;z-index:2;} ${mediaWrappers(s)}:after{content:"";position:absolute;inset:${Math.round(gap * .52)}px;border:1px dashed ${hexToRgba(theme.primary,.34+i*.28)};border-radius:999px;transform-origin:center!important;animation:ssxCircleOrbit360Inner ${Math.max(5, Number(speed) + 2).toFixed(1)}s linear infinite!important;pointer-events:none;z-index:2;} @keyframes ssxCircleOrbit360{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)}}@keyframes ssxCircleOrbit360Inner{0%{transform:rotate(360deg)}100%{transform:rotate(0deg)}}`;
      }
    },
    {
      id: "img-shape-blob",
      label: "Animated image shapes · Blob",
      description: "Morphs the image into a liquid blob with soft brand glow.",
      intensityLabel: "Speed",
      preview: "shape-glass",
      css: (s, i, _settings, theme) => {
        const speed = Math.max(4, 12 - i * 7).toFixed(1);
        const border = Math.round(2 + i * 5);
        return `${mediaSurfaces(s)}{position:relative!important;overflow:hidden!important;border:${border}px dotted ${theme.primary}!important;border-radius:52% 48% 66% 34% / 38% 64% 36% 62%!important;filter:drop-shadow(0 0 ${Math.round(8+i*20)}px ${hexToRgba(theme.accent,.48)})!important;animation:ssxImageBlobMorph ${speed}s ease-in-out infinite!important;} ${mediaTargets(s)}{border-radius:inherit!important;object-fit:cover!important;} ${mediaSurfaces(s)}:hover{animation-duration:3s!important;filter:drop-shadow(0 0 ${Math.round(18+i*22)}px ${hexToRgba(theme.accent,.72)})!important;}@keyframes ssxImageBlobMorph{0%,100%{border-radius:52% 48% 66% 34% / 38% 64% 36% 62%}25%{border-radius:64% 36% 52% 48% / 62% 38% 64% 36%}50%{border-radius:58% 42% 42% 58% / 58% 69% 31% 42%}75%{border-radius:42% 58% 58% 42% / 42% 31% 69% 58%}}`;
      }
    },
    {
      ...
      className: "ssx-parallax",
      params: [{ key: "distance", label: "Move distance", value: 72 }],
      css: (s, i, settings) => `${imageMotionTargets(s)}{transition:transform ${Math.max(.28,1.15-i*.72).toFixed(2)}s ease,opacity ${Math.max(.28,1.15-i*.72).toFixed(2)}s ease;will-change:transform,opacity;} ${hiddenImageMotionTargets(s)}{opacity:.2;transform:translateY(${Math.round(20+param(settings,"distance",72)*1.45)}px);}`
    },
    {
      id: "img-scroll-left",
      ...
      className: "ssx-reveal",
      params: [{ key: "distance", label: "Move distance", value: 72 }],
      css: (s, i, settings) => `${imageMotionTargets(s)}{transition:transform ${Math.max(.28,1.15-i*.72).toFixed(2)}s cubic-bezier(.2,.8,.2,1),opacity ${Math.max(.28,1.15-i*.72).toFixed(2)}s ease;will-change:transform,opacity;} ${hiddenImageMotionTargets(s)}{opacity:0;transform:translateX(${Math.round(34+param(settings,"distance",72)*1.9)}px);}`
    },
    {
      id: "img-scroll-right",
    ...
    },
    {
      ...
      className: "ssx-reveal",
      params: [{ key: "distance", label: "Move distance", value: 68 }],
      css: (s, i, settings) => `${imageMotionTargets(s)}{transition:transform ${Math.max(.28,1.15-i*.72).toFixed(2)}s cubic-bezier(.2,.8,.2,1),opacity ${Math.max(.28,1.15-i*.72).toFixed(2)}s ease;will-change:transform,opacity;} ${hiddenImageMotionTargets(s)}{opacity:0;transform:translateY(${Math.round(28+param(settings,"distance",68)*1.55)}px);}`
    },
    {
      id: "img-scroll-down",
    ...
    },
    {
      ...
      className: "ssx-reveal",
      params: [{ key: "distance", label: "Scale amount", value: 55 }],
      css: (s, i, settings) => `${imageMotionTargets(s)}{transition:transform ${Math.max(.28,1.15-i*.72).toFixed(2)}s cubic-bezier(.2,.8,.2,1),opacity ${Math.max(.28,1.15-i*.72).toFixed(2)}s ease;transform-origin:center;will-change:transform,opacity;} ${hiddenImageMotionTargets(s)}{opacity:0;transform:scale(${(1-(.08+param(settings,"distance",55)*.0025)).toFixed(2)});}`
    },
    {
      ...
      className: "ssx-reveal",
      params: [{ key: "distance", label: "Scale amount", value: 55 }],
      css: (s, i, settings) => `${imageMotionTargets(s)}{transition:transform ${Math.max(.28,1.15-i*.72).toFixed(2)}s cubic-bezier(.2,.8,.2,1),opacity ${Math.max(.28,1.15-i*.72).toFixed(2)}s ease;transform-origin:center;will-change:transform,opacity;} ${hiddenImageMotionTargets(s)}{opacity:0;transform:scale(${(1.06+param(settings,"distance",55)*.003).toFixed(2)});}`
    },
    {
      id: "img-scroll-flip",
      ...
      preview: "type-3d",
      className: "ssx-reveal",
      css: (s, i) => `${imageMotionTargets(s)}{transition:transform ${Math.max(.35,1.2-i*.7).toFixed(2)}s ease,opacity .45s ease;transform-style:preserve-3d;will-change:transform,opacity;} ${hiddenImageMotionTargets(s)}{opacity:0;transform:perspective(900px) rotateY(${Math.round(35+i*85)}deg);}`
    },
    {
      id: "img-scroll-focus",
      ...
      preview: "motion-reveal",
      className: "ssx-reveal",
      css: (s, i) => `${imageMotionTargets(s)}{transition:filter ${Math.max(.35,1.1-i*.6).toFixed(2)}s ease,opacity .45s ease;will-change:filter,opacity;} ${hiddenImageMotionTargets(s)}{opacity:.25;filter:blur(${Math.round(4+i*10)}px);}`
    },
    {
      id: "img-scroll-spin",
      ...
      preview: "motion-spin",
      className: "ssx-reveal",
      css: (s, i) => `${imageMotionTargets(s)}{transition:transform ${Math.max(.35,1.2-i*.7).toFixed(2)}s ease,opacity .45s ease;will-change:transform,opacity;} ${hiddenImageMotionTargets(s)}{opacity:0;transform:rotate(${Math.round(25+i*140)}deg) scale(.92);}`
    },
    {
      id: "img-scroll-3d",
      ...
      preview: "type-3d",
      className: "ssx-reveal",
      css: (s, i) => `${imageMotionTargets(s)}{transition:transform ${Math.max(.35,1.2-i*.7).toFixed(2)}s ease,opacity .45s ease;transform-style:preserve-3d;will-change:transform,opacity;} ${hiddenImageMotionTargets(s)}{opacity:0;transform:perspective(900px) rotateX(${Math.round(15+i*45)}deg) translateY(40px);}`
    },
    {
      id: "img-loop-pulse",
    ...
    },
    {
      ...
      intensityLabel: "Scale",
      preview: "hover-scale",
      css: (s, i) => `${mediaWrappers(s)}{overflow:hidden!important;} ${mediaTargets(s)}{transition:transform .24s ease!important;} ${mediaWrappers(s)}:hover img,${s}:hover img{}`
    },
    {
      id: "img-hover-up",
      ...
      intensityLabel: "Lift",
      preview: "button-lift",
      css: (s, i) => `${mediaTargets(s)}{transition:transform .24s ease!important;} ${mediaWrappers(s)}:hover img,${s}:hover img{}`
    },
    {
      id: "img-hover-pop",
      ...
      intensityLabel: "Pop",
      preview: "engagement-jelly",
      css: (s, i) => `${mediaTargets(s)}{transition:transform .22s cubic-bezier(.2,1.6,.3,1)!important;} ${mediaWrappers(s)}:hover img,${s}:hover img{}`
    },
    {
      id: "img-hover-tilt",
    ...
    {
      id: "img-cursor-dot",
      label: "Cursor on hover · Soft dot",
      description: "Uses a clean brand dot over selected images.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
    {
      id: "img-cursor-ring",
      label: "Cursor on hover · Ring",
      description: "Adds a minimal ring cursor for gallery hover states.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
    {
      id: "img-cursor-trail",
      label: "Cursor on hover · Dot follower",
      description: "Adds a cursor dot with a smooth trailing ring.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
    {
      id: "img-cursor-glow",
      label: "Cursor on hover · Glow",
      description: "Creates a luminous hover cursor for images.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
    {
      id: "img-cursor-spotlight",
      label: "Cursor on hover · Spotlight",
      description: "Shows a soft lens over selected images.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
    {
      id: "img-cursor-crosshair",
      label: "Cursor on hover · Crosshair",
      description: "Adds a sharp crosshair cursor for image details.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      css: (s) => `${s},${s} *{cursor:none!important;}`
    },
    ...
    },
    {
      ...
      intensityLabel: "Speed",
      preview: "hover-overlay",
      css: (s, i, _settings, theme) => `${s}{position:relative!important;overflow:hidden!important;} ${mediaTargets(s)}{transition:filter ${Math.max(.18,.65-i*.35).toFixed(2)}s ease,transform ${Math.max(.18,.65-i*.35).toFixed(2)}s ease!important;filter:brightness(.82)!important;} ${s}:hover img{filter:brightness(1)!important;} ${s}:after{content:"";position:absolute;inset:0;background:${hexToRgba(theme.primary,.16+i*.28)};opacity:1;transition:opacity ${Math.max(.18,.65-i*.35).toFixed(2)}s ease;pointer-events:none;} ${s}:hover:after{opacity:0;}`
    }
  ];

  const siteCursorEffects = [
    {
      id: "site-cursor-native-dot",
      label: "Website cursor · Native + dot",
      description: "Keep the normal mouse cursor and add a smooth brand dot that follows it.",
      intensityLabel: "Dot size",
      preview: "cursor-dot",
      globalOnly: true,
      css: (s) => `${s},${s} *{cursor:auto!important;} ${s} a,${s} button,${s} [role="button"],${s} input[type="button"],${s} input[type="submit"],${s} .sqs-button-element,${s} .sqs-block-button-element{cursor:pointer!important;}`
    },
    {
      id: "site-cursor-dot",
      label: "Website cursor · Soft dot",
      description: "Minimal brand dot that follows the cursor across the whole site.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      globalOnly: true,
      css: (s) => `${s},${s} *{cursor:none!important;}`
    ...
    {
      id: "site-cursor-ring",
      label: "Website cursor · Hollow circle",
      description: "A clean circle cursor often used on editorial Framer-style pages.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      globalOnly: true,
      css: (s) => `${s},${s} *{cursor:none!important;}`
    ...
    {
      id: "site-cursor-trail",
      label: "Website cursor · Dot follower",
      description: "Small dot with a smooth follower ring for modern Webflow interactions.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      globalOnly: true,
      css: (s) => `${s},${s} *{cursor:none!important;}`
    ...
    {
      id: "site-cursor-glow",
      label: "Website cursor · Glow dot",
      description: "Bright cursor halo for premium landing pages and portfolios.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      globalOnly: true,
      css: (s) => `${s},${s} *{cursor:none!important;}`
    ...
    {
      id: "site-cursor-spotlight",
      label: "Website cursor · Spotlight",
      description: "Soft lens cursor inspired by spotlight reveal interactions.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      globalOnly: true,
      css: (s) => `${s},${s} *{cursor:none!important;}`
    ...
    {
      id: "site-cursor-crosshair",
      label: "Website cursor · Crosshair",
      description: "Precise graphic cursor for studios, galleries and experimental layouts.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      globalOnly: true,
      css: (s) => `${s},${s} *{cursor:none!important;}`
    ...
    {
      id: "site-cursor-outline",
      label: "Website cursor · Double outline",
      description: "Elegant outlined cursor that stays subtle on image-heavy pages.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      globalOnly: true,
      css: (s) => `${s},${s} *{cursor:none!important;}`
    ...
    {
      id: "site-cursor-emoji",
      label: "Website cursor · Emoji / custom",
      description: "Use an emoji, symbol or short custom word as the site cursor.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      globalOnly: true,
      css: (s) => `${s},${s} *{cursor:none!important;}`
    ...
    {
      id: "site-cursor-large",
      label: "Website cursor · Large dot",
      description: "Bold large dot cursor for high-impact creative websites.",
      intensityLabel: "Size",
      preview: "cursor-dot",
      globalOnly: true,
      css: (s) => `${s},${s} *{cursor:none!important;}`
  ...
  const buttonStyleEffects = [
    {
      id: "btn-style-animated-arrow-pill",
      label: "Animated arrow pill",
      description: "Spark-style pill: the label slides aside while a round arrow swaps from right to left on hover.",
      intensityLabel: "Motion",
      preview: "button-arrow",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-animated-arrow-pill";
        const fill = settings.color || theme.soft;
        const text = settings.textColor || theme.ink;
        return `${buttonClassTargets(s, className)}{color:${text}!important;background:none!important;border:none!important;padding:0!important;}`;
      }
    },
    {
      id: "btn-style-circle-expand",
      label: "Circle expand",
      description: "Left circle expands into the full pill while the arrow slides forward.",
      intensityLabel: "Motion",
      preview: "button-arrow",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-circle-expand";
        const text = settings.textColor || theme.ink;
        return `${buttonClassTargets(s, className)}{color:${text}!important;background:none!important;border:none!important;padding:0!important;}`;
      }
    },
    {
      id: "btn-style-line-fill-arrow",
      label: "Line fill arrow",
      description: "Thin arrow line grows into a full button background on hover.",
      intensityLabel: "Fill",
      preview: "button-line",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-line-fill-arrow";
        const text = settings.textColor || theme.ink;
        return `${buttonClassTargets(s, className)}{color:${text}!important;background:none!important;border:none!important;padding:0!important;}`;
      }
    },
    {
      id: "btn-style-rotating-circle",
      label: "Rotating circle",
      description: "Round CTA with rotating outer text and a centered arrow button.",
      intensityLabel: "Spin",
      preview: "button-arrow",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-rotating-circle";
        const text = settings.textColor || theme.soft;
        return `${buttonClassTargets(s, className)}{color:${text}!important;background:none!important;border:none!important;padding:0!important;}`;
      }
    },
    {
      id: "btn-style-letter-flip-v2",
      label: "Letter flip",
      description: "Letters flip vertically on hover while the button keeps its exact position.",
      intensityLabel: "Speed",
      preview: "button-line",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-letter-flip-v2";
        const text = settings.textColor || theme.soft;
        return `${buttonClassTargets(s, className)}{color:${text}!important;background:none!important;border:none!important;padding:0!important;}`;
      }
    },
    {
      id: "btn-style-shine-fill-v2",
      label: "Shine fill",
      description: "Stable filled pill with a diagonal shine sweep on hover.",
      intensityLabel: "Shine",
      preview: "button-glow",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-shine-fill-v2";
        const text = settings.textColor || theme.soft;
        return `${buttonClassTargets(s, className)}{color:${text}!important;background:none!important;border:none!important;padding:0!important;}`;
      }
    },
    {
      id: "btn-style-arrow-nudge-v2",
      label: "Arrow nudge",
      description: "Clean pill CTA where the icon nudges forward and rotates on hover.",
      intensityLabel: "Push",
      preview: "button-arrow",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-arrow-nudge-v2";
        const text = settings.textColor || theme.ink;
        return `${buttonClassTargets(s, className)}{color:${text}!important;background:none!important;border:none!important;padding:0!important;}`;
      }
    },
    {
      id: "btn-style-bouncy-v2",
      label: "Bouncy CTA",
      description: "Springy pill with soft scale feedback on hover and press.",
      intensityLabel: "Bounce",
      preview: "button-glow",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-bouncy-v2";
        const text = settings.textColor || theme.ink;
        return `${buttonClassTargets(s, className)}{color:${text}!important;background:none!important;border:none!important;padding:0!important;}`;
      }
    },
    {
      id: "btn-style-bouncy-cta",
      label: "Bouncy CTA",
      description: "Springy premium CTA with a soft bounce on hover and press feedback.",
      intensityLabel: "Bounce",
      preview: "button-glow",
      css: (s, i, settings, theme) => {
        const fill = settings.color || theme.accent;
        const text = settings.textColor || theme.ink;
        const hover = settings.hoverColor || theme.gradientB;
        const outline = settings.outlineColor || fill;
        const radius = Math.round(20 + param(settings, "radius", 72) * .46);
        const speed = Math.max(.34, .72 - i * .2).toFixed(2);
        const height = Math.round(36 + param(settings, "height", 44) * .38);
        const pad = Math.round(14 + param(settings, "width", 50) * .34);
        const circle = Math.max(26, Math.round(height * .72));
        const right = Math.max(5, Math.round((height - circle) / 2 + 5));
        const arrow = Math.max(15, Math.round(circle * .58));
        const shift = Math.round(2 + i * 5);
        const bounce = Math.max(1.01, 1 + i * .035).toFixed(3);
        const verticalPad = Math.max(8, Math.round(height * .26));
        return `${buttonBaseCss(s, text, text)} ${buttonTargets(s)}{--ssx-btn-bouncy-shift:${shift}px;min-height:${height}px!important;padding:${verticalPad}px ${pad + circle + 14}px ${verticalPad}px ${pad}px!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(outline,.28)}!important;border-radius:${radius}px!important;overflow:hidden!important;isolation:isolate!important;box-shadow:0 10px 22px ${hexToRgba(fill,.14)},inset 0 1px 0 ${hexToRgba("#ffffff",.34)}!important;transition:transform ${speed}s cubic-bezier(.34,1.56,.64,1),background .22s ease,box-shadow .22s ease,color .22s ease!important;line-height:1.1!important;} ${buttonTextTargets(s)}{position:relative!important;z-index:2!important;color:inherit!important;white-space:nowrap!important;} ${buttonTargets(s)}:after{content:"↗"!important;position:absolute!important;right:${right}px!important;top:50%!important;transform:translateY(-50%) rotate(0deg)!important;width:${circle}px!important;height:${circle}px!important;min-width:${circle}px!important;min-height:${circle}px!important;border-radius:999px!important;display:grid!important;place-items:center!important;background:${hover}!important;color:${outline}!important;font-size:${arrow}px!important;font-weight:900!important;line-height:1!important;box-shadow:inset 0 0 0 1px ${hexToRgba(outline,.16)}!important;transition:transform ${speed}s cubic-bezier(.34,1.56,.64,1),background .22s ease,color .22s ease!important;z-index:1!important;pointer-events:none!important;} ${buttonTargets(s)}:hover{transform:translateY(-1px) scale(${bounce})!important;background:${hover}!important;color:${text}!important;box-shadow:0 ${Math.round(13 + i * 9)}px ${Math.round(26 + i * 18)}px ${hexToRgba(hover,.18)},inset 0 1px 0 ${hexToRgba("#ffffff",.42)}!important;} ${buttonTargets(s)}:hover:after{transform:translate(var(--ssx-btn-bouncy-shift),-50%) rotate(45deg) scale(1.04)!important;background:${text}!important;color:${fill}!important;} ${buttonTargets(s)}:active{transform:translateY(1px) scale(.985)!important;}`;
      }
    },
    {
      id: "btn-style-letter-flip",
      label: "Letter flip",
      description: "Button text flips vertically on hover without moving the button.",
      intensityLabel: "Speed",
      preview: "button-line",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-letter-flip";
        const target = buttonClassTargets(s, className);
        const fill = settings.color || theme.primary;
        const text = settings.textColor || theme.soft;
        const radius = Math.round(14 + param(settings, "radius", 65) * .34);
        const speed = Math.max(.22, .5 - i * .18).toFixed(2);
        let stagger = "";
        for (let n = 1; n <= 24; n += 1) stagger += `${target} .ssx-flip-top .ssx-btn-char:nth-child(${n}),${target} .ssx-flip-bottom .ssx-btn-char:nth-child(${n}){transition-delay:${(n * .018).toFixed(3)}s!important;}`;
        return `${buttonAdapterBaseCss(s, className, text, text)} ${buttonAdapterStableSizeCss(s, className, settings)} ${target}{overflow:hidden!important;border:1px solid ${hexToRgba(settings.outlineColor || fill,.35)}!important;background:${fill}!important;color:${text}!important;border-radius:${radius}px!important;font-weight:850!important;letter-spacing:0!important;cursor:pointer!important;box-shadow:none!important;transform:none!important;} ${target} .ssx-btn-text{display:flex!important;gap:0!important;line-height:1!important;white-space:nowrap!important;} ${target} .ssx-flip-bottom{position:absolute!important;inset:0!important;display:flex!important;align-items:center!important;justify-content:center!important;} ${target} .ssx-flip-bottom .ssx-btn-char{transform:translateY(-1.2em);} ${target}:hover .ssx-flip-top .ssx-btn-char{transform:translateY(1.2em);} ${target}:hover .ssx-flip-bottom .ssx-btn-char{transform:translateY(0);} ${target} .ssx-btn-char{display:inline-block!important;transition:transform ${speed}s cubic-bezier(.22,1,.36,1)!important;will-change:transform!important;} ${stagger}`;
      }
    },
    {
      id: "btn-style-swap-arrow",
      label: "Swap arrow",
      description: "Hover text slides in and the arrow turns, while the button keeps its size.",
      intensityLabel: "Speed",
      preview: "button-arrow",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-swap-arrow";
        const target = buttonClassTargets(s, className);
        const fill = settings.color || theme.soft;
        const text = settings.textColor || theme.ink;
        const hover = settings.hoverColor || theme.gradientB;
        const outline = settings.outlineColor || theme.primary;
        const radius = Math.round(18 + param(settings, "radius", 65) * .42);
        const speed = Math.max(.2, .48 - i * .16).toFixed(2);
        return `${buttonAdapterBaseCss(s, className, text, text)} ${buttonAdapterStableSizeCss(s, className, settings)} ${target}{overflow:hidden!important;gap:10px!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(outline,.22)}!important;border-radius:${radius}px!important;font-weight:850!important;box-shadow:0 8px 18px ${hexToRgba(theme.primary,.08)}!important;transition:background .2s ease,box-shadow .2s ease!important;transform:none!important;} ${target}:hover{background:${hover}!important;box-shadow:0 10px 22px ${hexToRgba(theme.primary,.12)}!important;} ${target} .ssx-btn-swap-wrap{position:relative!important;display:inline-block!important;overflow:hidden!important;line-height:1.1!important;min-width:82px!important;height:1.15em!important;z-index:2!important;} ${target} .ssx-btn-swap-top,${target} .ssx-btn-swap-bottom{display:block!important;transition:transform ${speed}s cubic-bezier(.22,1,.36,1)!important;white-space:nowrap!important;} ${target} .ssx-btn-swap-bottom{position:absolute!important;inset:0!important;transform:translateY(115%);} ${target}:hover .ssx-btn-swap-top{transform:translateY(-115%);} ${target}:hover .ssx-btn-swap-bottom{transform:translateY(0);} ${target} .ssx-btn-circle-icon{position:relative!important;z-index:2!important;width:24px!important;height:24px!important;display:grid!important;place-items:center!important;color:${text}!important;transition:transform .22s ease!important;flex:0 0 auto!important;} ${target} .ssx-btn-circle-icon svg{width:24px!important;height:24px!important;display:block!important;} ${target}:hover .ssx-btn-circle-icon{transform:rotate(45deg);}`;
      }
    },
    {
      id: "btn-style-arrow-push",
      label: "Arrow push",
      description: "Clean pill CTA with arrow nudge and gentle press feedback.",
      intensityLabel: "Push",
      preview: "button-arrow",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-arrow-push";
        const target = buttonClassTargets(s, className);
        const fill = settings.color || theme.accent;
        const text = settings.textColor || theme.ink;
        const hover = settings.hoverColor || theme.gradientB;
        const radius = Math.round(24 + param(settings, "radius", 70) * .5);
        const move = Math.round(3 + i * 5);
        return `${buttonAdapterBaseCss(s, className, text, text)} ${buttonAdapterStableSizeCss(s, className, settings)} ${target}{gap:10px!important;overflow:hidden!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(settings.outlineColor || fill,.32)}!important;border-radius:${radius}px!important;font-weight:850!important;transition:background .2s ease,transform .16s ease,box-shadow .2s ease!important;box-shadow:0 8px 18px ${hexToRgba(fill,.10)}!important;} ${target}:hover{background:${hover}!important;} ${target}:active{transform:scale(.97)!important;} ${target} .ssx-btn-label{position:relative!important;z-index:2!important;white-space:nowrap!important;} ${target} svg{position:relative!important;z-index:2!important;width:26px!important;height:26px!important;margin-left:1px!important;fill:currentColor!important;transition:transform .25s ease!important;flex:0 0 auto!important;} ${target}:hover svg{transform:translateX(${move}px);}`;
      }
    },
    {
      id: "btn-style-shine-fill",
      label: "Shine fill",
      description: "Stable filled button with a diagonal shine passing through on hover.",
      intensityLabel: "Shine",
      preview: "button-glow",
      css: (s, i, settings, theme) => {
        const fill = settings.color || theme.primary;
        const text = settings.textColor || theme.soft;
        const shine = settings.hoverColor || theme.accent;
        const radius = Math.round(12 + param(settings, "radius", 58) * .46);
        const speed = Math.max(.34, .9 - i * .42).toFixed(2);
        return `${buttonBaseCss(s, text, text)} ${buttonStableSizeCss(s, settings)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(settings.outlineColor || fill,.28)}!important;border-radius:${radius}px!important;box-shadow:0 8px 18px ${hexToRgba(fill,.12)}!important;transition:background .2s ease,box-shadow .2s ease!important;transform:none!important;} ${buttonTargets(s)}:before{content:"";position:absolute;top:-70%;left:-55%;width:36%;height:240%;background:linear-gradient(90deg,transparent,${hexToRgba(shine,.72)},transparent);transform:rotate(24deg);transition:left ${speed}s ease;z-index:1;pointer-events:none;} ${buttonTargets(s)}:hover:before{left:130%;}`;
      }
    },
    {
      id: "btn-style-soft-glass",
      label: "Soft glass",
      description: "Subtle glass pill with controlled blur and no layout jump.",
      intensityLabel: "Glass",
      preview: "button-glass",
      css: (s, i, settings, theme) => {
        const fill = settings.color || theme.soft;
        const text = settings.textColor || theme.ink;
        const outline = settings.outlineColor || theme.primary;
        const radius = Math.round(16 + param(settings, "radius", 70) * .45);
        const blur = Math.round(4 + i * 9);
        return `${buttonBaseCss(s, text, text)} ${buttonStableSizeCss(s, settings)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;background:${hexToRgba(fill,.56)}!important;color:${text}!important;border:1px solid ${hexToRgba(outline,.22)}!important;border-radius:${radius}px!important;backdrop-filter:blur(${blur}px) saturate(1.08)!important;-webkit-backdrop-filter:blur(${blur}px) saturate(1.08)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.45),0 10px 22px ${hexToRgba(outline,.08)}!important;transition:box-shadow .2s ease,background .2s ease!important;transform:none!important;} ${buttonTargets(s)}:hover{box-shadow:inset 0 1px 0 rgba(255,255,255,.55),0 12px 26px ${hexToRgba(settings.hoverColor || theme.accent,.14)}!important;}`;
      }
    },
    {
      id: "btn-style-outline-slide",
      label: "Outline slide",
      description: "Outlined button fills from the left on hover while text stays readable.",
      intensityLabel: "Fill",
      preview: "button-line",
      css: (s, i, settings, theme) => {
        const fill = settings.color || "transparent";
        const text = settings.textColor || theme.primary;
        const hover = settings.hoverColor || theme.primary;
        const hoverText = settings.hoverTextColor || theme.soft;
        const outline = settings.outlineColor || theme.primary;
        const radius = Math.round(10 + param(settings, "radius", 42) * .44);
        const speed = Math.max(.24, .66 - i * .26).toFixed(2);
        return `${buttonBaseCss(s, text, hoverText)} ${buttonStableSizeCss(s, settings)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;isolation:isolate!important;background:${fill}!important;color:${text}!important;border:1px solid ${outline}!important;border-radius:${radius}px!important;box-shadow:none!important;transition:color .2s ease!important;transform:none!important;} ${buttonTargets(s)}:before{content:"";position:absolute;inset:0;background:${hover};transform:translateX(-102%);transition:transform ${speed}s cubic-bezier(.22,1,.36,1);z-index:0;pointer-events:none;} ${buttonTargets(s)}:hover:before{transform:translateX(0);} ${buttonTargets(s)}:hover{color:${hoverText}!important;}`;
      }
    }
  ];

  const activeButtonStyleEffects = buttonStyleEffects.filter((effect) => !retiredButtonStyleIds.includes(effect.id));
  const allEffects = effectCatalog.flatMap((group) => group.items.map((effect) => ({ ...effect, group: group.group }))).concat(textEffectExtras.map((effect) => ({ ...effect, group: "Text" })), imageEffectExtras.map((effect) => ({ ...effect, group: "Image" })), siteCursorEffects.map((effect) => ({ ...effect, group: "Cursor" })), activeButtonStyleEffects.map((effect) => ({ ...effect, group: "Buttons" })));
  ...
  const featureLibrary = [
    ...
    { id: "button-suite", title: "Button styles", target: ["button", "section", "page", "text", "image"], icon: "▭", description: "Customizable button presets for one button or all page buttons.", options: buttonStyleIds },
    { id: "site-cursor-suite", title: "Website cursor", target: ["text", "section", "page", "button", "image"], icon: "⌖", description: "Trendy global cursors for the whole website.", options: siteCursorEffects.map((effect) => effect.id) }
  ];
  const textSections = [
    ...
    { id: "loop", icon: "⌁", title: "Loop animations", description: "Continuous animation for callouts and badges.", options: ["txt-word-loop", "txt-loop-pulse", "txt-loop-hover", "txt-loop-fade", "txt-loop-spin", "txt-loop-shake", "txt-loop-wiggle"] },
    ...
    { id: "cursor", icon: "☝", title: "Cursor on hover", description: "Custom cursor options for selected text.", options: textCursorVisualIds.concat(["txt-cursor-hide"]) }
  ];
  const imageSections = [
    { id: "image-styles", icon: "▧", title: "Image styles", description: "Shadows, borders and subtle 3D image depth.", options: ["img-style-shadow", "img-style-solid-shadow", "img-style-border", "img-style-3d"] },
    { id: "image-shapes", icon: "▱", title: "Animated image shapes", description: "Blob masks and orbiting frames that keep the image size.", options: ["img-shape-square-orbit", "img-shape-circle-orbit", "img-shape-blob"] },
    ...
    { id: "image-cursor", icon: "☝", title: "Cursor on hover", description: "Custom cursor helpers for selected images.", options: imageCursorVisualIds },
  ...
  function normalizeConfig(input) {
    ...
    next.mode = "Studio";
    next.version = defaultConfig.version;
    return next;
  }
  ...
  function normalizeEffectsMap(value) {
    ...
    Object.entries(value || {}).forEach(([selector, effects]) => {
      ...
      Object.entries(effects || {}).forEach(([id, settings]) => {
        if (!effectById.has(id) || retiredButtonStyleIds.includes(id) || !settings || typeof settings !== "object") return;
        cleanEffects[id] = {
          enabled: Boolean(settings.enabled),
          intensity: clampIntensity(settings.intensity === undefined ? 60 : settings.intensity),
          params: normalizeParams(settings.params),
          customText: typeof settings.customText === "string" ? settings.customText.slice(0, 140) : ""
        };
      });
  ...
  function featuresForCurrentTarget() {
    ...
    const preferredOrder = kind === "button"
      ? ["button-suite", "text-suite", "site-cursor-suite", "image-suite"]
      : kind === "image"
        ? ["image-suite", "text-suite", "button-suite", "site-cursor-suite"]
        : ["text-suite", "image-suite", "button-suite", "site-cursor-suite"];
    return all.sort((a, b) => {
  ...
  function setEffectSetting(selector, effectId, patch) {
    ...
    if (!effect) return;
    if (selector === "body" && /^txt-block-/.test(effectId) && patch.enabled) {
    ...
    const shouldReplayMotion = /^(txt|img)-scroll-/.test(effectId) && patch.enabled && !current.enabled;
    const next = {
      ...current,
      ...patch,
    ...
    if (shouldReplayMotion) replayMotionFor(selector, effect);
    saveConfig();
    applyEffects();
  }

  function replayMotionFor(selector, effect) {
    motionTargetsFor(selector, effect).forEach((element) => {
      element.classList.remove("ssx-in");
    });
  }
  ...
  function isDesignerMode() {
    const params = new URLSearchParams(location.search);
    let topParams = new URLSearchParams();
    let topPath = "";
    try {
      topParams = window.top && window.top !== window ? new URLSearchParams(window.top.location.search) : params;
      topPath = window.top && window.top.location ? window.top.location.pathname : "";
    } catch {}
    const rawEditorValue = params.get("ssx_editor") || params.get("ssx") || params.get("poema") || topParams.get("ssx_editor") || topParams.get("ssx") || topParams.get("poema") || "";
    const editorValue = String(rawEditorValue).toLowerCase();
    const keyValue = String(params.get("ssx_key") || topParams.get("ssx_key") || "").toLowerCase();
    const explicitlyOff = ["0", "false", "off", "hide"].includes(editorValue) || ["0", "false", "off", "hide"].includes(keyValue);
    if (explicitlyOff) {
      localStorage.setItem(DESIGNER_KEY, "0");
      return false;
    }
    const forced = Boolean(editorValue) || keyValue === EDITOR_TOKEN || keyValue === "1";
    if (forced) localStorage.setItem(PANEL_KEY, "1");
    const adminShell = window.top !== window && /\/config(\/|$)/.test(topPath || "");
    ...
    if (forced || sqsHints) localStorage.setItem(DESIGNER_KEY, "1");
    const saved = localStorage.getItem(DESIGNER_KEY) === "1" && (forced || sqsHints);
  ...
  function applyEffects() {
    ...
    Object.entries(config.effects).forEach(([selector, effects]) => {
      Object.entries(effects || {}).forEach(([id, settings]) => {
        const effect = effectById.get(id);
        if (!effect || !settings.enabled || retiredButtonStyleIds.includes(id)) return;
    ...
    setupWordBlurReveal();
    setupWordLoop();
    setupButtonStyleAdapters();
    setupMotionObservers();
    setupCursorFollower(theme);
    syncSelectedIndicator();
  }

  function buttonAdapterClassFor(effectId) {
    return {
      "btn-style-animated-arrow-pill": "ssx-btn-animated-arrow-pill",
      "btn-style-circle-expand": "ssx-btn-circle-expand",
      "btn-style-line-fill-arrow": "ssx-btn-line-fill-arrow",
      "btn-style-rotating-circle": "ssx-btn-rotating-circle",
      "btn-style-letter-flip-v2": "ssx-btn-letter-flip-v2",
      "btn-style-shine-fill-v2": "ssx-btn-shine-fill-v2",
      "btn-style-arrow-nudge-v2": "ssx-btn-arrow-nudge-v2",
      "btn-style-bouncy-v2": "ssx-btn-bouncy-v2"
    }[effectId] || "";
  }

  function setupButtonStyleAdapters() {
    const theme = config.theme || defaultConfig.theme;
    const active = new Map();
    Object.entries(config.effects || {}).forEach(([selector, effects]) => {
      buttonStyleIds.forEach((effectId) => {
        const className = buttonAdapterClassFor(effectId);
        if (!className || !effects?.[effectId]?.enabled) return;
        try {
          document.querySelectorAll(buttonTargets(selector)).forEach((button) => {
            if (!button.closest(`#${APP_ID}`)) active.set(button, { effectId, className, settings: effects[effectId], theme });
          });
        } catch {}
      });
    });

    document.querySelectorAll("[data-ssx-button-adapter]").forEach((button) => {
      const next = active.get(button);
      if (!next || button.dataset.ssxButtonAdapter !== next.effectId) restoreButtonAdapter(button);
    });

    active.forEach(({ effectId, className, settings, theme }, button) => {
      const signature = `${effectId}:${JSON.stringify({ settings, theme })}`;
      if (button.dataset.ssxButtonAdapter === effectId && button.dataset.ssxButtonSignature === signature) {
        syncButtonAdapterMetrics(button, settings, theme);
        return;
      }
      restoreButtonAdapter(button);
      applyButtonAdapter(button, effectId, className, settings, signature, theme);
    });
  }

  function syncButtonAdapterMetrics(button, settings = {}, theme = config.theme || defaultConfig.theme) {
    if (!button) return;
    if (button.dataset.ssxButtonAdapter === "btn-style-animated-arrow-pill") {
      button.style.removeProperty("--ssx-btn-width");
      applyArrowPillButtonStyle(button, settings, theme);
      return;
    }
    if (button.dataset.ssxButtonAdapter === "btn-style-circle-expand") {
      button.style.removeProperty("--ssx-btn-width");
      applyCircleExpandButtonStyle(button, settings, theme);
      return;
    }
    if (button.dataset.ssxButtonAdapter === "btn-style-line-fill-arrow") {
      button.style.removeProperty("--ssx-btn-width");
      applyLineFillArrowButtonStyle(button, settings, theme);
      return;
    }
    if (button.dataset.ssxButtonAdapter === "btn-style-rotating-circle") {
      button.style.removeProperty("--ssx-btn-width");
      applyRotatingCircleButtonStyle(button, settings, theme);
      return;
    }
    if (button.dataset.ssxButtonAdapter === "btn-style-letter-flip-v2") {
      button.style.removeProperty("--ssx-btn-width");
      applyLetterFlipV2ButtonStyle(button, settings, theme);
      return;
    }
    if (button.dataset.ssxButtonAdapter === "btn-style-shine-fill-v2") {
      button.style.removeProperty("--ssx-btn-width");
      applyShineFillV2ButtonStyle(button, settings, theme);
      return;
    }
    if (button.dataset.ssxButtonAdapter === "btn-style-arrow-nudge-v2") {
      button.style.removeProperty("--ssx-btn-width");
      applyArrowNudgeV2ButtonStyle(button, settings, theme);
      return;
    }
    if (button.dataset.ssxButtonAdapter === "btn-style-bouncy-v2") {
      button.style.removeProperty("--ssx-btn-width");
      applyBouncyV2ButtonStyle(button, settings, theme);
      return;
    }
    const rect = button.getBoundingClientRect();
    const parentRect = button.parentElement ? button.parentElement.getBoundingClientRect() : null;
    const measuredWidth = Math.max(96, Math.round(rect.width || button.offsetWidth || 160));
    const parentWidth = parentRect ? Math.max(0, Math.round(parentRect.width || 0)) : 0;
    const isFullWidth = parentWidth > 0 && measuredWidth >= parentWidth * 0.72;
    button.style.setProperty("--ssx-btn-width", isFullWidth ? "100%" : `${measuredWidth}px`);
  }

  function applyArrowPillButtonStyle(button, settings = {}, theme = config.theme || defaultConfig.theme) {
    const buttonId = button.dataset.ssxButtonId;
    if (!buttonId) return;
    const rect = button.getBoundingClientRect();
    const computed = getComputedStyle(button);
    const measuredHeight = Math.max(34, Math.round(rect.height || button.offsetHeight || parseFloat(computed.lineHeight) || 48));
    const fontSize = Math.max(12, Math.round(parseFloat(computed.fontSize) || 16));
    const height = Math.round(Math.max(measuredHeight, 38 + param(settings, "height", 44) * .38));
    const iconSize = height;
    const label = button.querySelector(".ssx-spbtn-main")?.textContent || button.getAttribute("aria-label") || button.textContent || "Button";
    const labelWidth = Math.max(86, Math.round(label.length * fontSize * .68));
    const measuredWidth = Math.max(0, Math.round(rect.width || button.offsetWidth || 0));
    const desiredWidth = labelWidth + 32 + Math.round(param(settings, "width", 50) * .42);
    const textWidth = Math.min(Math.max(desiredWidth, measuredWidth > 0 && measuredWidth < 420 ? measuredWidth - iconSize : 0, 98), 260);
    const totalWidth = textWidth + iconSize;
    const fill = settings.color || theme.soft;
    const text = settings.textColor || theme.ink;
    const hover = settings.hoverColor || theme.accent;
    const radius = Math.round(24 + param(settings, "radius", 70) * .55);
    const speed = Math.round(360 + (1 - intensity01(settings.intensity)) * 340);
    const selector = `[data-ssx-button-id="${buttonId}"]`;
    const styleId = `${APP_ID}-button-${buttonId}`;
    let style = document.getElementById(styleId);
    if (!style) {
      style = document.createElement("style");
      style.id = styleId;
      style.setAttribute("data-ssx-button-style", buttonId);
      document.head.appendChild(style);
    }
    style.textContent = `
      ${selector},.sqs-block-newsletter ${selector}{
      ...
      ${selector}:hover{opacity:1!important;transform:none!important;}
      ${selector} .ssx-spbtn-box{
        position:relative!important;
      ...
      ${selector} .ssx-spbtn-main{
        display:flex!important;
        align-items:center!important;
        justify-content:center!important;
        width:${textWidth}px!important;
        min-width:${textWidth}px!important;
        height:${height}px!important;
        ...
        transition:transform ${speed}ms cubic-bezier(.33,1,.68,1),background 180ms ease,color 180ms ease!important;
        z-index:2!important;
      }
      ${selector} .ssx-spbtn-left,${selector} .ssx-spbtn-right{
        ...
        color:${text}!important;
        transition:transform ${speed}ms cubic-bezier(.33,1,.68,1),border-radius ${speed}ms cubic-bezier(.33,1,.68,1),background 180ms ease,color 180ms ease!important;
      }
      ${selector} .ssx-spbtn-left{
        position:absolute!important;
        left:0!important;
        top:0!important;
        transform:scale(1,.3) translateX(2px)!important;
        border-radius:34%!important;
        z-index:1!important;
      }
      ${selector} .ssx-spbtn-right{transform:scale(1)!important;z-index:1!important;}
      ${selector} .ssx-spbtn-arrow{
        display:grid!important;
        place-items:center!important;
        width:100%!important;
        height:100%!important;
        font-size:${Math.max(18, Math.round(fontSize * 1.15))}px!important;
        font-weight:900!important;
        line-height:1!important;
        transition:transform ${speed}ms cubic-bezier(.11,0,.5,0)!important;
      }
      ${selector} .ssx-spbtn-left .ssx-spbtn-arrow{transform:translate(-20%,10%)!important;}
      ${selector} .ssx-spbtn-right .ssx-spbtn-arrow{transform:translate(0,0)!important;}
      ${selector}:hover .ssx-spbtn-main{transform:translateX(${iconSize}px)!important;}
      ${selector}:hover .ssx-spbtn-left{transform:scale(1)!important;border-radius:999px!important;z-index:3!important;}
      ${selector}:hover .ssx-spbtn-right{transform:scale(1,.3) translateX(-2px)!important;border-radius:34%!important;z-index:0!important;}
      ${selector}:hover .ssx-spbtn-left .ssx-spbtn-arrow{transform:translate(0,0)!important;}
      ${selector}:hover .ssx-spbtn-right .ssx-spbtn-arrow{transform:translate(20%,-10%)!important;}
      ${selector}:active .ssx-spbtn-box{transform:scale(.985)!important;}
    `;
  }

  function buttonScopedStyle(button, cssText) {
    const buttonId = button.dataset.ssxButtonId;
    if (!buttonId) return;
    const styleId = `${APP_ID}-button-${buttonId}`;
    let style = document.getElementById(styleId);
  ...
  function buttonMeasure(button, settings = {}) {
    const rect = button.getBoundingClientRect();
    const computed = getComputedStyle(button);
    const fontSize = Math.max(12, Math.round(parseFloat(computed.fontSize) || 16));
    const measuredHeight = Math.max(34, Math.round(rect.height || button.offsetHeight || parseFloat(computed.lineHeight) || 48));
    ...
    const labelWidth = Math.max(86, Math.round(label.length * fontSize * .68));
    const measuredWidth = Math.max(0, Math.round(rect.width || button.offsetWidth || 0));
    const desiredWidth = labelWidth + 54 + Math.round(param(settings, "width", 50) * .56);
    const width = Math.min(Math.max(desiredWidth, measuredWidth > 0 && measuredWidth < 460 ? measuredWidth : 0, 148), 360);
    return { fontSize, height, width, label };
  }

  function applyCircleExpandButtonStyle(button, settings = {}, theme = config.theme || defaultConfig.theme) {
    const buttonId = button.dataset.ssxButtonId;
    if (!buttonId) return;
    const { fontSize, height, width } = buttonMeasure(button, settings);
    const circle = Math.max(34, Math.round(height));
    const radius = Math.round(circle / 2);
    const fill = settings.color || theme.ink;
    const text = settings.textColor || theme.ink;
    const hoverText = settings.hoverTextColor || theme.soft;
    const arrow = settings.outlineColor || theme.soft;
    const speed = Math.round(320 + (1 - intensity01(settings.intensity)) * 280);
    const selector = `[data-ssx-button-id="${buttonId}"]`;
    buttonScopedStyle(button, `
      ${selector},.sqs-block-newsletter ${selector}{
      ...
      ${selector}:hover{opacity:1!important;transform:none!important;}
      ${selector} .ssx-circle-expand-wrap{
        position:relative!important;
        display:block!important;
        width:${width}px!important;
        height:${height}px!important;
      }
      ${selector} .ssx-circle-expand-circle{
        position:absolute!important;
        left:0!important;
        top:0!important;
        width:${circle}px!important;
        height:${height}px!important;
        margin:0!important;
        background:${fill}!important;
        border-radius:${radius}px!important;
        transition:width ${speed}ms cubic-bezier(.65,0,.076,1),background 180ms ease!important;
        z-index:1!important;
      }
      ${selector} .ssx-circle-expand-icon{
        position:absolute!important;
        left:${Math.round(circle * .34)}px!important;
        top:50%!important;
        width:${Math.round(circle * .38)}px!important;
        height:2px!important;
        transform:translateY(-50%)!important;
        background:transparent!important;
        transition:transform ${speed}ms cubic-bezier(.65,0,.076,1),background ${speed}ms cubic-bezier(.65,0,.076,1)!important;
      }
      ${selector} .ssx-circle-expand-icon:before{
        content:""!important;
        position:absolute!important;
        right:0!important;
        top:-${Math.max(3, Math.round(circle * .09))}px!important;
        width:${Math.round(circle * .2)}px!important;
        height:${Math.round(circle * .2)}px!important;
        border-top:2px solid ${arrow}!important;
        border-right:2px solid ${arrow}!important;
        transform:rotate(45deg)!important;
      }
      ${selector} .ssx-circle-expand-text{
        position:absolute!important;
        inset:0!important;
        display:flex!important;
        align-items:center!important;
        justify-content:center!important;
        padding:0 ${Math.round(circle * .34)}px 0 ${Math.round(circle * .62)}px!important;
        color:${text}!important;
        font-size:${fontSize}px!important;
        font-weight:850!important;
        line-height:1!important;
        ...
        white-space:nowrap!important;
        transition:color ${speed}ms cubic-bezier(.65,0,.076,1)!important;
        z-index:2!important;
      }
      ${selector}:hover .ssx-circle-expand-circle{width:100%!important;}
      ${selector}:hover .ssx-circle-expand-icon{background:${arrow}!important;transform:translate(${Math.round(circle * .34)}px,-50%)!important;}
      ${selector}:hover .ssx-circle-expand-text{color:${hoverText}!important;}
      ${selector}:active .ssx-circle-expand-wrap{transform:scale(.985)!important;}
    `);
  }

  function applyLineFillArrowButtonStyle(button, settings = {}, theme = config.theme || defaultConfig.theme) {
    const buttonId = button.dataset.ssxButtonId;
    if (!buttonId) return;
    const { fontSize, height, width } = buttonMeasure(button, settings);
    const fill = settings.color || theme.ink;
    const text = settings.textColor || theme.ink;
    const hoverText = settings.hoverTextColor || theme.soft;
    const line = settings.outlineColor || fill;
    const speed = Math.round(220 + (1 - intensity01(settings.intensity)) * 260);
    const rightPad = Math.round(24 + param(settings, "width", 50) * .16);
    const selector = `[data-ssx-button-id="${buttonId}"]`;
    buttonScopedStyle(button, `
      ${selector},.sqs-block-newsletter ${selector}{
        display:inline-block!important;
      ...
      ${selector} .ssx-line-fill-wrap{
        ...
        width:${width}px!important;
        height:${height}px!important;
        padding:0 ${rightPad}px 0 ${Math.round(rightPad * .72)}px!important;
        box-sizing:border-box!important;
        transition:padding ${speed}ms ease,color ${speed}ms ease!important;
      }
      ${selector} .ssx-line-fill-text{
        position:relative!important;
        z-index:2!important;
        color:${text}!important;
        font-size:${fontSize}px!important;
        ...
        line-height:1!important;
        white-space:nowrap!important;
        transition:color ${speed}ms ease!important;
      }
      ${selector} .ssx-line-fill-bg{
        position:absolute!important;
        right:0!important;
        top:50%!important;
        width:${Math.max(28, Math.round(height * .66))}px!important;
        height:2px!important;
        margin-top:-1px!important;
        background:${line}!important;
        z-index:1!important;
        transition:width ${speed}ms ease,height ${speed}ms ease,transform ${speed}ms ease,background 180ms ease!important;
      }
      ${selector} .ssx-line-fill-bg:before,
      ${selector} .ssx-line-fill-bg:after{
        content:""!important;
        position:absolute!important;
        right:-2px!important;
        width:${Math.max(8, Math.round(height * .2))}px!important;
        height:2px!important;
        background:${line}!important;
        transition:right ${speed}ms ease,top ${speed}ms ease,bottom ${speed}ms ease,background 180ms ease!important;
      }
      ${selector} .ssx-line-fill-bg:before{bottom:${Math.max(3, Math.round(height * .08))}px!important;transform:rotate(45deg)!important;}
      ${selector} .ssx-line-fill-bg:after{top:${Math.max(3, Math.round(height * .08))}px!important;transform:rotate(-45deg)!important;}
      ${selector}:hover .ssx-line-fill-wrap{padding-right:${Math.round(rightPad * .72)}px!important;}
      ${selector}:hover .ssx-line-fill-text{color:${hoverText}!important;}
      ${selector}:hover .ssx-line-fill-bg{
        width:100%!important;
        height:100%!important;
        transform:translateY(-50%)!important;
        background:${fill}!important;
      }
      ${selector}:hover .ssx-line-fill-bg:before,
      ${selector}:hover .ssx-line-fill-bg:after{right:0!important;background:${fill}!important;}
      ${selector}:hover .ssx-line-fill-bg:before{bottom:${Math.max(5, Math.round(height * .15))}px!important;}
      ${selector}:hover .ssx-line-fill-bg:after{top:${Math.max(5, Math.round(height * .15))}px!important;}
      ${selector}:active .ssx-line-fill-wrap{transform:scale(.985)!important;}
    `);
  }

  function applyRotatingCircleButtonStyle(button, settings = {}, theme = config.theme || defaultConfig.theme) {
    const buttonId = button.dataset.ssxButtonId;
    if (!buttonId) return;
    const { fontSize } = buttonMeasure(button, settings);
    const size = Math.round(78 + param(settings, "height", 58) * .72);
    const center = Math.round(size * .42);
    const fill = settings.color || theme.primary;
    const text = settings.textColor || theme.soft;
    const centerFill = settings.hoverColor || theme.soft;
    const centerText = settings.outlineColor || theme.primary;
    const speed = Math.max(5, Math.round(14 - intensity01(settings.intensity) * 8));
    const selector = `[data-ssx-button-id="${buttonId}"]`;
    buttonScopedStyle(button, `
      ${selector},.sqs-block-newsletter ${selector}{display:inline-grid!important;place-items:center!important;width:${size}px!important;min-width:${size}px!important;max-width:${size}px!important;height:${size}px!important;min-height:${size}px!important;padding:0!important;border:0!important;background:transparent!important;color:${text}!important;text-decoration:none!important;line-height:1!important;box-shadow:none!important;overflow:visible!important;position:relative!important;isolation:isolate!important;transform:none!important;vertical-align:middle!important}
      ${selector}:hover{opacity:1!important;transform:none!important}
      ${selector} .ssx-rotate-circle-wrap{position:relative!important;width:${size}px!important;height:${size}px!important;border-radius:999px!important;background:${fill}!important;color:${text}!important;display:grid!important;place-items:center!important;overflow:hidden!important;transition:transform .2s ease,background .2s ease!important}
      ${selector} .ssx-rotate-circle-text{position:absolute!important;inset:0!important;border-radius:999px!important;animation:ssx-rotate-circle-${buttonId} ${speed}s linear infinite!important}
      ${selector} .ssx-rotate-circle-text span{position:absolute!important;left:50%!important;top:8px!important;transform-origin:0 ${Math.round(size / 2 - 8)}px!important;font-size:${Math.max(8, Math.round(fontSize * .54))}px!important;font-weight:850!important;line-height:1!important;color:${text}!important;text-transform:uppercase!important}
      ${selector} .ssx-rotate-circle-core{position:relative!important;z-index:2!important;width:${center}px!important;height:${center}px!important;border-radius:999px!important;background:${centerFill}!important;color:${centerText}!important;display:grid!important;place-items:center!important;font-size:${Math.round(center * .42)}px!important;font-weight:900!important;transition:transform .24s cubic-bezier(.34,1.56,.64,1),background .2s ease,color .2s ease!important}
      ${selector}:hover .ssx-rotate-circle-wrap{transform:scale(1.04)!important}
      ${selector}:hover .ssx-rotate-circle-core{transform:rotate(45deg) scale(1.04)!important}
      ${selector}:active .ssx-rotate-circle-wrap{transform:scale(.97)!important}
      @keyframes ssx-rotate-circle-${buttonId}{to{transform:rotate(360deg)}}
    `);
  }

  function applyLetterFlipV2ButtonStyle(button, settings = {}, theme = config.theme || defaultConfig.theme) {
    const buttonId = button.dataset.ssxButtonId;
    if (!buttonId) return;
    const { fontSize, height, width } = buttonMeasure(button, settings);
    const fill = settings.color || theme.primary;
    const text = settings.textColor || theme.soft;
    const hover = settings.hoverColor || fill;
    const outline = settings.outlineColor || fill;
    const radius = Math.round(18 + param(settings, "radius", 62) * .46);
    const speed = Math.round(220 + (1 - intensity01(settings.intensity)) * 280);
    const selector = `[data-ssx-button-id="${buttonId}"]`;
    buttonScopedStyle(button, `
      ${selector},.sqs-block-newsletter ${selector}{display:inline-grid!important;place-items:center!important;width:${width}px!important;min-width:${width}px!important;max-width:${width}px!important;height:${height}px!important;min-height:${height}px!important;padding:0!important;border:1px solid ${hexToRgba(outline,.34)}!important;border-radius:${radius}px!important;background:${fill}!important;color:${text}!important;text-decoration:none!important;line-height:1!important;box-shadow:none!important;overflow:hidden!important;position:relative!important;isolation:isolate!important;transform:none!important;vertical-align:middle!important}
      ${selector}:hover{background:${hover}!important;opacity:1!important;transform:none!important}
      ${selector} .ssx-letter-flip-wrap{position:relative!important;width:100%!important;height:100%!important;display:grid!important;place-items:center!important;color:${text}!important;font-size:${fontSize}px!important;font-weight:850!important;line-height:1!important;white-space:nowrap!important}
      ${selector} .ssx-letter-layer{position:absolute!important;inset:0!important;display:flex!important;align-items:center!important;justify-content:center!important;color:${text}!important}
      ${selector} .ssx-letter-bottom .ssx-btn-char{transform:translateY(-1.25em)!important}
      ${selector}:hover .ssx-letter-top .ssx-btn-char{transform:translateY(1.25em)!important}
      ${selector}:hover .ssx-letter-bottom .ssx-btn-char{transform:translateY(0)!important}
      ${selector} .ssx-btn-char{display:inline-block!important;color:${text}!important;transition:transform ${speed}ms cubic-bezier(.22,1,.36,1)!important}
      ${selector}:active{transform:scale(.985)!important}
    `);
  }

  function applyShineFillV2ButtonStyle(button, settings = {}, theme = config.theme || defaultConfig.theme) {
    const buttonId = button.dataset.ssxButtonId;
    if (!buttonId) return;
    const { fontSize, height, width } = buttonMeasure(button, settings);
    const fill = settings.color || theme.primary;
    ...
    const shine = settings.hoverColor || theme.accent;
    const outline = settings.outlineColor || fill;
    const radius = Math.round(14 + param(settings, "radius", 60) * .52);
    const speed = Math.round(420 + (1 - intensity01(settings.intensity)) * 420);
    const selector = `[data-ssx-button-id="${buttonId}"]`;
    buttonScopedStyle(button, `
      ${selector},.sqs-block-newsletter ${selector}{display:inline-grid!important;place-items:center!important;width:${width}px!important;min-width:${width}px!important;max-width:${width}px!important;height:${height}px!important;min-height:${height}px!important;padding:0!important;border:1px solid ${hexToRgba(outline,.32)}!important;border-radius:${radius}px!important;background:${fill}!important;color:${text}!important;text-decoration:none!important;line-height:1!important;box-shadow:0 10px 22px ${hexToRgba(fill,.14)}!important;overflow:hidden!important;position:relative!important;isolation:isolate!important;transform:none!important;vertical-align:middle!important}
      ${selector}:before{content:""!important;position:absolute!important;top:-80%!important;left:-60%!important;width:38%!important;height:260%!important;background:linear-gradient(90deg,transparent,${hexToRgba(shine,.72)},transparent)!important;transform:rotate(24deg)!important;transition:left ${speed}ms ease!important;z-index:1!important;pointer-events:none!important}
      ${selector}:hover:before{left:130%!important}
      ${selector}:hover{opacity:1!important;transform:none!important;box-shadow:0 14px 28px ${hexToRgba(fill,.18)}!important}
      ${selector} .ssx-simple-btn-text{position:relative!important;z-index:2!important;color:${text}!important;font-size:${fontSize}px!important;font-weight:850!important;line-height:1!important;white-space:nowrap!important}
      ${selector}:active{transform:scale(.985)!important}
    `);
  }

  function applyArrowNudgeV2ButtonStyle(button, settings = {}, theme = config.theme || defaultConfig.theme) {
    const buttonId = button.dataset.ssxButtonId;
    if (!buttonId) return;
    const { fontSize, height, width } = buttonMeasure(button, settings);
    const fill = settings.color || theme.accent;
    ...
    const hover = settings.hoverColor || theme.gradientB;
    const outline = settings.outlineColor || fill;
    const radius = Math.round(22 + param(settings, "radius", 72) * .56);
    const move = Math.round(3 + intensity01(settings.intensity) * 7);
    const selector = `[data-ssx-button-id="${buttonId}"]`;
    buttonScopedStyle(button, `
      ${selector},.sqs-block-newsletter ${selector}{display:inline-flex!important;align-items:center!important;justify-content:center!important;width:${width}px!important;min-width:${width}px!important;max-width:${width}px!important;height:${height}px!important;min-height:${height}px!important;padding:0 ${Math.round(height * .22)}px!important;border:1px solid ${hexToRgba(outline,.30)}!important;border-radius:${radius}px!important;background:${fill}!important;color:${text}!important;text-decoration:none!important;line-height:1!important;box-shadow:0 9px 18px ${hexToRgba(fill,.13)}!important;overflow:hidden!important;position:relative!important;isolation:isolate!important;transform:none!important;vertical-align:middle!important;gap:10px!important}
      ${selector}:hover{background:${hover}!important;opacity:1!important;transform:none!important}
      ${selector} .ssx-arrow-nudge-text{color:${text}!important;font-size:${fontSize}px!important;font-weight:850!important;line-height:1!important;white-space:nowrap!important}
      ${selector} .ssx-arrow-nudge-icon{width:${Math.round(height * .48)}px!important;height:${Math.round(height * .48)}px!important;border-radius:999px!important;display:grid!important;place-items:center!important;background:${text}!important;color:${fill}!important;font-size:${Math.round(height * .28)}px!important;font-weight:900!important;transition:transform .25s ease,background .2s ease,color .2s ease!important;flex:0 0 auto!important}
      ${selector}:hover .ssx-arrow-nudge-icon{transform:translateX(${move}px) rotate(45deg)!important;background:${theme.ink}!important;color:${theme.soft}!important}
      ${selector}:active{transform:scale(.965)!important}
    `);
  }

  function applyBouncyV2ButtonStyle(button, settings = {}, theme = config.theme || defaultConfig.theme) {
    const buttonId = button.dataset.ssxButtonId;
    if (!buttonId) return;
    const { fontSize, height, width } = buttonMeasure(button, settings);
    const fill = settings.color || theme.accent;
    ...
    const hover = settings.hoverColor || theme.gradientB;
    const outline = settings.outlineColor || fill;
    const radius = Math.round(20 + param(settings, "radius", 72) * .55);
    const scale = (1.01 + intensity01(settings.intensity) * .05).toFixed(3);
    const selector = `[data-ssx-button-id="${buttonId}"]`;
    buttonScopedStyle(button, `
      ${selector},.sqs-block-newsletter ${selector}{display:inline-grid!important;place-items:center!important;width:${width}px!important;min-width:${width}px!important;max-width:${width}px!important;height:${height}px!important;min-height:${height}px!important;padding:0!important;border:1px solid ${hexToRgba(outline,.28)}!important;border-radius:${radius}px!important;background:${fill}!important;color:${text}!important;text-decoration:none!important;line-height:1!important;box-shadow:0 10px 22px ${hexToRgba(fill,.14)},inset 0 1px 0 rgba(255,255,255,.28)!important;overflow:hidden!important;position:relative!important;isolation:isolate!important;transform:none!important;vertical-align:middle!important;transition:transform .34s cubic-bezier(.34,1.56,.64,1),background .2s ease,box-shadow .2s ease!important}
      ${selector}:hover{background:${hover}!important;opacity:1!important;transform:translateY(-1px) scale(${scale})!important;box-shadow:0 16px 30px ${hexToRgba(hover,.18)},inset 0 1px 0 rgba(255,255,255,.36)!important}
      ${selector} .ssx-simple-btn-text{position:relative!important;z-index:2!important;color:${text}!important;font-size:${fontSize}px!important;font-weight:850!important;line-height:1!important;white-space:nowrap!important}
      ${selector}:active{transform:translateY(1px) scale(.97)!important}
    `);
  }

  function restoreButtonAdapter(button) {
    if (!button || !button.dataset?.ssxButtonAdapter) return;
    ["ssx-btn-animated-arrow-pill", "ssx-btn-circle-expand", "ssx-btn-line-fill-arrow", "ssx-btn-rotating-circle", "ssx-btn-letter-flip-v2", "ssx-btn-shine-fill-v2", "ssx-btn-arrow-nudge-v2", "ssx-btn-bouncy-v2", "ssx-btn-bouncy-cta", "ssx-btn-letter-flip", "ssx-btn-swap-arrow", "ssx-btn-arrow-push", "ssx-button-adapter"].forEach((className) => button.classList.remove(className));
    if (button.__ssxOriginalHtml != null) button.innerHTML = button.__ssxOriginalHtml;
    delete button.__ssxOriginalHtml;
    delete button.dataset.ssxButtonAdapter;
    delete button.dataset.ssxButtonSignature;
    const buttonId = button.dataset.ssxButtonId;
    if (buttonId) document.getElementById(`${APP_ID}-button-${buttonId}`)?.remove();
    delete button.dataset.ssxButtonId;
    button.removeAttribute("data-ssx-bouncy-id");
    button.style.removeProperty("--ssx-btn-width");
  }

  function applyButtonAdapter(button, effectId, className, settings = {}, signature = effectId, theme = config.theme || defaultConfig.theme) {
    const label = (button.textContent || button.getAttribute("aria-label") || "Button").replace(/[↗↖↘↙]/g, "").replace(/\s+/g, " ").trim() || "Button";
    button.__ssxOriginalHtml = button.innerHTML;
    button.dataset.ssxButtonAdapter = effectId;
    button.dataset.ssxButtonSignature = signature;
    if (!button.dataset.ssxButtonId) button.dataset.ssxButtonId = `spbtn-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
    button.classList.add("ssx-button-adapter", className);
    if (!button.getAttribute("aria-label")) button.setAttribute("aria-label", label);

    if (effectId === "btn-style-animated-arrow-pill") {
      button.textContent = "";
      const box = document.createElement("span");
      box.className = "ssx-spbtn-box";
      const left = document.createElement("span");
      left.className = "ssx-spbtn-left";
      const leftArrow = document.createElement("span");
      leftArrow.className = "ssx-spbtn-arrow";
      leftArrow.setAttribute("aria-hidden", "true");
      leftArrow.textContent = "↗";
      left.appendChild(leftArrow);
      const main = document.createElement("span");
      main.className = "ssx-spbtn-main";
      main.textContent = label;
      const right = document.createElement("span");
      right.className = "ssx-spbtn-right";
      const rightArrow = document.createElement("span");
      rightArrow.className = "ssx-spbtn-arrow";
    ...
    if (effectId === "btn-style-circle-expand") {
      button.textContent = "";
      const wrap = document.createElement("span");
      wrap.className = "ssx-circle-expand-wrap";
      const circle = document.createElement("span");
      circle.className = "ssx-circle-expand-circle";
      const icon = document.createElement("span");
      icon.className = "ssx-circle-expand-icon";
      icon.setAttribute("aria-hidden", "true");
      circle.appendChild(icon);
      const text = document.createElement("span");
      text.className = "ssx-circle-expand-text";
    ...
    if (effectId === "btn-style-line-fill-arrow") {
      button.textContent = "";
      const wrap = document.createElement("span");
      wrap.className = "ssx-line-fill-wrap";
      const text = document.createElement("span");
      text.className = "ssx-line-fill-text";
      text.textContent = label;
      const bg = document.createElement("span");
      bg.className = "ssx-line-fill-bg";
    ...
    if (effectId === "btn-style-rotating-circle") {
      button.textContent = "";
      const wrap = document.createElement("span");
      wrap.className = "ssx-rotate-circle-wrap";
      const ring = document.createElement("span");
      ring.className = "ssx-rotate-circle-text";
      const ringText = `${label} • ${label} • `;
      [...ringText].forEach((char, index) => {
        const item = document.createElement("span");
        item.textContent = char;
        item.style.transform = `rotate(${index * (360 / ringText.length)}deg)`;
        ring.appendChild(item);
      });
      const core = document.createElement("span");
      core.className = "ssx-rotate-circle-core";
    ...
    if (effectId === "btn-style-letter-flip-v2") {
      button.textContent = "";
      const wrap = document.createElement("span");
      wrap.className = "ssx-letter-flip-wrap";
      const top = makeLetterLayer(label, "ssx-letter-layer ssx-letter-top");
      const bottom = makeLetterLayer(label, "ssx-letter-layer ssx-letter-bottom");
    ...
    if (effectId === "btn-style-shine-fill-v2" || effectId === "btn-style-bouncy-v2") {
      button.textContent = "";
      const text = document.createElement("span");
      text.className = "ssx-simple-btn-text";
      text.textContent = label;
    ...
    if (effectId === "btn-style-arrow-nudge-v2") {
      button.textContent = "";
      const text = document.createElement("span");
      text.className = "ssx-arrow-nudge-text";
      text.textContent = label;
      const icon = document.createElement("span");
      icon.className = "ssx-arrow-nudge-icon";
      icon.setAttribute("aria-hidden", "true");
      icon.textContent = "↗";
      button.appendChild(text);
    ...
    if (effectId === "btn-style-bouncy-cta") {
      button.textContent = "";
      button.setAttribute("data-ssx-bouncy-id", "1");
      const box = document.createElement("span");
      box.className = "spb-349-box d-flex";
      const left = document.createElement("span");
      left.className = "spb-349-left d-flex";
      const leftArrow = document.createElement("small");
      leftArrow.textContent = "↗";
      left.appendChild(leftArrow);
      const main = document.createElement("span");
      main.className = "spb-349-main d-flex";
      const text = document.createElement("span");
      text.className = "ssx-btn-label";
      text.textContent = label;
      main.appendChild(text);
      const right = document.createElement("span");
      right.className = "spb-349-right d-flex";
      const rightArrow = document.createElement("small");
      rightArrow.textContent = "↗";
      right.appendChild(rightArrow);
      box.appendChild(left);
      box.appendChild(main);
    ...
    if (effectId === "btn-style-letter-flip") {
      button.textContent = "";
      button.appendChild(makeLetterLayer(label, "ssx-flip-top"));
      button.appendChild(makeLetterLayer(label, "ssx-flip-bottom"));
      return;
    }

    if (effectId === "btn-style-swap-arrow") {
      const hoverLabel = String(settings.customText || "Right Now").trim() || "Right Now";
      button.textContent = "";
      const wrap = document.createElement("span");
      wrap.className = "ssx-btn-swap-wrap";
      const top = document.createElement("span");
      top.className = "ssx-btn-swap-top";
      top.textContent = label;
      const bottom = document.createElement("span");
      bottom.className = "ssx-btn-swap-bottom";
      bottom.textContent = hoverLabel;
      wrap.appendChild(top);
      wrap.appendChild(bottom);
      button.appendChild(wrap);
      button.appendChild(makeCircleArrowIcon("ssx-btn-circle-icon"));
      return;
    }

    if (effectId === "btn-style-arrow-push") {
      button.textContent = "";
      const text = document.createElement("span");
      text.className = "ssx-btn-label";
      text.textContent = label;
      button.appendChild(text);
      button.appendChild(makeArrowSvg());
      return;
    }
  ...
  function makeLetterLayer(label, className) {
    const layer = document.createElement("span");
    layer.className = `ssx-btn-text ${className}`;
    Array.from(label).forEach((char) => {
      const span = document.createElement("span");
      span.className = "ssx-btn-char";
  ...
  function makeArrowIcon(className) {
    ...
    icon.setAttribute("aria-hidden", "true");
    icon.innerHTML = '<svg viewBox="0 0 24 24" focusable="false"><path d="M16.1716 10.9999L10.8076 5.63589L12.2218 4.22168L20 11.9999L12.2218 19.778L10.8076 18.3638L16.1716 12.9999H4V10.9999H16.1716Z"></path></svg>';
    return icon;
  }
  ...
  function makeArrowSvg() {
    ...
    svg.innerHTML = '<path d="M16.1716 10.9999L10.8076 5.63589L12.2218 4.22168L20 11.9999L12.2218 19.778L10.8076 18.3638L16.1716 12.9999H4V10.9999H16.1716Z"></path>';
    return svg;
  }

  function makeCircleArrowIcon(className) {
    const icon = document.createElement("span");
    icon.className = className;
    icon.setAttribute("aria-hidden", "true");
    icon.innerHTML = '<svg class="ssx-btn-arrow-svg" viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M16.1716 10.9999L10.8076 5.63589L12.2218 4.22168L20 11.9999L12.2218 19.778L10.8076 18.3638L16.1716 12.9999H4V10.9999H16.1716Z"></path></svg>';
    return icon;
  }
  ...
  function motionTargetsFor(selector, effect) {
    const textSelector = "h1,h2,h3,h4,p,li,blockquote,.sqsrte-large,.sqsrte-small";
    const imageSelector = "img,picture,figure,.image-block,.sqs-block-image,.fluid-image-container,.image-card";
    const scopeSelector = selector === "body" || selector === "html" ? "" : selector;
    const targetSelector = /^txt-scroll-/.test(effect.id)
      ? textSelector
      : /^img-scroll-/.test(effect.id)
        ? imageSelector
        : "";
    if (!targetSelector) {
      try {
        return Array.from(document.querySelectorAll(selector)).filter((element) => element !== document.body && element !== document.documentElement && !element.closest(`#${APP_ID}`));
      } catch {
        return [];
      }
    }
    try {
      const query = scopeSelector ? `${scopeSelector}:is(${targetSelector}),${scopeSelector} :is(${targetSelector})` : targetSelector;
      return Array.from(document.querySelectorAll(query)).filter((element) => {
        if (element === document.body || element === document.documentElement || element.closest(`#${APP_ID}`)) return false;
        if (/^txt-scroll-/.test(effect.id) && element.querySelector("img,button,video,iframe,canvas,a > svg")) return false;
        return true;
      });
    } catch {
      return [];
...
`;
  ...
  function setupMotionObservers() {
    ...
    const animated = document.querySelectorAll(".ssx-reveal,.ssx-parallax,.ssx-stagger,.ssx-word-blur-reveal");
  ...
  function cleanupWordLoops() {
    wordLoopTimers.forEach((timer) => window.clearInterval(timer));
    wordLoopTimers = [];
    document.querySelectorAll(".ssx-word-loop[data-ssx-word-loop]").forEach((node) => {
      const text = document.createTextNode(node.getAttribute("data-original") || "loop");
      node.replaceWith(text);
      if (text.parentNode && text.parentNode.normalize) text.parentNode.normalize();
    });
  }

  function parseLoopWords(settings) {
    return String(settings.customText || "poema, nice")
      .split(",")
      .map((word) => word.trim())
      .filter(Boolean)
      .slice(0, 12);
  ...
  function wordLoopMode(settings) {
    const style = param(settings, "style", 50);
    if (style < 34) return "instant";
    if (style > 66) return "blur";
    return "slide";
  }

  function setupWordLoop() {
    cleanupWordLoops();
    const active = [];
    Object.entries(config.effects || {}).forEach(([selector, effects]) => {
      const settings = effects && effects["txt-word-loop"];
      if (settings && settings.enabled) active.push({ selector, settings });
    });
    if (!active.length) return;
    active.forEach(({ selector, settings }) => {
      const words = parseLoopWords(settings);
      if (!words.length) return;
      const mode = wordLoopMode(settings);
      const interval = Math.max(650, Math.round(2300 - clampIntensity(settings.intensity || 60) * 14));
      let targets = [];
      try {
        targets = Array.from(document.querySelectorAll(wordLoopTargets(selector)));
      } catch {
        targets = [];
      }
      targets.forEach((target) => wrapLoopMarkerInTarget(target, words, mode, interval));
    });
  }

  function wrapLoopMarkerInTarget(target, words, mode, interval) {
    if (!target || target.closest(`#${APP_ID}`) || target.querySelector(".ssx-word-loop")) return;
    const walker = document.createTreeWalker(target, NodeFilter.SHOW_TEXT, {
      acceptNode(node) {
        if (!node.nodeValue || !/\bloop\b/i.test(node.nodeValue)) return NodeFilter.FILTER_REJECT;
        const parent = node.parentElement;
        if (!parent || parent.closest(`#${APP_ID},.ssx-word-loop,script,style,noscript,textarea,input,button,svg`)) return NodeFilter.FILTER_REJECT;
        return NodeFilter.FILTER_ACCEPT;
      }
    });
    const nodes = [];
    while (walker.nextNode()) nodes.push(walker.currentNode);
    nodes.forEach((node) => replaceLoopMarkers(node, words, mode, interval));
  }

  function replaceLoopMarkers(node, words, mode, interval) {
    const parts = String(node.nodeValue || "").split(/(\bloop\b)/gi);
    if (parts.length <= 1) return;
    const fragment = document.createDocumentFragment();
    parts.forEach((part) => {
      if (/^loop$/i.test(part)) fragment.appendChild(createWordLoopNode(part, words, mode, interval));
      else if (part) fragment.appendChild(document.createTextNode(part));
    });
    node.replaceWith(fragment);
  }

  function createWordLoopNode(original, words, mode, interval) {
    const outer = document.createElement("span");
    outer.className = `ssx-word-loop is-${mode}`;
    outer.setAttribute("data-ssx-word-loop", "1");
    outer.setAttribute("data-original", original || "loop");
    outer.style.setProperty("--ssx-word-loop-width", `${Math.max(...words.map((word) => word.length), 4)}ch`);
    const inner = document.createElement("span");
    inner.className = "ssx-word-loop-word";
  ...
  function setupCursorFollower(theme) {
    ...
    document.getElementById(`${CURSOR_ID}-trail`)?.remove();
    const entries = [];
    Object.entries(config.effects || {}).forEach(([selector, effects]) => {
      Object.entries(effects || {}).forEach(([id, settings]) => {
        if (settings && settings.enabled && /^(txt|img|site)-cursor-/.test(id)) entries.push({ selector, id, settings });
      });
    });
    const visualEntries = entries.filter((entry) => !/-cursor-hide$/.test(entry.id));
    ...
    cursor.style.cssText = [
      ...
      "font-weight:900",
      "line-height:1",
      "transition:width .12s ease,height .12s ease,opacity .12s ease,background .12s ease,color .12s ease,border .12s ease,box-shadow .12s ease"
    ].join(";");
    const cursorTrail = document.createElement("div");
    cursorTrail.id = `${CURSOR_ID}-trail`;
    ...
    cursorTrail.style.cssText = [
      ...
      "transform:translate(-50%,-50%)",
      "border-radius:999px",
      "transition:left .18s cubic-bezier(.2,.8,.2,1),top .18s cubic-bezier(.2,.8,.2,1),width .14s ease,height .14s ease,opacity .14s ease,border .14s ease,background .14s ease"
    ].join(";");
    ...
    cursorMoveHandler = (event) => {
      ...
      const matched = cursorEntryForTarget(visualEntries, event.target);
      if (!matched) {
        cursor.style.display = "none";
        cursorTrail.style.display = "none";
        return;
      }
      const nativeDot = visualEntries.find((entry) => entry.selector === matched.selector && /-cursor-native-dot$/.test(entry.id));
      const trail = visualEntries.find((entry) => entry.selector === matched.selector && /-cursor-trail$/.test(entry.id));
      const ring = visualEntries.find((entry) => entry.selector === matched.selector && /-cursor-ring$/.test(entry.id));
      const dot = visualEntries.find((entry) => entry.selector === matched.selector && /-cursor-dot$/.test(entry.id));
      const glow = visualEntries.find((entry) => entry.selector === matched.selector && /-cursor-glow$/.test(entry.id));
      const spotlight = visualEntries.find((entry) => entry.selector === matched.selector && /-cursor-spotlight$/.test(entry.id));
      const crosshair = visualEntries.find((entry) => entry.selector === matched.selector && /-cursor-crosshair$/.test(entry.id));
      const outline = visualEntries.find((entry) => entry.selector === matched.selector && /-cursor-outline$/.test(entry.id));
      const large = visualEntries.find((entry) => entry.selector === matched.selector && /-cursor-large$/.test(entry.id));
      const emoji = visualEntries.find((entry) => entry.selector === matched.selector && /-cursor-emoji$/.test(entry.id));
      const sizeSource = spotlight || glow || crosshair || outline || nativeDot || trail || ring || dot || large || emoji || matched;
      const size = Math.round(24 + clampIntensity(sizeSource.settings.intensity || 60) * 0.42);
      const cursorColor = sizeSource.settings.color || theme.accent;
      const outlineColor = sizeSource.settings.outlineColor || theme.primary;
      const outlineSize = Math.round(size * (0.75 + param(sizeSource.settings, "outlineSize", 60) * 0.012));
      const borderWidth = Math.max(1, Math.round(1 + param(sizeSource.settings, "outlineWidth", 35) * 0.04));
      cursor.style.display = nativeDot ? "none" : "grid";
      ...
      if (nativeDot) {
        ...
        cursorTrail.style.width = `${dotSize}px`;
        cursorTrail.style.height = `${dotSize}px`;
        cursorTrail.style.background = cursorColor;
        cursorTrail.style.border = `${borderWidth}px solid ${hexToRgba(outlineColor, 0.34)}`;
        cursorTrail.style.opacity = "0.96";
        cursorTrail.style.boxShadow = `0 0 0 ${Math.round(dotSize * 0.85)}px ${hexToRgba(cursorColor, 0.12)},0 10px 24px ${hexToRgba(outlineColor, 0.18)}`;
        cursorTrail.style.mixBlendMode = "normal";
        cursorTrail.style.backdropFilter = "none";
        cursorTrail.style.webkitBackdropFilter = "none";
      } else if (emoji) {
        const customCursor = String(emoji.settings.customText || "").trim().slice(0, 8);
        ...
        cursor.textContent = customCursor || cursorEmojiOptions[emojiIndex] || cursorEmojiOptions[0];
        ...
        cursor.textContent = "";
        cursor.style.background = hexToRgba(cursorColor, .18);
        cursor.style.border = `${borderWidth}px solid ${hexToRgba(outlineColor, .34)}`;
        cursor.style.boxShadow = `0 14px 42px ${hexToRgba(outlineColor, .18)}, inset 0 0 0 1px ${hexToRgba(theme.soft, .45)}`;
        cursor.style.backdropFilter = "blur(2px)";
        cursor.style.webkitBackdropFilter = "blur(2px)";
        cursorTrail.style.display = "none";
      } else if (glow) {
        const dotSize = Math.max(12, Math.round(size * .36));
        cursor.style.width = `${dotSize}px`;
        cursor.style.height = `${dotSize}px`;
        cursor.textContent = "";
        cursor.style.background = cursorColor;
        cursor.style.border = `${borderWidth}px solid ${hexToRgba(outlineColor, .32)}`;
        cursor.style.boxShadow = `0 0 0 ${Math.round(outlineSize * .2)}px ${hexToRgba(cursorColor, .24)}, 0 0 ${Math.round(outlineSize * .72)}px ${hexToRgba(outlineColor, .38)}`;
        cursorTrail.style.display = "none";
      } else if (crosshair) {
        cursor.textContent = "+";
        cursor.style.background = "transparent";
        cursor.style.border = `${borderWidth}px solid ${hexToRgba(outlineColor, .55)}`;
        cursor.style.color = outlineColor;
        cursor.style.fontSize = `${Math.round(size * .78)}px`;
        cursor.style.boxShadow = `0 0 0 1px ${hexToRgba(theme.soft, .86)}, 0 0 0 ${Math.round(outlineSize * .15)}px ${hexToRgba(cursorColor, .12)}`;
        cursorTrail.style.display = "none";
      } else if (outline) {
        cursor.textContent = "";
        cursor.style.background = "transparent";
        cursor.style.border = `${borderWidth}px solid ${outlineColor}`;
        cursor.style.boxShadow = `inset 0 0 0 2px ${hexToRgba(theme.soft, .72)}, 0 0 0 ${Math.round(outlineSize * .14)}px ${hexToRgba(cursorColor, .12)}`;
        cursorTrail.style.display = "none";
      } else if (trail) {
        const dotSize = Math.max(8, Math.round(size * .28));
        cursor.style.width = `${dotSize}px`;
        cursor.style.height = `${dotSize}px`;
        cursor.textContent = "";
        cursor.style.background = cursorColor;
        cursor.style.border = `1px solid ${hexToRgba(outlineColor, .42)}`;
        cursor.style.boxShadow = `0 8px 18px ${hexToRgba(outlineColor, .22)}`;
        cursorTrail.style.display = "block";
        cursorTrail.style.left = `${event.clientX}px`;
        cursorTrail.style.top = `${event.clientY}px`;
        cursorTrail.style.width = `${outlineSize}px`;
        cursorTrail.style.height = `${outlineSize}px`;
        cursorTrail.style.background = hexToRgba(cursorColor, .08);
        cursorTrail.style.border = `${borderWidth}px solid ${hexToRgba(outlineColor, .48)}`;
        cursorTrail.style.opacity = ".95";
      } else if (ring) {
        cursor.textContent = "";
        cursor.style.background = hexToRgba(cursorColor, .08);
        cursor.style.border = `${borderWidth}px solid ${outlineColor}`;
        cursor.style.boxShadow = `0 0 0 ${Math.round(outlineSize * .12)}px ${hexToRgba(cursorColor, .16)}`;
        cursorTrail.style.display = "none";
      } else if (dot) {
        const dotSize = Math.max(10, Math.round(size * .42));
        cursor.style.width = `${dotSize}px`;
        cursor.style.height = `${dotSize}px`;
        cursor.textContent = "";
        cursor.style.background = cursorColor;
        cursor.style.border = `${borderWidth}px solid ${hexToRgba(outlineColor, .38)}`;
        cursor.style.boxShadow = `0 10px 24px ${hexToRgba(outlineColor, .20)}`;
        cursorTrail.style.display = "none";
      } else if (large) {
        cursor.textContent = "";
        cursor.style.background = cursorColor;
        cursor.style.border = `${borderWidth}px solid ${outlineColor}`;
        cursor.style.boxShadow = `0 10px 28px ${hexToRgba(outlineColor, .22)}`;
        cursorTrail.style.display = "none";
      } else {
        cursor.textContent = "";
        cursor.style.background = "transparent";
  ...
  function renderPanel() {
    ...
    if (localStorage.getItem(PANEL_KEY) === "0") {
      renderCollapsed();
      return;
    }
      ...
      <div class="ssx-panel" role="dialog" aria-label="Studio Poema">
        <div class="ssx-head" data-ssx-drag>
          ...
          <div>
            ...
            <div class="ssx-subtitle">v79 · private editor mode</div>
          </div>
  ...
  function renderFeatureDetail(featureId) {
    ...
    if (feature.id === "button-suite") return renderButtonSuite(feature);
  ...
  function renderButtonSuite(feature) {
    const selector = currentScopeTarget(effectById.get(buttonStyleIds[0]));
    const scopeText = selector === "body" ? "All page buttons" : targetLabel(config.activeTarget);
    return `
      <div class="ssx-text-head">
        <button class="ssx-inline-back" type="button" data-ssx-feature-back>← Back</button>
        <div>
          <div class="ssx-tool-title">Button styles</div>
          <p class="ssx-tool-desc">Choose one button preset, then customize colors and motion.</p>
        </div>
        <span class="ssx-scope-pill">Editing: ${escapeHtml(scopeText)}</span>
      </div>
      <div class="ssx-help-box"><p>Use the top scope switch: <strong>Section</strong> styles the selected button or selected area, <strong>Whole page</strong> styles every button on the page.</p></div>
      <div class="ssx-option-list">
        ${renderNoneOption(feature)}
        ${feature.options.map((id) => {
          const effect = effectById.get(id);
          return effect ? renderFeatureOption(feature, effect) : "";
        }).join("")}
      </div>
    `;
  }
  ...
  function renderTextSection(feature, section) {
    ...
    if (section.id === "block") {
      const targetReady = blockTargetReady();
      return `
        <section class="ssx-text-section ${isOpen ? "is-open" : ""}">
          ...
          ${isOpen ? `
            <div class="ssx-text-options">
              <button class="ssx-text-option" type="button" data-ssx-text-section-none="${escapeHtml(section.id)}" aria-pressed="${targetReady && !active}" ${targetReady ? "" : "disabled"}>
                <span>${targetReady ? "Clear block styling" : "No section selected"}</span>
                <span>${targetReady && active ? "×" : ""}</span>
              </button>
  ...
  function renderTextOption(feature, section, effect) {
    ...
    const disabledGlobalMotion = selector === "body" && /^txt-block-/.test(effect.id);
    return `
  ...
  function blockTargetReady() {
    return currentScopeTarget(effectById.get("txt-block-opacity")) !== "body";
  }

  function renderBlockStylingControls() {
    const theme = config.theme || defaultConfig.theme;
    const disabled = !blockTargetReady();
    ...
    return `
      ${disabled ? `
        <div class="ssx-control-title">Select a section or block first</div>
        <button class="ssx-control-line" type="button" data-ssx-pick>
  ...
  function renderCursorControls() {
    ...
    const cursorChoices = [
      ["txt-cursor-dot", "Soft dot cursor"],
      ["txt-cursor-ring", "Ring cursor"],
      ["txt-cursor-trail", "Dot + follower"],
      ["txt-cursor-glow", "Glow cursor"],
      ["txt-cursor-spotlight", "Spotlight lens"],
      ["txt-cursor-crosshair", "Crosshair cursor"],
      ["txt-cursor-outline", "Outline cursor"],
      ["txt-cursor-emoji", "Emoji / custom"],
      ["txt-cursor-large", "Large dot cursor"]
    ];
    const activeChoiceId = cursorChoices.find(([id]) => blockSetting(id).enabled)?.[0] || "txt-cursor-dot";
    const activeVisual = blockSetting(activeChoiceId);
    const sizeValue = activeVisual.intensity || 60;
    ...
    return `
      <label class="ssx-control-line"><span>Enter cursor here</span><input type="text" value="${escapeHtml(emoji.customText || "")}" data-ssx-custom-text-effect="txt-cursor-emoji" maxlength="8" placeholder="${escapeHtml(cursorEmojiOptions[selectedEmoji] || "☺")}"></label>
      ${cursorChoices.map(([id, label]) => {
        const setting = blockSetting(id);
        const detail = id === "txt-cursor-emoji" ? escapeHtml(setting.customText || cursorEmojiOptions[selectedEmoji] || "☺") : `${Math.round(24 + (setting.intensity || sizeValue) * .42)}px`;
        return `<button class="ssx-control-line" type="button" data-ssx-switch-effect="${id}"><span>${label}</span><em>${detail}</em><b class="ssx-toggle-dot ${setting.enabled ? "is-on" : ""}"></b></button>`;
      }).join("")}
      ...
      ${activeChoiceId !== "txt-cursor-emoji" ? `<label class="ssx-control-line"><span>Cursor color</span><input type="color" value="${escapeHtml(theme.accent || defaultConfig.theme.accent)}" data-ssx-color="accent"></label>` : ""}
      <label class="ssx-control-line"><span>Cursor size</span><em>${Math.round(24 + sizeValue * .42)}px</em><input type="range" min="0" max="100" value="${sizeValue}" data-ssx-intensity="${activeChoiceId}" data-ssx-auto-enable="true"></label>
  ...
  function renderTextControls(section, effect, settings, disabled) {
    ...
    if (section.id === "image-cursor") {
      const selectedCursor = /native-dot/i.test(effect.id) ? "Native + dot" : /spotlight/i.test(effect.id) ? "Spotlight" : /crosshair/i.test(effect.id) ? "Crosshair" : /glow/i.test(effect.id) ? "Glow" : /trail/i.test(effect.id) ? "Dot + follower" : /ring/i.test(effect.id) ? "Ring" : /dot/i.test(effect.id) ? "Soft dot" : /large/i.test(effect.id) ? "Large dot" : /emoji/i.test(effect.id) ? "Emoji" : "Custom";
      return `
        ${/emoji/i.test(effect.id) ? `<label class="ssx-control-line"><span>Enter cursor here</span><input type="text" value="${escapeHtml(settings.customText || "")}" data-ssx-custom-text-effect="${escapeHtml(effect.id)}" maxlength="8" placeholder="☺" ${disabled ? "disabled" : ""}></label>` : `<div class="ssx-control-line"><span>Cursor type</span><em>${selectedCursor}</em></div>`}
        ${!/emoji/i.test(effect.id) ? colorControl("Cursor color", "accent") : ""}
    ...
    if (section.id === "loop" && effect.id === "txt-word-loop") {
      const words = settings.customText || "poema, nice";
      const style = param(settings, "style", 50);
      const styleLabel = style < 34 ? "Instant" : style > 66 ? "Blur reveal" : "Slide up";
      const nextStyle = style < 34 ? "50" : style > 66 ? "0" : "100";
      return `
        <label class="ssx-control-line"><span>Words</span><input type="text" value="${escapeHtml(words)}" data-ssx-custom-text-effect="txt-word-loop" maxlength="140" placeholder="poema, nice" ${disabled ? "disabled" : ""}></label>
        <label class="ssx-control-line"><span>Speed</span><em>${speed}</em><input type="range" min="0" max="100" value="${value}" data-ssx-intensity="txt-word-loop" ${disabled ? "disabled" : ""}></label>
        <button class="ssx-control-line" type="button" data-ssx-param-toggle="txt-word-loop:style" data-ssx-param-value="${nextStyle}" ${disabled ? "disabled" : ""}><span>Switch style</span><em>${styleLabel}</em><b>‹ ›</b></button>
        <div class="ssx-help-box"><p>Write the marker word <strong>loop</strong> inside your page text. This tool swaps only that marker with the words above, in the selected section or whole page.</p></div>
      `;
    }
    ...
    if (section.id === "cursor") {
      const emoji = blockSetting("txt-cursor-emoji");
      const selectedCursor = /spotlight/i.test(effect.id) ? "Spotlight" : /crosshair/i.test(effect.id) ? "Crosshair" : /outline/i.test(effect.id) ? "Outline" : /glow/i.test(effect.id) ? "Glow" : /trail/i.test(effect.id) ? "Dot + follower" : /ring/i.test(effect.id) ? "Ring" : /dot/i.test(effect.id) ? "Soft dot" : /large/i.test(effect.id) ? "Large dot" : /emoji/i.test(effect.id) ? "Emoji" : "Hidden";
      const selectedEmoji = Math.max(0, Math.min(cursorEmojiOptions.length - 1, Math.round(param(emoji, "emoji", 0) / (100 / Math.max(1, cursorEmojiOptions.length - 1)))));
      return `
        ${/emoji/i.test(effect.id) ? `<label class="ssx-control-line"><span>Enter cursor here</span><input type="text" value="${escapeHtml(emoji.customText || "")}" data-ssx-custom-text-effect="txt-cursor-emoji" maxlength="8" placeholder="${escapeHtml(cursorEmojiOptions[selectedEmoji] || "☺")}" ${disabled ? "disabled" : ""}></label>` : ""}
        <div class="ssx-control-line"><span>Cursor preview</span><em>${/emoji/i.test(effect.id) ? escapeHtml(cursorEmojiOptions[selectedEmoji] || "☺") : selectedCursor}</em></div>
        ...
        ${!/emoji|hide/i.test(effect.id) ? colorControl("Cursor color", "accent") : ""}
        <div class="ssx-control-grid">
          <label class="ssx-control-card"><small>Size</small><span>${size}</span><input type="range" min="0" max="100" value="${value}" data-ssx-intensity="${escapeHtml(effect.id)}" data-ssx-auto-enable="true" ${disabled ? "disabled" : ""}></label>
        </div>
      `;
    }
    return `
  ...
  function renderFeatureOption(feature, effect) {
    ...
    const settings = getEffectSettings(selector, effect.id);
    const disabledGlobalMotion = selector === "body" && /^txt-block-/.test(effect.id);
    const isSiteCursor = feature.id === "site-cursor-suite";
    const isButtonStyle = feature.id === "button-suite";
    return `
      <article class="ssx-option-card" data-ssx-preview-title="${escapeHtml(effect.label)}" data-ssx-preview-desc="${escapeHtml(effect.description)}">
        ...
        ${isSiteCursor ? renderSiteCursorFeatureControls(effect, settings, disabledGlobalMotion) : isButtonStyle ? renderButtonFeatureControls(effect, settings, disabledGlobalMotion) : `
  ...
  function renderSiteCursorFeatureControls(effect, settings, disabled) {
    const theme = config.theme || defaultConfig.theme;
    const emojiIndex = Math.max(0, Math.min(cursorEmojiOptions.length - 1, Math.round(param(settings, "emoji", 0) / (100 / Math.max(1, cursorEmojiOptions.length - 1)))));
    const outlineSize = param(settings, "outlineSize", 60);
    const outlineWidth = param(settings, "outlineWidth", 35);
    return `
      <div class="ssx-option-controls">
        <div class="ssx-preview ${escapeHtml(effect.preview || "")}"></div>
        <div>
          ${effect.id === "site-cursor-emoji" ? `
            <label class="ssx-control-line"><span>Cursor text</span><input type="text" value="${escapeHtml(settings.customText || "")}" data-ssx-custom-text-effect="site-cursor-emoji" maxlength="8" placeholder="${escapeHtml(cursorEmojiOptions[emojiIndex] || "☺")}" ${disabled ? "disabled" : ""}></label>
            <div class="ssx-emoji-grid">${cursorEmojiOptions.map((item, index) => `<button type="button" data-ssx-site-cursor-emoji="${index}" aria-pressed="${index === emojiIndex}">${escapeHtml(item)}</button>`).join("")}</div>
          ` : `
            <label class="ssx-control-line"><span>Cursor color</span><input type="color" value="${escapeHtml(settings.color || theme.accent || defaultConfig.theme.accent)}" data-ssx-effect-color="${escapeHtml(effect.id)}:color" ${disabled ? "disabled" : ""}></label>
            <label class="ssx-control-line"><span>Outline color</span><input type="color" value="${escapeHtml(settings.outlineColor || theme.primary || defaultConfig.theme.primary)}" data-ssx-effect-color="${escapeHtml(effect.id)}:outlineColor" ${disabled ? "disabled" : ""}></label>
          `}
          <div class="ssx-range-row">
            <input type="range" min="0" max="100" value="${settings.intensity}" data-ssx-intensity="${escapeHtml(effect.id)}" ${disabled ? "disabled" : ""}>
            <div class="ssx-range-label">Dot size · ${Math.round(24 + settings.intensity * .42)}px</div>
          </div>
          ${effect.id !== "site-cursor-emoji" ? `
            <div class="ssx-range-row">
              <input type="range" min="0" max="100" value="${outlineSize}" data-ssx-param-effect="${escapeHtml(effect.id)}" data-ssx-param-key="outlineSize" ${disabled ? "disabled" : ""}>
              <div class="ssx-range-label">Follow / outline size · ${outlineSize}</div>
            </div>
            <div class="ssx-range-row">
              <input type="range" min="0" max="100" value="${outlineWidth}" data-ssx-param-effect="${escapeHtml(effect.id)}" data-ssx-param-key="outlineWidth" ${disabled ? "disabled" : ""}>
              <div class="ssx-range-label">Outline width · ${Math.max(1, Math.round(1 + outlineWidth * .04))}px</div>
            </div>
          ` : ""}
        </div>
        <button class="ssx-restore" type="button" data-ssx-restore-effect="${escapeHtml(effect.id)}">Restore</button>
  ...
  function renderButtonFeatureControls(effect, settings, disabled) {
    const theme = config.theme || defaultConfig.theme;
    const radius = param(settings, "radius", 55);
    const width = param(settings, "width", 50);
    const height = param(settings, "height", 44);
    return `
      <div class="ssx-option-controls">
        <div class="ssx-preview ${escapeHtml(effect.preview || "")}"></div>
        <div>
          <div class="ssx-control-grid">
            <label class="ssx-control-card"><small>Fill</small><span>Color</span><input type="color" value="${escapeHtml(settings.color || theme.primary || defaultConfig.theme.primary)}" data-ssx-effect-color="${escapeHtml(effect.id)}:color" ${disabled ? "disabled" : ""}></label>
            <label class="ssx-control-card"><small>Text</small><span>Color</span><input type="color" value="${escapeHtml(settings.textColor || theme.soft || defaultConfig.theme.soft)}" data-ssx-effect-color="${escapeHtml(effect.id)}:textColor" ${disabled ? "disabled" : ""}></label>
            <label class="ssx-control-card"><small>Outline</small><span>Color</span><input type="color" value="${escapeHtml(settings.outlineColor || theme.ink || defaultConfig.theme.ink)}" data-ssx-effect-color="${escapeHtml(effect.id)}:outlineColor" ${disabled ? "disabled" : ""}></label>
            <label class="ssx-control-card"><small>Hover</small><span>Color</span><input type="color" value="${escapeHtml(settings.hoverColor || theme.accent || defaultConfig.theme.accent)}" data-ssx-effect-color="${escapeHtml(effect.id)}:hoverColor" ${disabled ? "disabled" : ""}></label>
          </div>
          <div class="ssx-range-row">
            <input type="range" min="0" max="100" value="${settings.intensity}" data-ssx-intensity="${escapeHtml(effect.id)}" ${disabled ? "disabled" : ""}>
            <div class="ssx-range-label">${escapeHtml(effect.intensityLabel || "Amount")} · ${settings.intensity}</div>
          ...
          <div class="ssx-range-row">
            <input type="range" min="0" max="100" value="${width}" data-ssx-param-effect="${escapeHtml(effect.id)}" data-ssx-param-key="width" ${disabled ? "disabled" : ""}>
            <div class="ssx-range-label">Width / padding · ${Math.round(12 + width * .42)}px</div>
          </div>
          <div class="ssx-range-row">
            <input type="range" min="0" max="100" value="${height}" data-ssx-param-effect="${escapeHtml(effect.id)}" data-ssx-param-key="height" ${disabled ? "disabled" : ""}>
            <div class="ssx-range-label">Height · ${Math.round(36 + height * .38)}px</div>
          </div>
          ${effect.id === "btn-style-swap-arrow" ? `
            <label class="ssx-control-line"><span>Hover text</span><input type="text" value="${escapeHtml(settings.customText || "")}" data-ssx-custom-text-effect="${escapeHtml(effect.id)}" maxlength="18" placeholder="Right Now" ${disabled ? "disabled" : ""}></label>
          ` : ""}
          <div class="ssx-range-row">
            <input type="range" min="0" max="100" value="${radius}" data-ssx-param-effect="${escapeHtml(effect.id)}" data-ssx-param-key="radius" ${disabled ? "disabled" : ""}>
            <div class="ssx-range-label">Corners · ${radius}</div>
          </div>
        </div>
        <button class="ssx-restore" type="button" data-ssx-restore-effect="${escapeHtml(effect.id)}">Restore</button>
      </div>
  ...
  function bindPanel(root) {
    ...
    root.querySelectorAll("[data-ssx-effect-color]").forEach((input) => {
      const updateEffectColor = () => {
        const [effectId, key] = input.dataset.ssxEffectColor.split(":");
        const effect = effectById.get(effectId);
      ...
      input.addEventListener("input", updateEffectColor);
      input.addEventListener("change", updateEffectColor);
    });
    ...
    root.querySelectorAll("[data-ssx-feature-effect]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (!current.enabled && feature.id === "site-cursor-suite" && siteCursorVisualIds.includes(effectId)) {
          siteCursorVisualIds.filter((id) => id !== effectId).forEach((id) => {
            const otherEffect = effectById.get(id);
            if (otherEffect) setEffectSetting(currentScopeTarget(otherEffect), id, { enabled: false, intensity: getEffectSettings(currentScopeTarget(otherEffect), id).intensity || 60 });
          });
        }
        if (!current.enabled && feature.id === "button-suite" && buttonStyleIds.includes(effectId)) {
          buttonStyleIds.concat(retiredButtonStyleIds).filter((id) => id !== effectId).forEach((id) => {
            const otherEffect = effectById.get(id);
            if (otherEffect) setEffectSetting(currentScopeTarget(otherEffect), id, { enabled: false, intensity: getEffectSettings(currentScopeTarget(otherEffect), id).intensity || 60 });
          });
        }
    ...
    root.querySelectorAll("[data-ssx-switch-effect]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (section && section.id === "cursor" && textCursorVisualIds.includes(effectId)) {
          textCursorVisualIds.filter((id) => id !== effectId).forEach((id) => {
            const otherEffect = effectById.get(id);
            if (otherEffect) setEffectSetting(currentScopeTarget(otherEffect), id, { enabled: false, intensity: getEffectSettings(currentScopeTarget(otherEffect), id).intensity || 60 });
          });
        }
        if (section && section.id === "image-cursor" && imageCursorVisualIds.includes(effectId)) {
          imageCursorVisualIds.filter((id) => id !== effectId).forEach((id) => {
            const otherEffect = effectById.get(id);
            if (otherEffect) setEffectSetting(currentScopeTarget(otherEffect), id, { enabled: false, intensity: getEffectSettings(currentScopeTarget(otherEffect), id).intensity || 60 });
          });
        }
        if (siteCursorVisualIds.includes(effectId)) {
          siteCursorVisualIds.filter((id) => id !== effectId).forEach((id) => {
            const otherEffect = effectById.get(id);
            if (otherEffect) setEffectSetting(currentScopeTarget(otherEffect), id, { enabled: false, intensity: getEffectSettings(currentScopeTarget(otherEffect), id).intensity || 60 });
          });
    ...
    root.querySelectorAll("[data-ssx-site-cursor-emoji]").forEach((button) => {
      button.addEventListener("click", () => {
        const effect = effectById.get("site-cursor-emoji");
        if (!effect) return;
        siteCursorVisualIds.filter((id) => id !== "site-cursor-emoji").forEach((id) => {
          const otherEffect = effectById.get(id);
          if (otherEffect) setEffectSetting(currentScopeTarget(otherEffect), id, { enabled: false, intensity: getEffectSettings(currentScopeTarget(otherEffect), id).intensity || 60 });
        });
        const selector = currentScopeTarget(effect);
        const current = getEffectSettings(selector, "site-cursor-emoji");
        const maxIndex = Math.max(1, cursorEmojiOptions.length - 1);
        setEffectSetting(selector, "site-cursor-emoji", {
          enabled: true,
          intensity: current.intensity || 60,
          params: { emoji: Math.round((Number(button.dataset.ssxSiteCursorEmoji) / maxIndex) * 100) },
    ...
    root.querySelectorAll("[data-ssx-restore-effect]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        const selector = currentScopeTarget(effect);
        setEffectSetting(selector, button.dataset.ssxRestoreEffect, { enabled: false, intensity: 60, params: {}, color: "", textColor: "", outlineColor: "", hoverColor: "", hoverTextColor: "", customText: "" });
        renderPanel();
    ...
    root.querySelectorAll("[data-ssx-intensity]").forEach((input) => {
      input.addEventListener("input", () => {
        ...
        if (valueNode && input.dataset.ssxAutoEnable === "true") {
          ...
          if (/^(txt|img|site)-cursor-(dot|ring|trail|large|emoji|glow|spotlight|crosshair|outline)$/.test(input.dataset.ssxIntensity || "")) valueNode.textContent = `${Math.round(24 + amount * .42)}px`;
        }
    ...
    root.querySelectorAll("[data-ssx-param-effect]").forEach((input) => {
      input.addEventListener("input", () => {
        ...
        const valueNode = input.parentElement && input.parentElement.querySelector("em");
        const amount = clampIntensity(input.value);
        const paramLabel = input.dataset.ssxParamKey === "outlineSize"
          ? `Follow / outline size · ${amount}`
            : input.dataset.ssxParamKey === "outlineWidth"
              ? `Outline width · ${Math.max(1, Math.round(1 + amount * .04))}px`
            : input.dataset.ssxParamKey === "width"
              ? `Width / padding · ${Math.round(12 + amount * .42)}px`
            : input.dataset.ssxParamKey === "height"
              ? `Height · ${Math.round(36 + amount * .38)}px`
            : input.dataset.ssxParamKey === "size"
              ? `Size · ${Math.round(76 + amount * .86)}px`
              : input.dataset.ssxParamKey === "textSize"
                ? `Text size · ${Math.round(11 + amount * .08)}px`
            : `${paramDef ? paramDef.label : input.dataset.ssxParamKey} · ${amount}`;
        if (valueNode) valueNode.textContent = `${amount}`;
        if (input.nextElementSibling) input.nextElementSibling.textContent = paramLabel;
      });
    });
    root.querySelectorAll("[data-ssx-custom-text-effect]").forEach((input) => {
      input.addEventListener("input", () => {
        ...
        const autoEnable = /^(txt|img|site)-cursor-emoji$/.test(input.dataset.ssxCustomTextEffect) || input.dataset.ssxCustomTextEffect === "btn-style-swap-arrow";
        if (input.dataset.ssxCustomTextEffect === "site-cursor-emoji") {
          siteCursorVisualIds.filter((id) => id !== "site-cursor-emoji").forEach((id) => {
            const otherEffect = effectById.get(id);
            if (otherEffect) setEffectSetting(currentScopeTarget(otherEffect), id, { enabled: false, intensity: getEffectSettings(currentScopeTarget(otherEffect), id).intensity || 60 });
          });
        ...
        setEffectSetting(selector, input.dataset.ssxCustomTextEffect, {
          enabled: autoEnable ? true : current.enabled,
          intensity: current.intensity,
          params: current.params,
  ...
  function cleanRuntimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/studio-poema-v79.js?v=${defaultConfig.version}`;
    try {
      const url = new URL(SCRIPT_SRC || fallback, location.href);
      url.searchParams.delete("ssx_editor");
      url.searchParams.delete("ssx");
      url.searchParams.delete("ssx_key");
      url.searchParams.delete("poema");
  ...
  function buildCompiledCss() {
    ...
    Object.entries(config.effects).forEach(([selector, effects]) => {
      Object.entries(effects || {}).forEach(([id, settings]) => {
        const effect = effectById.get(id);
        if (effect && settings.enabled && !retiredButtonStyleIds.includes(id)) css += `/* ${id} | ${selector} | intensity ${settings.intensity} */\n${effect.css(selector, intensity01(settings.intensity), settings, theme)}\n\n`;
      });
    });
    return css.trim();
  }
