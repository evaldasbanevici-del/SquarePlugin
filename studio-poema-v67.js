(() => {
  ...
  const STORAGE_KEY = `${APP_ID}:config:v67`;
  ...
  const buttonStyleEffects = [
    {
      ...
      css: (s, i, settings, theme) => {
        ...
        const lift = Math.round(1 + i * 5);
        const stretch = (1.02 + i * .08).toFixed(3);
        const squash = (0.985 - i * .035).toFixed(3);
        const speed = Math.max(.34, .72 - i * .2).toFixed(2);
        const iconSize = Math.round(30 + i * 8);
        const svgSize = Math.round(iconSize * .54);
        const hardTarget = ".ssx-btn-bouncy-cta";
        return `${buttonAdapterBaseCss(s, className, text, text)} ${buttonAdapterStableSizeCss(s, className, settings)} ${target},${hardTarget}{position:relative!important;overflow:hidden!important;gap:12px!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(outline,.36)}!important;border-radius:${radius}px!important;font-weight:900!important;box-shadow:0 10px 24px ${hexToRgba(fill,.16)},inset 0 1px 0 ${hexToRgba("#ffffff",.38)}!important;transition:background .18s ease,box-shadow .18s ease!important;transform-origin:center!important;will-change:transform!important;width:fit-content!important;max-width:100%!important;min-width:auto!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;} ${target}:before,${hardTarget}:before{content:"";position:absolute;inset:-30%;background:radial-gradient(circle at 50% 120%,${hexToRgba("#ffffff",.32)},transparent 46%);opacity:.55;transform:translateY(24%);transition:transform .22s ease,opacity .22s ease;z-index:0;pointer-events:none;} ${target} .ssx-btn-label,${hardTarget} .ssx-btn-label{position:relative!important;z-index:2!important;white-space:nowrap!important;display:inline-block!important;line-height:1!important;} ${target} .ssx-btn-circle-icon,${hardTarget} .ssx-btn-circle-icon{position:relative!important;z-index:2!important;width:${iconSize}px!important;height:${iconSize}px!important;min-width:${iconSize}px!important;max-width:${iconSize}px!important;min-height:${iconSize}px!important;max-height:${iconSize}px!important;flex:0 0 ${iconSize}px!important;border-radius:999px!important;display:inline-grid!important;place-items:center!important;color:${settings.outlineColor || theme.ink}!important;background:${hover}!important;box-shadow:inset 0 0 0 1px ${hexToRgba(settings.outlineColor || theme.ink,.18)}!important;transition:transform .24s cubic-bezier(.22,1.55,.36,1),background .18s ease,color .18s ease!important;overflow:hidden!important;line-height:0!important;} ${target} .ssx-btn-circle-icon svg,${hardTarget} .ssx-btn-circle-icon svg{width:${svgSize}px!important;height:${svgSize}px!important;min-width:${svgSize}px!important;max-width:${svgSize}px!important;min-height:${svgSize}px!important;max-height:${svgSize}px!important;display:block!important;fill:currentColor!important;transition:transform .28s cubic-bezier(.22,1.55,.36,1)!important;position:static!important;transform:none;} ${target}:hover,${hardTarget}:hover{background:${hover}!important;animation:ssxBtnBouncyCta ${speed}s cubic-bezier(.22,1.55,.36,1) both!important;box-shadow:0 ${Math.round(12+i*12)}px ${Math.round(26+i*22)}px ${hexToRgba(hover,.2)},inset 0 1px 0 ${hexToRgba("#ffffff",.46)}!important;} ${target}:hover:before,${hardTarget}:hover:before{transform:translateY(0);opacity:.9;} ${target}:hover .ssx-btn-circle-icon,${hardTarget}:hover .ssx-btn-circle-icon{background:${text}!important;color:${fill}!important;transform:translateX(${Math.round(2+i*5)}px) rotate(45deg) scale(1.04);} ${target}:hover .ssx-btn-circle-icon svg,${hardTarget}:hover .ssx-btn-circle-icon svg{transform:translateX(1px);} ${target}:active,${hardTarget}:active{transform:scale(.96)!important;} @keyframes ssxBtnBouncyCta{0%{transform:translateY(0) scale(1)}35%{transform:translateY(-${lift}px) scale(${stretch})}58%{transform:translateY(1px) scale(${squash})}78%{transform:translateY(-1px) scale(1.018)}100%{transform:translateY(0) scale(1)}}`;
      }
    },
    {
...
`;
      ...
      <div class="ssx-panel" role="dialog" aria-label="Studio Poema">
        <div class="ssx-head" data-ssx-drag>
          ...
          <div>
            ...
            <div class="ssx-subtitle">v67 · button icon clamp</div>
          </div>
  ...
  function cleanRuntimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/studio-poema-v67.js?v=${defaultConfig.version}`;
    try {
