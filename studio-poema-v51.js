(() => {
  ...
  const STORAGE_KEY = `${APP_ID}:config:v51`;
  ...
  const imageEffectExtras = [
    {
      ...
      intensityLabel: "Depth",
      preview: "image-shadow",
      css: (s, i, _settings, theme) => `${mediaSurfaces(s)}{box-shadow:0 ${Math.round(14+i*34)}px ${Math.round(28+i*70)}px ${hexToRgba(theme.primary,.12+i*.22)}!important;overflow:visible!important;}`
    },
    {
      ...
      css: (s, i, settings, theme) => {
        ...
        return `${mediaSurfaces(s)}{box-shadow:${offset}px ${offset}px 0 ${theme.primary}!important;overflow:visible!important;}`;
      }
    },
    {
      id: "img-style-border",
    ...
    {
      id: "img-shape-organic-1",
      label: "Animated image shapes · Organic 1",
      description: "Morphs the image into a soft organic blob while preserving its size.",
      intensityLabel: "Speed",
      preview: "shape-glass",
      css: (s, i) => `${mediaSurfaces(s)}{overflow:hidden!important;clip-path:polygon(8% 4%,92% 0%,100% 35%,93% 94%,44% 100%,0% 88%,3% 34%)!important;animation:ssxOrganicOne ${Math.max(4,12-i*7).toFixed(1)}s ease-in-out infinite alternate!important;}@keyframes ssxOrganicOne{50%{clip-path:polygon(2% 16%,84% 4%,98% 48%,88% 100%,34% 92%,7% 78%,0% 30%)}}`
    },
    {
      id: "img-shape-organic-2",
      label: "Animated image shapes · Organic 2",
      description: "Creates a rounded flowing image silhouette.",
      intensityLabel: "Speed",
      preview: "shape-glass",
      css: (s, i) => `${mediaSurfaces(s)}{overflow:hidden!important;border-radius:42% 58% 48% 52% / 46% 38% 62% 54%!important;animation:ssxOrganicTwo ${Math.max(4,13-i*7).toFixed(1)}s ease-in-out infinite alternate!important;}@keyframes ssxOrganicTwo{50%{border-radius:58% 42% 62% 38% / 38% 58% 42% 62%}}`
    },
    {
      id: "img-shape-organic-3",
      label: "Animated image shapes · Organic 3",
      description: "Adds a subtle asymmetric editorial crop.",
      intensityLabel: "Speed",
      preview: "shape-glass",
      css: (s, i) => `${mediaSurfaces(s)}{overflow:hidden!important;clip-path:ellipse(47% 50% at 51% 50%)!important;animation:ssxOrganicThree ${Math.max(4,12-i*7).toFixed(1)}s ease-in-out infinite alternate!important;}@keyframes ssxOrganicThree{50%{clip-path:ellipse(55% 43% at 46% 54%)}}`
    },
    {
      id: "img-shape-organic-4",
      label: "Animated image shapes · Organic 4",
      description: "Uses a premium pebble-like image mask.",
      intensityLabel: "Speed",
      preview: "shape-glass",
      css: (s, i) => `${mediaSurfaces(s)}{overflow:hidden!important;border-radius:64% 36% 48% 52% / 38% 52% 48% 62%!important;animation:ssxOrganicFour ${Math.max(4,14-i*7).toFixed(1)}s ease-in-out infinite alternate!important;}@keyframes ssxOrganicFour{50%{border-radius:38% 62% 36% 64% / 56% 34% 66% 44%}}`
    },
    {
      id: "img-shape-organic-5",
      label: "Animated image shapes · Organic 5",
      description: "Creates a soft liquid card shape for product and editorial images.",
      intensityLabel: "Speed",
      preview: "shape-glass",
      css: (s, i) => `${mediaSurfaces(s)}{overflow:hidden!important;clip-path:polygon(15% 0%,82% 4%,100% 22%,94% 82%,72% 100%,15% 94%,0% 70%,3% 18%)!important;animation:ssxOrganicFive ${Math.max(4,13-i*7).toFixed(1)}s ease-in-out infinite alternate!important;}@keyframes ssxOrganicFive{50%{clip-path:polygon(8% 12%,72% 0%,96% 28%,100% 72%,78% 94%,22% 100%,0% 78%,4% 24%)}}`
    },
    {
  ...
  const textSections = [
    ...
    { id: "cursor", icon: "☝", title: "Cursor on hover", description: "Custom cursor options for selected text.", options: ["txt-cursor-dot", "txt-cursor-ring", "txt-cursor-trail", "txt-cursor-emoji", "txt-cursor-large", "txt-cursor-hide"] }
  ];
  const imageSections = [
    ...
    { id: "image-shapes", icon: "▱", title: "Animated image shapes", description: "Organic moving image masks and orbiting frames that keep the image size.", options: ["img-shape-square-orbit", "img-shape-circle-orbit", "img-shape-organic-1", "img-shape-organic-2", "img-shape-organic-3", "img-shape-organic-4", "img-shape-organic-5"] },
...
`;
  ...
  function setupCursorFollower(theme) {
    ...
    cursorMoveHandler = (event) => {
      ...
      const sizeSource = trail || ring || dot || large || emoji || matched;
      ...
      <div class="ssx-panel" role="dialog" aria-label="Studio Poema">
        <div class="ssx-head" data-ssx-drag>
          ...
          <div>
            ...
            <div class="ssx-subtitle">v51 · word loop</div>
          </div>
  ...
  function renderCursorControls() {
    ...
    const dot = blockSetting("txt-cursor-dot");
    const ring = blockSetting("txt-cursor-ring");
    const trail = blockSetting("txt-cursor-trail");
    const large = blockSetting("txt-cursor-large");
    const hide = blockSetting("txt-cursor-hide");
    const activeVisual = trail.enabled ? trail : ring.enabled ? ring : dot.enabled ? dot : large.enabled ? large : emoji.enabled ? emoji : dot;
    ...
    return `
      <button class="ssx-control-line" type="button" data-ssx-switch-effect="txt-cursor-dot"><span>Soft dot cursor</span><em>${Math.round(Math.max(10, (24 + sizeValue * .42) * .42))}px</em><b class="ssx-toggle-dot ${dot.enabled ? "is-on" : ""}"></b></button>
      <button class="ssx-control-line" type="button" data-ssx-switch-effect="txt-cursor-ring"><span>Ring cursor</span><em>${Math.round(24 + sizeValue * .42)}px</em><b class="ssx-toggle-dot ${ring.enabled ? "is-on" : ""}"></b></button>
      <button class="ssx-control-line" type="button" data-ssx-switch-effect="txt-cursor-trail"><span>Dot + follower</span><em>${Math.round(24 + sizeValue * .42)}px</em><b class="ssx-toggle-dot ${trail.enabled ? "is-on" : ""}"></b></button>
      ...
      ${dot.enabled || ring.enabled || trail.enabled || large.enabled ? `<label class="ssx-control-line"><span>Cursor color</span><input type="color" value="${escapeHtml(theme.accent || defaultConfig.theme.accent)}" data-ssx-color="accent"></label>` : ""}
      <label class="ssx-control-line"><span>Cursor size</span><em>${Math.round(24 + sizeValue * .42)}px</em><input type="range" min="0" max="100" value="${sizeValue}" data-ssx-intensity="${trail.enabled ? "txt-cursor-trail" : ring.enabled ? "txt-cursor-ring" : dot.enabled ? "txt-cursor-dot" : large.enabled ? "txt-cursor-large" : "txt-cursor-emoji"}" data-ssx-auto-enable="true"></label>
  ...
  function renderTextControls(section, effect, settings, disabled) {
    ...
    if (section.id === "cursor") {
      const emoji = blockSetting("txt-cursor-emoji");
      const selectedCursor = /trail/i.test(effect.id) ? "Dot + follower" : /ring/i.test(effect.id) ? "Ring" : /dot/i.test(effect.id) ? "Soft dot" : /large/i.test(effect.id) ? "Large dot" : /emoji/i.test(effect.id) ? "Emoji" : "Hidden";
      const selectedEmoji = Math.max(0, Math.min(cursorEmojiOptions.length - 1, Math.round(param(emoji, "emoji", 0) / (100 / Math.max(1, cursorEmojiOptions.length - 1)))));
      return `
  ...
  function bindPanel(root) {
    ...
    root.querySelectorAll("[data-ssx-switch-effect]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (section && section.id === "cursor" && /^txt-cursor-(dot|ring|trail|emoji|large)$/.test(effectId)) {
          ["txt-cursor-dot", "txt-cursor-ring", "txt-cursor-trail", "txt-cursor-emoji", "txt-cursor-large"].filter((id) => id !== effectId).forEach((id) => {
        ...
        if (section && section.id === "image-cursor" && (effectId === "img-cursor-emoji" || effectId === "img-cursor-large")) {
    ...
    root.querySelectorAll("[data-ssx-intensity]").forEach((input) => {
      input.addEventListener("input", () => {
        ...
        if (valueNode && input.dataset.ssxAutoEnable === "true") {
          ...
          if (/^txt-cursor-(dot|ring|trail|large|emoji)$/.test(input.dataset.ssxIntensity || "")) valueNode.textContent = `${Math.round(24 + amount * .42)}px`;
        }
  ...
  function cleanRuntimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/studio-poema-v51.js?v=${defaultConfig.version}`;
    try {
