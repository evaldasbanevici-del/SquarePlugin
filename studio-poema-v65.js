(() => {
  ...
  const STORAGE_KEY = `${APP_ID}:config:v65`;
  ...
  const buttonStyleEffects = [
    {
      ...
      css: (s, i, settings, theme) => {
        ...
        return `${buttonAdapterBaseCss(s, className, text, text)} ${buttonAdapterStableSizeCss(s, className, settings)} ${target}{position:relative!important;overflow:hidden!important;gap:12px!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(outline,.36)}!important;border-radius:${radius}px!important;font-weight:900!important;box-shadow:0 10px 24px ${hexToRgba(fill,.16)},inset 0 1px 0 ${hexToRgba("#ffffff",.38)}!important;transition:background .18s ease,box-shadow .18s ease!important;transform-origin:center!important;will-change:transform!important;} ${target}:before{content:"";position:absolute;inset:-30%;background:radial-gradient(circle at 50% 120%,${hexToRgba("#ffffff",.32)},transparent 46%);opacity:.55;transform:translateY(24%);transition:transform .22s ease,opacity .22s ease;z-index:0;pointer-events:none;} ${target} .ssx-btn-label{position:relative!important;z-index:2!important;white-space:nowrap!important;} ${target} .ssx-btn-circle-icon{position:relative!important;z-index:2!important;width:${Math.round(30+i*12)}px!important;height:${Math.round(30+i*12)}px!important;min-width:${Math.round(30+i*12)}px!important;border-radius:999px!important;display:grid!important;place-items:center!important;color:${settings.outlineColor || theme.ink}!important;background:${hover}!important;box-shadow:inset 0 0 0 1px ${hexToRgba(settings.outlineColor || theme.ink,.18)}!important;transition:transform .24s cubic-bezier(.22,1.55,.36,1),background .18s ease,color .18s ease!important;overflow:hidden!important;} ${target} .ssx-btn-circle-icon svg{width:100%!important;height:100%!important;display:block!important;transition:transform .28s cubic-bezier(.22,1.55,.36,1)!important;} ${target}:hover{background:${hover}!important;animation:ssxBtnBouncyCta ${speed}s cubic-bezier(.22,1.55,.36,1) both!important;box-shadow:0 ${Math.round(12+i*12)}px ${Math.round(26+i*22)}px ${hexToRgba(hover,.2)},inset 0 1px 0 ${hexToRgba("#ffffff",.46)}!important;} ${target}:hover:before{transform:translateY(0);opacity:.9;} ${target}:hover .ssx-btn-circle-icon{background:${text}!important;color:${fill}!important;transform:translateX(${Math.round(2+i*6)}px) rotate(45deg) scale(1.04);} ${target}:hover .ssx-btn-circle-icon svg{transform:translateX(1px);} ${target}:active{transform:scale(.96)!important;} @keyframes ssxBtnBouncyCta{0%{transform:translateY(0) scale(1)}35%{transform:translateY(-${lift}px) scale(${stretch})}58%{transform:translateY(1px) scale(${squash})}78%{transform:translateY(-1px) scale(1.018)}100%{transform:translateY(0) scale(1)}}`;
      }
    },
    {
...
`;
      ...
      <div class="ssx-panel" role="dialog" aria-label="Studio Poema">
        <div class="ssx-head" data-ssx-drag>
          ...
          <div>
            ...
            <div class="ssx-subtitle">v65 · bouncy arrow button</div>
          </div>
  ...
  function cleanRuntimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/studio-poema-v65.js?v=${defaultConfig.version}`;
    try {
