(() => {
  ...
  const STORAGE_KEY = `${APP_ID}:config:v74`;
  ...
  const buttonStyleEffects = [
    {
      ...
      css: (s, i, settings, theme) => {
        ...
        const radius = Math.round(26 + param(settings, "radius", 70) * .58);
        const speed = Math.round(340 + (1 - i) * 360);
        const height = Math.round(38 + param(settings, "height", 44) * .38);
        const pad = Math.round(12 + param(settings, "width", 50) * .22);
        const iconSize = Math.max(30, Math.round(height * .82));
        const textBox = `max(80px, calc(var(--ssx-btn-width, 160px) - ${iconSize + 8}px))`;
        return `${buttonAdapterBaseCss(s, className, text, text)}
          ${target}{
            --ssx-btn-height:${height}px;
            --ssx-btn-icon:${iconSize}px;
            min-height:${height}px!important;
            width:var(--ssx-btn-width, auto)!important;
          ...
          ${target},.sqs-block-newsletter ${target}{background:none!important;border:none!important;padding:0!important;}
          ${target}:hover{opacity:1!important;transform:none!important;}
          ${target} .ssx-spbtn-box{position:relative!important;display:flex!important;align-items:center!important;justify-content:flex-start!important;width:100%!important;height:${height}px!important;isolation:isolate!important;color:${text}!important;}
          ${target} .ssx-spbtn-main{
            position:relative!important;
            z-index:2!important;
            display:flex!important;
            align-items:center!important;
            justify-content:center!important;
            width:${textBox}!important;
            min-width:${textBox}!important;
          ...
          ${target} .ssx-spbtn-left,${target} .ssx-spbtn-right{
            position:relative!important;
            z-index:1!important;
          ...
          ${target} .ssx-spbtn-left{
            position:absolute!important;
            left:0!important;
            top:50%!important;
            transform:translateY(-50%) scale(1,.3) translateX(2px)!important;
            border-radius:34%!important;
            z-index:1!important;
          }
          ${target} .ssx-spbtn-right{margin-left:0!important;transform:scale(1)!important;}
          ${target} .ssx-spbtn-arrow{display:grid!important;place-items:center!important;width:100%!important;height:100%!important;font-size:${Math.max(18, Math.round(iconSize * .54))}px!important;font-weight:900!important;line-height:1!important;transition:transform ${speed}ms cubic-bezier(.11,0,.5,0)!important;}
          ${target} .ssx-spbtn-left .ssx-spbtn-arrow{transform:translate(-20%,10%) rotate(0deg)!important;}
          ${target} .ssx-spbtn-right .ssx-spbtn-arrow{transform:translate(0,0) rotate(0deg)!important;}
          ${target}:hover .ssx-spbtn-main{transform:translateX(${iconSize}px)!important;background:${fill}!important;}
          ${target}:hover .ssx-spbtn-left{transform:translateY(-50%) scale(1)!important;border-radius:999px!important;z-index:3!important;}
          ${target}:hover .ssx-spbtn-right{transform:scale(1,.3) translateX(-2px)!important;border-radius:34%!important;z-index:0!important;}
          ${target}:hover .ssx-spbtn-left .ssx-spbtn-arrow{transform:translate(0,0) rotate(0deg)!important;}
          ${target}:hover .ssx-spbtn-right .ssx-spbtn-arrow{transform:translate(20%,-10%) rotate(0deg)!important;}
          ${target}:active .ssx-spbtn-box{transform:scale(.985)!important;}`;
      }
    },
    {
  ...
  function isDesignerMode() {
    ...
    const editorValue = params.get("ssx_editor") || params.get("ssx") || params.get("poema") || topParams.get("ssx_editor") || topParams.get("ssx") || topParams.get("poema") || "";
    const keyValue = params.get("ssx_key") || topParams.get("ssx_key") || "";
    const forced = editorValue === EDITOR_TOKEN || keyValue === EDITOR_TOKEN || (editorValue === "1" && keyValue === EDITOR_TOKEN);
  ...
  function setupButtonStyleAdapters() {
    ...
    active.forEach(({ effectId, className, settings }, button) => {
      ...
      if (button.dataset.ssxButtonAdapter === effectId && button.dataset.ssxButtonSignature === signature) {
        syncButtonAdapterMetrics(button);
        return;
      }
  ...
  function syncButtonAdapterMetrics(button) {
    if (!button) return;
...
`;
      ...
      <div class="ssx-panel" role="dialog" aria-label="Studio Poema">
        <div class="ssx-head" data-ssx-drag>
          ...
          <div>
            ...
            <div class="ssx-subtitle">v74 · private editor mode</div>
          </div>
  ...
  function cleanRuntimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/studio-poema-v74.js?v=${defaultConfig.version}`;
    try {
