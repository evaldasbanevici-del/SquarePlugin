(() => {
  ...
  const STORAGE_KEY = `${APP_ID}:config:v57`;
  ...
  const buttonStyleEffects = [
    ...
    },
    {
      ...
      css: (s, i, settings, theme) => {
        ...
        return `${buttonTargets(s)}{background:${fill}!important;color:${settings.textColor || theme.ink}!important;border:2px solid ${outline}!important;border-radius:${Math.round(6+param(settings,"radius",35)*.34)}px!important;box-shadow:${depth}px ${depth}px 0 ${outline}!important;transform:translate(0,0)!important;transition:transform .15s ease,box-shadow .15s ease!important;} ${buttonTargets(s)}:hover{transform:translate(${Math.round(depth*.45)}px,${Math.round(depth*.45)}px)!important;box-shadow:${Math.round(depth*.35)}px ${Math.round(depth*.35)}px 0 ${outline}!important;}`;
      }
    },
    {
    ...
    },
    {
      ...
      css: (s, i, settings, theme) => {
        ...
        return `${buttonTargets(s)}{position:relative!important;overflow:hidden!important;isolation:isolate!important;background:${fill}!important;color:${settings.textColor || theme.ink}!important;border:1px solid ${settings.outlineColor || theme.primary}!important;border-radius:${Math.round(8+param(settings,"radius",36)*.42)}px!important;transition:color .22s ease,transform .22s ease!important;} ${buttonTargets(s)}:before,${buttonTargets(s)}:after{content:"";position:absolute;top:0;width:51%;height:100%;background:${hover};z-index:-1;transition:transform ${Math.max(.28,.75-i*.32).toFixed(2)}s ease;} ${buttonTargets(s)}:before{left:0;transform:translateX(-105%);} ${buttonTargets(s)}:after{right:0;transform:translateX(105%);} ${buttonTargets(s)}:hover{color:${settings.hoverTextColor || theme.soft}!important;transform:translateY(-${Math.round(i*3)}px)!important;} ${buttonTargets(s)}:hover:before,${buttonTargets(s)}:hover:after{transform:translateX(0);}`;
      }
    },
    {
    ...
    },
    {
      ...
      css: (s, i, settings, theme) => {
        ...
        return `${buttonTargets(s)}{position:relative!important;isolation:isolate!important;overflow:visible!important;width:${size}px!important;min-width:${size}px!important;height:${size}px!important;min-height:${size}px!important;padding:0!important;display:inline-grid!important;place-items:center!important;border-radius:999px!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(outline,.38)}!important;box-shadow:0 ${Math.round(12+i*26)}px ${Math.round(28+i*54)}px ${hexToRgba(fill,.22)}!important;${textRule}} ${buttonTargets(s)}:before{content:"";position:absolute;inset:-9px;border-radius:999px;border:2px dashed ${outline};animation:ssxBtnCircleSpin ${speed}s linear infinite;pointer-events:none;} ${buttonTargets(s)}:after{content:${afterContent};position:absolute;inset:0;display:grid;place-items:center;color:${text};font-size:${Math.round(11+param(settings,"textSize",46)*.08)}px;font-weight:900;letter-spacing:0;text-align:center;line-height:1.05;padding:12px;z-index:1;pointer-events:none;} ${buttonTargets(s)}:hover:before{animation-duration:${Math.max(1.2, Number(speed)*.45).toFixed(1)}s;} @keyframes ssxBtnCircleSpin{to{transform:rotate(360deg)}}`;
      }
    },
    {
...
`;
      ...
      <div class="ssx-panel" role="dialog" aria-label="Studio Poema">
        <div class="ssx-head" data-ssx-drag>
          ...
          <div>
            ...
            <div class="ssx-subtitle">v57 · button styles</div>
          </div>
  ...
  function bindPanel(root) {
    ...
    root.querySelectorAll("[data-ssx-custom-text-effect]").forEach((input) => {
      input.addEventListener("input", () => {
        ...
        const autoEnable = /^(txt|img|site)-cursor-emoji$/.test(input.dataset.ssxCustomTextEffect);
  ...
  function cleanRuntimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/studio-poema-v57.js?v=${defaultConfig.version}`;
    try {
