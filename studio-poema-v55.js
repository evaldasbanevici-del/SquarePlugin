(() => {
  ...
  const STORAGE_KEY = `${APP_ID}:config:v55`;
  ...
  const allEffects = effectCatalog.flatMap((group) => group.items.map((effect) => ({ ...effect, group: group.group }))).concat(textEffectExtras.map((effect) => ({ ...effect, group: "Text" })), imageEffectExtras.map((effect) => ({ ...effect, group: "Image" })), siteCursorEffects.map((effect) => ({ ...effect, group: "Cursor" })));
...
`;
  ...
  function setupCursorFollower(theme) {
    ...
    cursorMoveHandler = (event) => {
      ...
      if (emoji) {
        ...
        cursor.textContent = "";
        cursor.style.background = hexToRgba(theme.accent, .18);
        cursor.style.border = `1px solid ${hexToRgba(theme.primary, .34)}`;
        cursor.style.boxShadow = `0 14px 42px ${hexToRgba(theme.primary, .18)}, inset 0 0 0 1px ${hexToRgba(theme.soft, .45)}`;
        ...
        cursor.style.border = `1px solid ${hexToRgba(theme.ink, .32)}`;
        cursor.style.boxShadow = `0 0 0 ${Math.round(size * .28)}px ${hexToRgba(theme.accent, .24)}, 0 0 ${Math.round(size * .9)}px ${hexToRgba(theme.primary, .38)}`;
        cursorTrail.style.display = "none";
      } else if (crosshair) {
        cursor.textContent = "+";
        cursor.style.background = "transparent";
        cursor.style.border = `1px solid ${hexToRgba(theme.primary, .55)}`;
        cursor.style.color = theme.primary;
        cursor.style.fontSize = `${Math.round(size * .78)}px`;
        cursor.style.boxShadow = `0 0 0 1px ${hexToRgba(theme.soft, .86)}, 0 0 0 6px ${hexToRgba(theme.accent, .12)}`;
        cursorTrail.style.display = "none";
      } else if (outline) {
        cursor.textContent = "";
        cursor.style.background = "transparent";
        cursor.style.border = `2px solid ${theme.primary}`;
        cursor.style.boxShadow = `inset 0 0 0 2px ${hexToRgba(theme.soft, .72)}, 0 0 0 7px ${hexToRgba(theme.accent, .12)}`;
        cursorTrail.style.display = "none";
      } else if (trail) {
        ...
        cursor.textContent = "";
        cursor.style.background = theme.accent;
        cursor.style.border = `1px solid ${hexToRgba(theme.ink, .42)}`;
        cursor.style.boxShadow = `0 8px 18px ${hexToRgba(theme.primary, .22)}`;
        cursorTrail.style.display = "block";
        cursorTrail.style.left = `${event.clientX}px`;
        cursorTrail.style.top = `${event.clientY}px`;
        cursorTrail.style.width = `${Math.round(size * 1.15)}px`;
        cursorTrail.style.height = `${Math.round(size * 1.15)}px`;
        cursorTrail.style.background = hexToRgba(theme.accent, .08);
        cursorTrail.style.border = `1.5px solid ${hexToRgba(theme.primary, .48)}`;
        cursorTrail.style.opacity = ".95";
      } else if (ring) {
        cursor.textContent = "";
        cursor.style.background = hexToRgba(theme.soft, .08);
        cursor.style.border = `2px solid ${theme.primary}`;
        cursor.style.boxShadow = `0 0 0 6px ${hexToRgba(theme.accent, .16)}`;
        cursorTrail.style.display = "none";
      } else if (dot) {
        ...
        cursor.textContent = "";
        cursor.style.background = theme.accent;
        cursor.style.border = `1px solid ${hexToRgba(theme.ink, .38)}`;
        cursor.style.boxShadow = `0 10px 24px ${hexToRgba(theme.primary, .20)}`;
        cursorTrail.style.display = "none";
      } else if (large) {
        cursor.textContent = "";
        cursor.style.background = theme.accent;
      ...
      <div class="ssx-panel" role="dialog" aria-label="Studio Poema">
        <div class="ssx-head" data-ssx-drag>
          ...
          <div>
            ...
            <div class="ssx-subtitle">v55 · website cursor</div>
          </div>
  ...
  function renderFeatureOption(feature, effect) {
    ...
    return `
      <article class="ssx-option-card" data-ssx-preview-title="${escapeHtml(effect.label)}" data-ssx-preview-desc="${escapeHtml(effect.description)}">
        ...
        ${isSiteCursor ? renderSiteCursorFeatureControls(effect, settings, disabledGlobalMotion) : `
    ...
    return `
      <div class="ssx-option-controls">
        ...
        <div>
          ...
          ` : `
            <label class="ssx-control-line"><span>Cursor color</span><input type="color" value="${escapeHtml(theme.accent || defaultConfig.theme.accent)}" data-ssx-color="accent" ${disabled ? "disabled" : ""}></label>
          `}
          <div class="ssx-range-row">
            <input type="range" min="0" max="100" value="${settings.intensity}" data-ssx-intensity="${escapeHtml(effect.id)}" ${disabled ? "disabled" : ""}>
            <div class="ssx-range-label">Size · ${Math.round(24 + settings.intensity * .42)}px</div>
          </div>
        </div>
        <button class="ssx-restore" type="button" data-ssx-restore-effect="${escapeHtml(effect.id)}">Restore</button>
      </div>
  ...
  function cleanRuntimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/studio-poema-v55.js?v=${defaultConfig.version}`;
    try {
