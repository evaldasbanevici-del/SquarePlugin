/* ============================================================
   ...
   File: FormaPlugin-v6.js · Single-file injection · No dependencies
...
(() => {
  ...
  const MEDIA = (s) => `${s} img,${s} video,${s} .sqs-block-image img,${s} .image-block-wrapper img,${s} .thumb-image,${s} .section-background,${s} [style*="background-image"]`;
  const MEDIA_WRAP = (s) => `${s} .sqs-block-image,${s} .image-block-wrapper,${s} figure,${s} .fe-block,${s} .image-block-outer-wrapper,${s} .sqs-image,${s} .gallery-grid-item,${s} .gallery-masonry-item,${s} .list-item-media,${s} .section-background`;
  ...
  },
  {
    ...
    items: [
      { id: "kn-blur-words", label: "Blur reveal · words", desc: "Each word sharpens and lifts in sequence.", amount: "Distance", cls: "fm-reveal", split: "word",
        params: [{ k: "scope", label: "Apply to", type: "select", options: ["Headings", "All text"], v: "Headings" }, { k: "speed", label: "Speed", v: 50 }, { k: "st", label: "Stagger", v: 40 }, { k: "blur", label: "Blur", v: 55 }],
        css: (s, t, p) => { const d = (1.15 - p("speed", 50) * .009).toFixed(2); return `${s} .fm-kt .fm-u{opacity:0!important;filter:blur(${Math.round(2 + p("blur", 55) * .14)}px);transform:translateY(${Math.round(8 + t * 30)}px);transition:opacity ${d}s ease,filter ${d}s ease,transform ${d}s cubic-bezier(.2,.8,.2,1);transition-delay:calc(var(--i)*${Math.round(15 + p("st", 40) * 1.6)}ms)}${s}.fm-in .fm-kt .fm-u{opacity:1!important;filter:none!important;transform:none!important}`; } },
      { id: "kn-blur-chars", label: "Blur reveal · characters", desc: "Letter-by-letter blur reveal.", amount: "Distance", cls: "fm-reveal", split: "char",
        params: [{ k: "scope", label: "Apply to", type: "select", options: ["Headings", "All text"], v: "Headings" }, { k: "speed", label: "Speed", v: 50 }, { k: "st", label: "Stagger", v: 30 }, { k: "blur", label: "Blur", v: 55 }],
        css: (s, t, p) => { const d = (1 - p("speed", 50) * .008).toFixed(2); return `${s} .fm-kt .fm-u{opacity:0!important;filter:blur(${Math.round(2 + p("blur", 55) * .12)}px);transform:translateY(${Math.round(4 + t * 18)}px);transition:opacity ${d}s ease,filter ${d}s ease,transform ${d}s cubic-bezier(.2,.8,.2,1);transition-delay:calc(var(--i)*${Math.round(8 + p("st", 30) * .9)}ms)}${s}.fm-in .fm-kt .fm-u{opacity:1!important;filter:none!important;transform:none!important}`; } },
      { id: "kn-blur-lines", label: "Blur reveal · lines", desc: "Whole lines sharpen one after another.", amount: "Distance", cls: "fm-reveal", split: "line",
        params: [{ k: "scope", label: "Apply to", type: "select", options: ["Headings", "All text"], v: "Headings" }, { k: "speed", label: "Speed", v: 50 }, { k: "st", label: "Stagger", v: 55 }, { k: "blur", label: "Blur", v: 55 }],
        css: (s, t, p) => { const d = (1.2 - p("speed", 50) * .009).toFixed(2); return `${s} .fm-kt .fm-u{opacity:0!important;filter:blur(${Math.round(2 + p("blur", 55) * .14)}px);transform:translateY(${Math.round(10 + t * 26)}px);transition:opacity ${d}s ease,filter ${d}s ease,transform ${d}s cubic-bezier(.2,.8,.2,1);transition-delay:calc(var(--l,0)*${Math.round(60 + p("st", 55) * 3)}ms)}${s}.fm-in .fm-kt .fm-u{opacity:1!important;filter:none!important;transform:none!important}`; } },
      { id: "kn-slide-words", label: "Slide up · words", desc: "Words rise from a clipped baseline.", amount: "Distance", cls: "fm-reveal", split: "word",
        params: [{ k: "scope", label: "Apply to", type: "select", options: ["Headings", "All text"], v: "Headings" }, { k: "speed", label: "Speed", v: 50 }, { k: "st", label: "Stagger", v: 40 }],
        css: (s, t, p) => { const d = (1 - p("speed", 50) * .008).toFixed(2); return `${s} .fm-kt .fm-w{overflow:hidden;vertical-align:bottom}${s} .fm-kt .fm-u{opacity:0!important;transform:translateY(${Math.round(60 + t * 50)}%);transition:opacity ${d}s ease,transform ${d}s cubic-bezier(.2,.8,.2,1);transition-delay:calc(var(--i)*${Math.round(15 + p("st", 40) * 1.6)}ms)}${s}.fm-in .fm-kt .fm-u{opacity:1!important;transform:none!important}`; } },
      { id: "kn-flip-chars", label: "Flip in · characters", desc: "Letters flip in like tiles.", amount: "Flip", cls: "fm-reveal", split: "char",
        ...
        css: (s, t, p) => { const d = (.9 - p("speed", 50) * .006).toFixed(2); return `${s} .fm-kt{perspective:600px}${s} .fm-kt .fm-u{opacity:0!important;transform:rotateX(${Math.round(50 + t * 40)}deg) translateY(8px);transform-origin:bottom;transition:opacity ${d}s ease,transform ${d}s cubic-bezier(.2,.8,.2,1);transition-delay:calc(var(--i)*${Math.round(8 + p("st", 30) * .9)}ms)}${s}.fm-in .fm-kt .fm-u{opacity:1!important;transform:none!important}`; } },
      { id: "kn-pop-chars", label: "Pop in · characters", desc: "Springy letter-by-letter pop.", amount: "Pop", cls: "fm-reveal", split: "char",
        ...
        css: (s, t, p) => `${s} .fm-kt .fm-u{opacity:0!important;transform:scale(${(.3 - t * .25).toFixed(2)});transition:opacity .4s ease,transform .55s cubic-bezier(.2,${(1.4 + t * .9).toFixed(1)},.3,1);transition-delay:calc(var(--i)*${Math.round(8 + p("st", 30) * .9)}ms)}${s}.fm-in .fm-kt .fm-u{opacity:1!important;transform:none!important}` },
      { id: "kn-wave-loop", label: "Wave loop", desc: "Letters bob in a continuous wave.", amount: "Height", split: "char",
        ...
        css: (s, t, p) => `${s} .fm-kt .fm-u{animation:fmKWave ${(2.6 - p("speed", 45) * .016).toFixed(2)}s ease-in-out infinite;animation-delay:calc(var(--i)*-90ms)}@keyframes fmKWave{0%,100%{transform:translateY(0)}50%{transform:translateY(-${Math.round(3 + t * 12)}px)}}` },
      { id: "kn-shimmer-loop", label: "Shimmer loop", desc: "A soft pulse travels through the letters.", amount: "Depth", split: "char",
        params: [{ k: "speed", label: "Speed", v: 45 }],
        css: (s, t, p) => `${s} .fm-kt .fm-u{animation:fmKShim ${(3 - p("speed", 45) * .02).toFixed(2)}s ease-in-out infinite;animation-delay:calc(var(--i)*-70ms)}@keyframes fmKShim{0%,100%{opacity:1}50%{opacity:${(.85 - t * .55).toFixed(2)}}}` },
      { id: "kn-jiggle-loop", label: "Jiggle loop", desc: "Playful per-letter wobble.", amount: "Jiggle", split: "char",
        ...
        css: (s, t, p) => `${s} .fm-kt .fm-u{animation:fmKJig ${(1.6 - p("speed", 40) * .009).toFixed(2)}s ease-in-out infinite;animation-delay:calc(var(--i)*-120ms)}@keyframes fmKJig{0%,100%{transform:rotate(0)}25%{transform:rotate(-${(1 + t * 4).toFixed(1)}deg)}75%{transform:rotate(${(1 + t * 4).toFixed(1)}deg)}}` },
      { id: "kn-typewriter-loop", label: "Typewriter loop", desc: "Types itself, holds, and repeats forever.", amount: "Hold", split: "char",
        ...
        css: (s, t, p) => { const step = Math.round(30 + (100 - p("st", 45)) * 1.2); const holdUnits = Math.round(8 + t * 30); return `${s} .fm-kt{--fmstep:${step}ms}${s} .fm-kt .fm-u{opacity:0!important;animation:fmKType calc((var(--n,20) + ${holdUnits})*var(--fmstep)) steps(1,end) infinite;animation-delay:calc(var(--i)*var(--fmstep))}@keyframes fmKType{0%{opacity:0}2%{opacity:1}80%{opacity:1}82%{opacity:0}100%{opacity:0}}`; } },
      ...
      { id: "kn-scramble", label: "Scramble morph", desc: "Letters cycle glyphs, then settle into the text.", amount: "Speed", cls: "fm-reveal", split: "char", scramble: true,
        ...
        css: (s) => `${s} .fm-kt .fm-u{display:inline-block;min-width:.3ch}` }
    ]
  },
  {
    group: "Buttons", icon: "◉",
  ...
  /* motion & kinetic effects on "all pages" run per-section so each
     section animates as it scrolls into view */
  const motionSel = (e, sel) => (e && (e.cls || e.split) && (sel === "body" || sel === "html")) ? "[data-section-id]" : sel;
  ...
  function setFx(sel, id, patch) {
    ...
    if (effect.cls || effect.split) {
      try {
        document.querySelectorAll(motionSel(effect, sel)).forEach(el => {
          seen.delete(el);
  ...
  function compileCss() {
    ...
    Object.entries(state.config.effects).forEach(([sel, list]) => {
      Object.entries(list).forEach(([id, st]) => {
        ...
        if (!e || !st.on) return;
        css += `/* ${id} | ${sel} | ${st.i} */\n${e.css(motionSel(e, sel), clamp(st.i) / 100, paramGetter(st), th)}\n`;
      });
    });
    return css;
  }
  ...
  function runScramble(container, st) {
    ...
    const units = [...container.querySelectorAll(".fm-kt .fm-u")];
  ...
  function syncWordLoops() {
    ...
    Object.entries(state.config.effects).forEach(([sel, list]) => {
      ...
      if (!st || !st.on) return;
      const eff = sel === "body" ? "[data-section-id]" : sel;
      let containers = [];
      try { containers = [...document.querySelectorAll(eff)]; } catch { /* bad selector */ }
      containers.forEach(container => {
        if (container.closest(`#${IDS.root}`)) return;
        const h = container.querySelector("h1,h2,h3");
        if (h && !h.closest(`#${IDS.root}`)) {
          try { startWordLoop(h, st, th); }
          catch (err) { console.warn("Forma word-loop:", err); }
        }
      });
    });
  }
  ...
  function syncMotionClasses() {
    ...
    Object.entries(state.config.effects).forEach(([sel, list]) => {
      Object.entries(list).forEach(([id, st]) => {
        const e = EFFECTS.get(id);
        if (!e || !st.on || !e.cls) return;
        const eff = motionSel(e, sel);
        try {
          document.querySelectorAll(eff).forEach(el => {
    ...
    Object.entries(state.config.effects).forEach(([sel, list]) => {
      Object.entries(list).forEach(([id, st]) => {
        const e = EFFECTS.get(id);
        if (!e || !st.on || !e.split) return;
        const eff = motionSel(e, sel);
        if (e.scramble) scrambleJobs.push({ sel: eff, st });
        const tq = (st.p && st.p.scope === "All text") ? "h1,h2,h3,p,li,blockquote" : "h1,h2,h3";
        try {
          document.querySelectorAll(eff).forEach(container => {
            if (container.closest(`#${IDS.root}`)) return;
            container.querySelectorAll(tq).forEach(h => needSplit.set(h, e.split));
          });
        } catch { /* skip */ }
      });
    });
...
<div class="fm-fx" data-open="${expanded}">
  ...
  function diagReport() {
    ...
    const env = [
      ...
      ["Images (img tags)", cnt("img")],
      ["Images (background divs)", cnt('.section-background,[style*="background-image"]')],
...
<div class="fm-pub">
  ...
  function bindPanel(root) {
    root.addEventListener("click", ev => {
      ...
      if (act === "toggle") {
        ...
        if (!cur.on && sel !== "body") warnIfNoMatch(e, sel);
  ...
  /* ============================================================
     ...
     ============================================================ */
  function runtimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/FormaPlugin-v6.js`;
    try {
