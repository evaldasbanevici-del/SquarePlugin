(() => {
  ...
  const STORAGE_KEY = `${APP_ID}:config:v61`;
  ...
  const buttonStyleIds = [
    ...
    "btn-style-arrow-bubble",
    "btn-style-swap-arrow",
    "btn-style-glass-tilt",
    "btn-style-premium-glass",
    "btn-style-orbit-text",
    "btn-style-arrow-push",
    "btn-style-circle-360",
    "btn-style-soft-pill",
    "btn-style-sketch-border"
  ];
  ...
  function buttonClassTargets(s, className) {
    return `${s}:is(a,button,.sqs-button-element,.sqs-block-button-element).${className},${s} .sqs-button-element.${className},${s} a.sqs-block-button-element.${className},${s} .sqs-block-button a.${className},${s} button.${className}`;
  }
  ...
  function buttonTextTargets(s) {
    return `${s}:is(a,button,.sqs-button-element,.sqs-block-button-element) span,${s}:is(a,button,.sqs-button-element,.sqs-block-button-element) strong,${s}:is(a,button,.sqs-button-element,.sqs-block-button-element) em,${s}:is(a,button,.sqs-button-element,.sqs-block-button-element) .sqs-button-element--inner,${s} .sqs-button-element span,${s} .sqs-button-element strong,${s} .sqs-button-element em,${s} a.sqs-block-button-element span,${s} .sqs-block-button a span,${s} button span`;
  }

  function buttonHoverTextTargets(s) {
    return `${s}:is(a,button,.sqs-button-element,.sqs-block-button-element):hover span,${s}:is(a,button,.sqs-button-element,.sqs-block-button-element):hover strong,${s}:is(a,button,.sqs-button-element,.sqs-block-button-element):hover em,${s} .sqs-button-element:hover span,${s} a.sqs-block-button-element:hover span,${s} .sqs-block-button a:hover span,${s} button:hover span`;
  }

  function buttonBaseCss(s, text, hoverText) {
    return `${buttonTargets(s)}{display:inline-flex!important;align-items:center!important;justify-content:center!important;text-align:center!important;line-height:1.05!important;text-decoration:none!important;transform-origin:center!important;will-change:transform,box-shadow,background,color!important;} ${buttonTextTargets(s)}{position:relative!important;z-index:2!important;color:${text}!important;text-decoration:none!important;} ${buttonTargets(s)}:hover,${buttonHoverTextTargets(s)}{color:${hoverText || text}!important;}`;
  }
  ...
  function buttonStableSizeCss(s, settings) {
    ...
    return `${buttonTargets(s)}{box-sizing:border-box!important;min-height:${height}px!important;padding:${Math.max(8, Math.round(height * .28))}px ${width}px!important;vertical-align:middle!important;} ${buttonTargets(s)}:hover{transform:none!important;}`;
  }
  ...
  const buttonStyleEffects = [
    {
      ...
      label: "Letter flip",
      description: "Each letter slides out and returns on hover, using the current button text.",
      intensityLabel: "Flip",
      preview: "button-line",
      css: (s, i, settings, theme) => {
        ...
        const radius = Math.round(18 + param(settings, "radius", 70) * .28);
        const speed = Math.max(.18, .42 - i * .18).toFixed(2);
        let stagger = "";
        for (let n = 1; n <= 24; n += 1) stagger += `${target} .ssx-flip-top .ssx-btn-char:nth-child(${n}),${target} .ssx-flip-bottom .ssx-btn-char:nth-child(${n}){transition-duration:${(Number(speed) + n * .035).toFixed(3)}s!important;}`;
        return `${buttonAdapterBaseCss(s, className, text, text)} ${buttonAdapterStableSizeCss(s, className, settings)} ${target}{overflow:hidden!important;border:none!important;background:${fill}!important;color:${text}!important;border-radius:${radius}px!important;font-weight:800!important;letter-spacing:0!important;cursor:pointer!important;} ${target} .ssx-btn-text{display:flex!important;gap:0!important;line-height:1!important;white-space:nowrap!important;} ${target} .ssx-flip-bottom{position:absolute!important;inset:0!important;display:flex!important;align-items:center!important;justify-content:center!important;} ${target} .ssx-flip-bottom .ssx-btn-char{transform:translateY(-1.25em);} ${target}:hover .ssx-flip-top .ssx-btn-char{transform:translateY(1.25em);} ${target}:hover .ssx-flip-bottom .ssx-btn-char{transform:translateY(0);} ${target} .ssx-btn-char{display:inline-block!important;transition-property:transform!important;transition-timing-function:cubic-bezier(.22,1,.36,1)!important;will-change:transform!important;} ${stagger}`;
      }
    },
    {
      id: "btn-style-arrow-bubble",
      label: "Arrow bubble",
      description: "Dual arrows and a circular fill slide across the button on hover.",
      intensityLabel: "Motion",
      preview: "button-arrow",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-arrow-bubble";
        const target = buttonClassTargets(s, className);
        const fill = settings.color || theme.primary;
        const text = settings.textColor || theme.soft;
        const hover = settings.hoverColor || theme.accent;
        const hoverText = settings.hoverTextColor || theme.ink;
        const outline = settings.outlineColor || theme.ink;
        const radius = Math.round(10 + param(settings, "radius", 45) * .22);
        const speed = Math.max(.36, .82 - i * .32).toFixed(2);
        return `${buttonAdapterBaseCss(s, className, text, hoverText)} ${buttonAdapterStableSizeCss(s, className, settings)} ${target}{overflow:hidden!important;gap:8px!important;background:${fill}!important;border:2px solid ${outline}!important;border-radius:${radius}px!important;color:${text}!important;font-weight:800!important;transition:color ${speed}s ease,border-radius ${speed}s ease,background ${speed}s ease!important;} ${target} .ssx-btn-label{position:relative!important;z-index:3!important;transform:translateX(-6px);transition:transform ${speed}s cubic-bezier(.22,1,.36,1),color ${speed}s ease!important;white-space:nowrap!important;} ${target} .ssx-btn-icon{position:absolute!important;width:24px!important;height:24px!important;display:grid!important;place-items:center!important;z-index:4!important;transition:left ${speed}s cubic-bezier(.22,1,.36,1),right ${speed}s cubic-bezier(.22,1,.36,1),transform ${speed}s ease,color ${speed}s ease!important;color:${text}!important;} ${target} .ssx-btn-icon svg{width:22px!important;height:22px!important;display:block!important;fill:currentColor!important;} ${target} .ssx-btn-icon-left{left:-32px;} ${target} .ssx-btn-icon-right{right:18px;} ${target} .ssx-btn-fill{position:absolute!important;top:50%!important;left:50%!important;width:20px!important;height:20px!important;border-radius:999px!important;background:${hover}!important;opacity:0!important;transform:translate(-50%,-50%)!important;transition:width ${speed}s ease,height ${speed}s ease,opacity ${speed}s ease!important;z-index:1!important;pointer-events:none!important;} ${target}:hover{color:${hoverText}!important;border-radius:${Math.round(radius+14)}px!important;} ${target}:hover .ssx-btn-fill{width:230px!important;height:230px!important;opacity:1!important;} ${target}:hover .ssx-btn-label{transform:translateX(12px);} ${target}:hover .ssx-btn-icon-left{left:16px;color:${hoverText}!important;} ${target}:hover .ssx-btn-icon-right{right:-34px;color:${hoverText}!important;}`;
      }
    },
    {
      id: "btn-style-swap-arrow",
      label: "Swap arrow",
      description: "Primary label slides away and a second label rises in with a rotating circle arrow.",
      intensityLabel: "Swap",
      preview: "button-arrow",
      css: (s, i, settings, theme) => {
        ...
        const radius = Math.round(22 + param(settings, "radius", 70) * .28);
        const speed = Math.max(.18, .42 - i * .16).toFixed(2);
        return `${buttonAdapterBaseCss(s, className, text, text)} ${buttonAdapterStableSizeCss(s, className, settings)} ${target}{overflow:hidden!important;gap:10px!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(settings.outlineColor || theme.primary,.12)}!important;border-radius:${radius}px!important;font-weight:800!important;box-shadow:0 8px 20px ${hexToRgba(theme.primary,.10)}!important;transition:background .2s ease,box-shadow .2s ease,transform .2s ease!important;} ${target}:hover{background:${hover}!important;box-shadow:0 12px 28px ${hexToRgba(theme.primary,.16)}!important;transform:translateY(-1px)!important;} ${target} .ssx-btn-swap-wrap{position:relative!important;display:inline-block!important;overflow:hidden!important;line-height:1.1!important;min-width:86px!important;height:1.15em!important;z-index:2!important;} ${target} .ssx-btn-swap-top,${target} .ssx-btn-swap-bottom{display:block!important;transition:transform ${speed}s cubic-bezier(.22,1,.36,1)!important;white-space:nowrap!important;} ${target} .ssx-btn-swap-bottom{position:absolute!important;inset:0!important;transform:translateY(115%);} ${target}:hover .ssx-btn-swap-top{transform:translateY(-115%);} ${target}:hover .ssx-btn-swap-bottom{transform:translateY(0);} ${target} .ssx-btn-circle-icon{position:relative!important;z-index:2!important;width:24px!important;height:24px!important;display:grid!important;place-items:center!important;color:${text}!important;transition:transform .22s ease!important;} ${target} .ssx-btn-circle-icon svg{width:24px!important;height:24px!important;display:block!important;} ${target}:hover .ssx-btn-circle-icon{transform:rotate(45deg);}`;
      }
    },
    {
      id: "btn-style-glass-tilt",
      label: "Glass tilt",
      description: "Capsule glass button with idle shine, soft pulse and 3D hover lift.",
      intensityLabel: "Glow",
      preview: "button-glass",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-glass-tilt";
        const target = buttonClassTargets(s, className);
        const fill = settings.color || theme.primary;
        const text = settings.textColor || theme.soft;
        const glow = settings.hoverColor || theme.accent;
        const outline = settings.outlineColor || "rgba(255,255,255,.28)";
        const width = Math.round(120 + param(settings, "width", 50) * 1.3);
        const height = Math.round(44 + param(settings, "height", 44) * .28);
        const radius = Math.round(22 + param(settings, "radius", 60) * .24);
        const lift = Math.round(4 + i * 12);
        const pulse = Math.max(1.6, 3.2 - i * 1.2).toFixed(2);
        return `${buttonAdapterBaseCss(s, className, text, text)} ${target}{width:${width}px!important;min-width:${width}px!important;height:${height}px!important;min-height:${height}px!important;padding:0 ${Math.round(height*.55)}px!important;overflow:hidden!important;border-radius:${radius}px!important;background:linear-gradient(to bottom,${hexToRgba(fill,.72)},${hexToRgba(fill,.38)})!important;border:1px solid ${outline}!important;box-shadow:inset 0 1px 2px rgba(255,255,255,.4),inset 0 -1px 2px rgba(0,0,0,.2),0 4px 8px rgba(0,0,0,.16),0 0 ${Math.round(14+i*20)}px ${hexToRgba(glow,.16)}!important;transform:rotateX(10deg) translateZ(0)!important;transition:transform .3s cubic-bezier(.34,1.56,.64,1),box-shadow .3s ease!important;perspective:1000px!important;animation:ssxBtnGlassPulse ${pulse}s ease-in-out infinite;} ${target} .ssx-btn-label{position:relative!important;z-index:2!important;font-weight:800!important;text-shadow:0 1px 2px rgba(0,0,0,.28);white-space:nowrap!important;} ${target}:before{content:"";position:absolute;top:0;left:-55px;width:55px;height:100%;background:linear-gradient(90deg,transparent,rgba(255,255,255,.12),rgba(255,255,255,.35),rgba(255,255,255,.12),transparent);transform:skewX(-25deg);animation:ssxBtnGlassShine 3s linear infinite;pointer-events:none;z-index:1;} ${target}:hover{transform:rotateX(0deg) translateY(-${Math.round(lift*.35)}px) scale(1.03)!important;box-shadow:inset 0 1px 2px rgba(255,255,255,.44),inset 0 -1px 2px rgba(0,0,0,.18),0 ${Math.round(7+i*8)}px ${Math.round(16+i*18)}px rgba(0,0,0,.22),0 0 ${Math.round(24+i*24)}px ${hexToRgba(glow,.28)}!important;} ${target}:active{transform:scale(.96)!important;} @keyframes ssxBtnGlassShine{0%{left:-60px}100%{left:260px}} @keyframes ssxBtnGlassPulse{0%,100%{filter:saturate(1)}50%{filter:saturate(1.12)}}`;
      }
    },
    {
      id: "btn-style-premium-glass",
      label: "Premium glass",
      description: "Layered soft-glass pill with conic edge, highlight and press depth.",
      intensityLabel: "Depth",
      preview: "button-glass",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-premium-glass";
        const target = buttonClassTargets(s, className);
        const fill = settings.color || theme.soft;
        const text = settings.textColor || theme.ink;
        const outline = settings.outlineColor || theme.primary;
        const shine = settings.hoverColor || theme.accent;
        const radius = Math.round(26 + param(settings, "radius", 70) * .38);
        const blur = Math.round(6 + i * 10);
        return `${buttonAdapterBaseCss(s, className, text, text)} ${buttonAdapterStableSizeCss(s, className, settings)} ${target}{--ssx-btn-angle:35deg;overflow:hidden!important;border-radius:${radius}px!important;background:linear-gradient(-75deg,${hexToRgba(fill,.32)},${hexToRgba("#ffffff",.62)},${hexToRgba(fill,.28)})!important;color:${text}!important;border:1px solid ${hexToRgba(outline,.22)}!important;box-shadow:inset 0 2px 2px rgba(0,0,0,.04),inset 0 -2px 2px rgba(255,255,255,.55),0 7px 18px ${hexToRgba(outline,.13)},0 0 0 1px rgba(255,255,255,.45) inset!important;backdrop-filter:blur(${blur}px)!important;-webkit-backdrop-filter:blur(${blur}px)!important;transition:transform .36s cubic-bezier(.25,1,.5,1),box-shadow .36s cubic-bezier(.25,1,.5,1),filter .36s ease!important;} ${target} .ssx-btn-label{position:relative!important;z-index:3!important;font-weight:800!important;text-shadow:0 .18em .08em rgba(0,0,0,.12);white-space:nowrap!important;} ${target}:before{content:"";position:absolute;inset:0;border-radius:inherit;background:linear-gradient(110deg,transparent 0%,${hexToRgba(shine,.38)} 42%,transparent 58%);background-size:230% 100%;background-position:0% 50%;mix-blend-mode:screen;transition:background-position .55s cubic-bezier(.25,1,.5,1);z-index:2;pointer-events:none;} ${target}:after{content:"";position:absolute;inset:0;border-radius:inherit;padding:2px;background:conic-gradient(from var(--ssx-btn-angle),${hexToRgba(outline,.58)},transparent 22%,${hexToRgba("#ffffff",.55)} 50%,transparent 78%,${hexToRgba(outline,.58)});-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude;transition:transform .45s ease;pointer-events:none;z-index:1;} ${target}:hover{transform:scale(.985)!important;box-shadow:inset 0 1px 2px rgba(0,0,0,.05),inset 0 -1px 2px rgba(255,255,255,.7),0 5px 14px ${hexToRgba(outline,.18)},0 0 0 1px rgba(255,255,255,.55) inset!important;} ${target}:hover:before{background-position:60% 50%;} ${target}:active{transform:rotateX(12deg) scale(.97)!important;}`;
      }
    },
    {
      id: "btn-style-orbit-text",
      label: "Orbit text",
      description: "Circular CTA with rotating label text around a center arrow.",
      intensityLabel: "Spin",
      preview: "motion-fixed",
      css: (s, i, settings, theme) => {
        const className = "ssx-btn-orbit-text";
        const target = buttonClassTargets(s, className);
        const fill = settings.color || theme.primary;
        const text = settings.textColor || theme.soft;
        const center = settings.hoverColor || theme.soft;
        const centerText = settings.outlineColor || theme.primary;
        const size = Math.round(92 + param(settings, "size", 50) * .9);
        const inner = Math.round(size * .42);
        const speed = Math.max(4, 13 - i * 7).toFixed(1);
        return `${buttonAdapterBaseCss(s, className, text, text)} ${target}{width:${size}px!important;min-width:${size}px!important;height:${size}px!important;min-height:${size}px!important;padding:0!important;border-radius:50%!important;overflow:hidden!important;display:grid!important;place-content:center!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(settings.outlineColor || theme.accent,.24)}!important;box-shadow:0 14px 34px ${hexToRgba(fill,.18)}!important;transition:background .3s ease,transform .2s ease!important;} ${target}:hover{background:${settings.outlineColor || theme.ink}!important;transform:scale(1.04)!important;} ${target} .ssx-btn-orbit-ring{position:absolute!important;inset:0!important;animation:ssxBtnOrbitText ${speed}s linear infinite;z-index:2!important;} ${target} .ssx-btn-orbit-ring span{position:absolute!important;left:50%!important;top:7px!important;transform-origin:0 ${Math.round(size/2 - 7)}px!important;transform:rotate(calc(360deg / var(--total) * var(--index)))!important;font-size:${Math.round(size*.095)}px!important;font-weight:800!important;line-height:1!important;color:${text}!important;} ${target} .ssx-btn-orbit-center{position:relative!important;z-index:3!important;width:${inner}px!important;height:${inner}px!important;overflow:hidden!important;background:${center}!important;color:${centerText}!important;border-radius:50%!important;display:flex!important;align-items:center!important;justify-content:center!important;} ${target} .ssx-btn-orbit-center svg{width:${Math.round(inner*.48)}px!important;height:${Math.round(inner*.48)}px!important;fill:currentColor!important;transition:transform .3s ease!important;} ${target} .ssx-btn-icon-copy{position:absolute!important;transform:translate(-150%,150%);} ${target}:hover .ssx-btn-orbit-center svg:first-child{transform:translate(150%,-150%);} ${target}:hover .ssx-btn-icon-copy{transform:translate(0);} @keyframes ssxBtnOrbitText{to{transform:rotate(360deg)}}`;
      }
    },
    {
      id: "btn-style-arrow-push",
      label: "Arrow push",
      description: "Clean pill CTA with arrow nudge and press feedback.",
      intensityLabel: "Push",
      preview: "button-arrow",
      css: (s, i, settings, theme) => {
        ...
        const text = settings.textColor || theme.ink;
        const hover = settings.hoverColor || theme.gradientB;
        const radius = Math.round(28 + param(settings, "radius", 75) * .52);
        const move = Math.round(3 + i * 7);
        return `${buttonAdapterBaseCss(s, className, text, text)} ${buttonAdapterStableSizeCss(s, className, settings)} ${target}{gap:10px!important;overflow:hidden!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(settings.outlineColor || fill,.35)}!important;border-radius:${radius}px!important;font-weight:900!important;transition:background .2s ease,transform .2s ease,box-shadow .2s ease!important;box-shadow:0 10px 24px ${hexToRgba(fill,.14)}!important;} ${target}:hover{background:${hover}!important;box-shadow:0 12px 28px ${hexToRgba(hover,.18)}!important;} ${target}:active{transform:scale(.95)!important;} ${target} .ssx-btn-label{position:relative!important;z-index:2!important;white-space:nowrap!important;} ${target} svg{position:relative!important;z-index:2!important;width:34px!important;height:34px!important;margin-left:2px!important;fill:currentColor!important;transition:transform .3s ease!important;} ${target}:hover svg{transform:translateX(${move}px);}`;
      }
    },
    {
      id: "btn-style-circle-360",
      label: "Circle 360",
      description: "Circular CTA with a rotating ring and optional custom label.",
      intensityLabel: "Spin",
      preview: "motion-fixed",
      css: (s, i, settings, theme) => {
        const fill = settings.color || theme.primary;
        const outline = settings.outlineColor || theme.accent;
        const text = settings.textColor || theme.soft;
        const label = String(settings.customText || "").trim().slice(0, 18);
        const size = Math.round(76 + param(settings, "size", 55) * .86);
        const speed = Math.max(2.8, 10 - i * 6.4).toFixed(1);
        const textRule = label ? `font-size:0!important;color:transparent!important;` : "";
        const hiddenInnerText = label ? `${buttonTextTargets(s)}{font-size:0!important;color:transparent!important;}` : "";
        const afterContent = label ? `"${escapeCssString(label)}"` : "attr(aria-label)";
        return `${buttonBaseCss(s, text, text)} ${buttonStableSizeCss(s, settings)} ${hiddenInnerText} ${buttonTargets(s)}{position:relative!important;isolation:isolate!important;overflow:visible!important;width:${size}px!important;min-width:${size}px!important;height:${size}px!important;min-height:${size}px!important;padding:0!important;display:inline-grid!important;place-items:center!important;border-radius:999px!important;background:${fill}!important;color:${text}!important;border:1px solid ${hexToRgba(outline,.38)}!important;box-shadow:0 ${Math.round(12+i*26)}px ${Math.round(28+i*54)}px ${hexToRgba(fill,.22)}!important;${textRule}} ${buttonTargets(s)}:before{content:"";position:absolute;inset:-9px;border-radius:999px;border:2px dashed ${outline};animation:ssxBtnCircleSpin ${speed}s linear infinite;pointer-events:none;z-index:0;} ${buttonTargets(s)}:after{content:${afterContent};position:absolute;inset:0;display:grid;place-items:center;color:${text};font-size:${Math.round(11+param(settings,"textSize",46)*.08)}px;font-weight:900;letter-spacing:0;text-align:center;line-height:1.05;padding:12px;z-index:3;pointer-events:none;} ${buttonTargets(s)}:hover:before{animation-duration:${Math.max(1.2, Number(speed)*.45).toFixed(1)}s;} @keyframes ssxBtnCircleSpin{to{transform:rotate(360deg)}}`;
      }
    },
    {
      id: "btn-style-soft-pill",
      label: "Soft pill",
      description: "Calm rounded button for clean service websites.",
      intensityLabel: "Softness",
      preview: "button-radius",
      css: (s, i, settings, theme) => `${buttonBaseCss(s, settings.textColor || theme.primary, settings.hoverTextColor || theme.ink)} ${buttonStableSizeCss(s, settings)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;background:${settings.color || theme.soft}!important;color:${settings.textColor || theme.primary}!important;border:1px solid ${hexToRgba(settings.outlineColor || theme.primary,.12+i*.22)}!important;border-radius:999px!important;box-shadow:0 ${Math.round(6+i*15)}px ${Math.round(16+i*35)}px ${hexToRgba(theme.primary,.08+i*.08)}!important;transition:background .22s ease,color .22s ease,box-shadow .22s ease!important;} ${buttonTargets(s)}:hover{background:${settings.hoverColor || theme.accent}!important;color:${settings.hoverTextColor || theme.ink}!important;}`
    },
    {
      id: "btn-style-sketch-border",
      label: "Sketch border",
      description: "Handmade-style border with playful hover movement.",
      intensityLabel: "Wobble",
      preview: "button-3d",
      css: (s, i, settings, theme) => `${buttonBaseCss(s, settings.textColor || theme.ink, settings.textColor || theme.ink)} ${buttonStableSizeCss(s, settings)} ${buttonTargets(s)}{position:relative!important;overflow:hidden!important;background:${settings.color || "transparent"}!important;color:${settings.textColor || theme.ink}!important;border:${Math.max(2,Math.round(2+i*2))}px solid ${settings.outlineColor || theme.ink}!important;border-radius:${Math.round(8+param(settings,"radius",28)*.36)}px ${Math.round(18+param(settings,"radius",28)*.48)}px ${Math.round(10+param(settings,"radius",28)*.3)}px ${Math.round(15+param(settings,"radius",28)*.42)}px!important;box-shadow:${Math.round(3+i*5)}px ${Math.round(4+i*6)}px 0 ${settings.outlineColor || theme.ink}!important;transition:box-shadow .18s ease!important;} ${buttonTargets(s)}:hover{box-shadow:${Math.round(1+i*2)}px ${Math.round(1+i*2)}px 0 ${settings.outlineColor || theme.ink}!important;}`
    }
  ];

  const allEffects = effectCatalog.flatMap((group) => group.items.map((effect) => ({ ...effect, group: group.group }))).concat(textEffectExtras.map((effect) => ({ ...effect, group: "Text" })), imageEffectExtras.map((effect) => ({ ...effect, group: "Image" })), siteCursorEffects.map((effect) => ({ ...effect, group: "Cursor" })), buttonStyleEffects.map((effect) => ({ ...effect, group: "Buttons" })));
  ...
  const featureLibrary = [
    ...
    { id: "button-suite", title: "Button styles", target: ["button", "section", "page", "text", "image"], icon: "▭", description: "Customizable button presets for one button or all page buttons.", options: buttonStyleEffects.map((effect) => effect.id) },
  ...
  function buttonAdapterClassFor(effectId) {
    return {
      "btn-style-letter-flip": "ssx-btn-letter-flip",
      "btn-style-arrow-bubble": "ssx-btn-arrow-bubble",
      "btn-style-swap-arrow": "ssx-btn-swap-arrow",
      "btn-style-glass-tilt": "ssx-btn-glass-tilt",
      "btn-style-premium-glass": "ssx-btn-premium-glass",
      "btn-style-orbit-text": "ssx-btn-orbit-text",
      "btn-style-arrow-push": "ssx-btn-arrow-push"
    }[effectId] || "";
  }
  ...
  function setupButtonStyleAdapters() {
      ...
      buttonStyleIds.forEach((effectId) => {
        ...
        try {
          document.querySelectorAll(buttonTargets(selector)).forEach((button) => {
            if (!button.closest(`#${APP_ID}`)) active.set(button, { effectId, className, settings: effects[effectId] });
          });
        } catch {}
      });
    });
    ...
    active.forEach(({ effectId, className, settings }, button) => {
      const signature = `${effectId}:${settings?.customText || ""}`;
      if (button.dataset.ssxButtonAdapter === effectId && button.dataset.ssxButtonSignature === signature) return;
      restoreButtonAdapter(button);
      applyButtonAdapter(button, effectId, className, settings, signature);
    });
  }

  function restoreButtonAdapter(button) {
    if (!button || !button.dataset?.ssxButtonAdapter) return;
    ["ssx-btn-letter-flip", "ssx-btn-arrow-bubble", "ssx-btn-swap-arrow", "ssx-btn-glass-tilt", "ssx-btn-premium-glass", "ssx-btn-orbit-text", "ssx-btn-arrow-push", "ssx-button-adapter"].forEach((className) => button.classList.remove(className));
  ...
  function applyButtonAdapter(button, effectId, className, settings = {}, signature = effectId) {
    const label = (button.textContent || button.getAttribute("aria-label") || "Button").trim() || "Button";
    ...
    if (effectId === "btn-style-arrow-bubble") {
      button.textContent = "";
      button.appendChild(makeArrowIcon("ssx-btn-icon ssx-btn-icon-left"));
      const text = document.createElement("span");
      text.className = "ssx-btn-label";
      text.textContent = label;
      button.appendChild(text);
      const fill = document.createElement("span");
      fill.className = "ssx-btn-fill";
      button.appendChild(fill);
      button.appendChild(makeArrowIcon("ssx-btn-icon ssx-btn-icon-right"));
      return;
    }
    ...
    if (effectId === "btn-style-orbit-text") {
      const orbitLabel = String(settings.customText || label || "STUDIO POEMA").trim() || "STUDIO POEMA";
      button.textContent = "";
      const textRing = document.createElement("span");
      textRing.className = "ssx-btn-orbit-ring";
      const letters = Array.from(`${orbitLabel} • `).slice(0, 24);
      letters.forEach((char, index) => {
        const span = document.createElement("span");
        span.style.setProperty("--index", String(index));
        span.style.setProperty("--total", String(letters.length));
        span.textContent = char === " " ? "\u00a0" : char;
        textRing.appendChild(span);
      });
      button.appendChild(textRing);
      const circle = document.createElement("span");
      circle.className = "ssx-btn-orbit-center";
      circle.appendChild(makeArrowSvg());
      const copy = makeArrowSvg();
      copy.classList.add("ssx-btn-icon-copy");
      circle.appendChild(copy);
  ...
  function makeCircleArrowIcon(className) {
    ...
    icon.className = className;
    icon.setAttribute("aria-hidden", "true");
    icon.innerHTML = '<svg viewBox="0 0 24 24" focusable="false"><circle fill="currentColor" r="11" cy="12" cx="12"></circle><path stroke-linejoin="round" stroke-linecap="round" stroke-width="2" stroke="white" fill="none" d="M7.5 16.5L16.5 7.5M16.5 7.5H10.5M16.5 7.5V13.5"></path></svg>';
    return icon;
  }
...
`;
  ...
  function setupCursorFollower(theme) {
    ...
    cursorMoveHandler = (event) => {
      ...
      const sizeSource = spotlight || glow || crosshair || outline || trail || ring || dot || large || emoji || matched;
      ...
      <div class="ssx-panel" role="dialog" aria-label="Studio Poema">
        <div class="ssx-head" data-ssx-drag>
          ...
          <div>
            ...
            <div class="ssx-subtitle">v61 · more button adapters</div>
          </div>
  ...
  function renderTextControls(section, effect, settings, disabled) {
    ...
    if (section.id === "image-cursor") {
      const selectedCursor = /spotlight/i.test(effect.id) ? "Spotlight" : /crosshair/i.test(effect.id) ? "Crosshair" : /glow/i.test(effect.id) ? "Glow" : /trail/i.test(effect.id) ? "Dot + follower" : /ring/i.test(effect.id) ? "Ring" : /dot/i.test(effect.id) ? "Soft dot" : /large/i.test(effect.id) ? "Large dot" : /emoji/i.test(effect.id) ? "Emoji" : "Custom";
      return `
  ...
  function renderButtonFeatureControls(effect, settings, disabled) {
    const theme = config.theme || defaultConfig.theme;
    const isRoundButton = effect.id === "btn-style-circle-360" || effect.id === "btn-style-orbit-text";
    const radius = param(settings, "radius", isRoundButton ? 50 : 55);
    const size = param(settings, "size", 55);
    const textSize = param(settings, "textSize", 46);
    ...
    return `
      <div class="ssx-option-controls">
        ...
        <div>
          ...
          ${effect.id !== "btn-style-circle-360" && effect.id !== "btn-style-orbit-text" ? `
            <div class="ssx-range-row">
          ...
          ` : ""}
          ${effect.id === "btn-style-circle-360" || effect.id === "btn-style-orbit-text" ? `
            <label class="ssx-control-line"><span>${effect.id === "btn-style-orbit-text" ? "Orbit text" : "Circle text"}</span><input type="text" value="${escapeHtml(settings.customText || "")}" data-ssx-custom-text-effect="${escapeHtml(effect.id)}" maxlength="${effect.id === "btn-style-orbit-text" ? "24" : "18"}" placeholder="${effect.id === "btn-style-orbit-text" ? "STUDIO POEMA" : "POEMA"}" ${disabled ? "disabled" : ""}></label>
            <div class="ssx-range-row">
              <input type="range" min="0" max="100" value="${size}" data-ssx-param-effect="${escapeHtml(effect.id)}" data-ssx-param-key="size" ${disabled ? "disabled" : ""}>
              <div class="ssx-range-label">${effect.id === "btn-style-orbit-text" ? "Orbit size" : "Circle size"} · ${effect.id === "btn-style-orbit-text" ? Math.round(92 + size * .9) : Math.round(76 + size * .86)}px</div>
            </div>
            ${effect.id === "btn-style-circle-360" ? `<div class="ssx-range-row">
              <input type="range" min="0" max="100" value="${textSize}" data-ssx-param-effect="${escapeHtml(effect.id)}" data-ssx-param-key="textSize" ${disabled ? "disabled" : ""}>
              <div class="ssx-range-label">Text size · ${Math.round(11 + textSize * .08)}px</div>
            </div>` : ""}
          ` : `
            <div class="ssx-range-row">
  ...
  function bindPanel(root) {
    ...
    root.querySelectorAll("[data-ssx-feature-effect]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (!current.enabled && feature.id === "button-suite" && buttonStyleIds.includes(effectId)) {
          buttonStyleIds.filter((id) => id !== effectId).forEach((id) => {
            const otherEffect = effectById.get(id);
            if (otherEffect) setEffectSetting(currentScopeTarget(otherEffect), id, { enabled: false, intensity: getEffectSettings(currentScopeTarget(otherEffect), id).intensity || 60 });
          });
        }
    ...
    root.querySelectorAll("[data-ssx-custom-text-effect]").forEach((input) => {
      input.addEventListener("input", () => {
        ...
        const autoEnable = /^(txt|img|site)-cursor-emoji$/.test(input.dataset.ssxCustomTextEffect) || /^(btn-style-circle-360|btn-style-orbit-text|btn-style-swap-arrow)$/.test(input.dataset.ssxCustomTextEffect);
  ...
  function cleanRuntimeSrc() {
    const fallback = `https://cdn.jsdelivr.net/gh/evaldasbanevici-del/SquarePlugin@main/studio-poema-v61.js?v=${defaultConfig.version}`;
    try {
